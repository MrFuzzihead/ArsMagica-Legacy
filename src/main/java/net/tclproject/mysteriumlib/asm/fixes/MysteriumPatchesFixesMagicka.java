...
public class MysteriumPatchesFixesMagicka {
    ...
    public static boolean isPlayerEthereal(EntityPlayer entityPlayer) {
        ...
        if (entityPlayer.inventory.armorInventory[0] != null) {
            if (ItemSoulspike.bootsHaveEtherealTag(entityPlayer.inventory.armorInventory[0])) {
                return true;
            }
        }
        return false;
    ...
    public static List selectEntitiesWithinAABB(World wrld, Class p_82733_1_, AxisAlignedBB p_82733_2_,
        IEntitySelector p_82733_3_, @ReturnedValue List returnedValue) {
    ...
    @Fix(returnSetting = EnumReturnSetting.ALWAYS, insertOnExit = true)
    public static EntityPlayer getClosestPlayer(World world, double p_72977_1_, double p_72977_3_, double p_72977_5_,
        double p_72977_7_, @ReturnedValue EntityPlayer returnedValue) {
        return isPlayerEthereal(returnedValue) ? null : returnedValue;
    }

    // same as last method: prevent ethereal players from being returned
    @Fix(returnSetting = EnumReturnSetting.ALWAYS, insertOnExit = true)
    public static List getEntitiesWithinAABBExcludingEntity(World world, Entity p_94576_1_, AxisAlignedBB p_94576_2_,
        IEntitySelector p_94576_3_, @ReturnedValue List returnedValue) {
        ArrayList toReturn = new ArrayList();
        for (int i = 0; i < returnedValue.size(); i++) {
            if (i >= returnedValue.size()) break; // because we now know how volatile entity lists are.
            if (!(returnedValue.get(i) instanceof EntityPlayer)) {
    ...
    @Fix
    public static void updateRenderer(WorldRenderer wr, EntityLivingBase p_147892_1_) {
        updatingRenderWorld = true;
    }

    @Fix(insertOnExit = true, targetMethod = "updateRenderer")
    public static void updateRendererEnd(WorldRenderer wr, EntityLivingBase p_147892_1_) {
        updatingRenderWorld = false;
    }

    @Fix
    public static void orientCamera(EntityRenderer er, float p_78467_1_) {
        orientingCamera = true;
    }

    @Fix(insertOnExit = true, targetMethod = "orientCamera")
    public static void orientCameraEnd(EntityRenderer er, float p_78467_1_) {
    ...
    public static boolean isEtherealMinecraft() {
        if (Minecraft.getMinecraft() != null && Minecraft.getMinecraft().thePlayer != null
            && Minecraft.getMinecraft().renderViewEntity != null) {
            if (orientingCamera && isPlayerEthereal(Minecraft.getMinecraft().thePlayer)
                && Minecraft.getMinecraft().renderViewEntity == Minecraft.getMinecraft().thePlayer) return true;
        }
        return false;
    }
    ...
    public static boolean getEntityRenderObject(RenderManager rm, Entity ent) {
        if (ent instanceof EntityPlayer) {
            if (playerModelMap.get(((EntityPlayer) ent).getCommandSenderName()) != null
                && playerModelMap.get(((EntityPlayer) ent).getCommandSenderName())
                    .startsWith("maid"))
                return true;
        }
        return false;
    ...
    public static boolean register(EventBus eb, Object target) {
        if (target.getClass()
            ...
            LogHelper.warn("--------------------------!!!--------------------------");
            LogHelper.warn(
                "The Ears mod transformer was found and disabled. This most likely means you're using FoamFix's hacky 1.8+ skin support.");
            LogHelper.warn(
                "Please disable it in FoamFix's config and use (preferably, as its solution is much more compatible and elegant) EtFuturumRequiem, or (if need be) SkinPort instead.");
            LogHelper.warn("--------------------------!!!--------------------------");
    ...
    public static boolean processClickWindow(NetHandlerPlayServer nhps, C0EPacketClickWindow p_147351_1_) {
        if (isPlayerEthereal(nhps.playerEntity)) {
            ...
            if (nhps.playerEntity.openContainer.windowId == p_147351_1_.func_149548_c()
                && nhps.playerEntity.openContainer.isPlayerNotUsingContainer(nhps.playerEntity)) {
    ...
    public static boolean returnFalse(Entity thisEntity) {
        return false;
    }
    ...
    public static boolean getRenderType(Block block) { // this is alright, we don't want to change overridden classes so
                                                       // allthatextends isn't necessary
    ...
    public static boolean isInvisibleToPlayer(Entity e, EntityPlayer p_98034_1_) {
        return (p_98034_1_.isPotionActive(BuffList.trueSight.id) || isPlayerEthereal(p_98034_1_)) ? false
            : e.isInvisible();
    }
    ...
    public static void onUpdate(EntityItem ei) {
        if (ei.getEntityItem() != null) {
            if (ei.getEntityItem()
                ...
                && ei.getEntityItem()
                    .getItemDamage() == ItemsCommonProxy.itemOre.META_COGNITIVEDUST) {
                ei.moveEntity(ei.motionX, ei.motionY, ei.motionZ);
                ei.motionY *= 0.0500000011920929D;
                ei.motionY += 0.025D;
            }
            if (ei.worldObj.isMaterialInBB(
                ei.boundingBox.expand(-0.10000000149011612D, -0.4000000059604645D, -0.10000000149011612D),
                liquidEssenceMaterial)) {
                double d0 = ei.posY;
                ei.moveEntity(ei.motionX, ei.motionY, ei.motionZ);
                ...
                if (ei.isCollidedHorizontally && ei.isOffsetPositionInLiquid(
                    ei.motionX,
                    ei.motionY + 0.6000000238418579D - ei.posY + d0,
                    ei.motionZ)) {
    ...
    @Fix(returnSetting = EnumReturnSetting.ALWAYS)
    public static boolean hasWorldObj(TileEntity te) // very roundabout way to slow down tileentity ticks. The TE won't
                                                     // update if hasWorldObj in it returns false. it's only used in
                                                     // rendering chests otherwise, so there's no harm in overwriting
                                                     // this function.
    {
        if (te.getWorldObj() != null) {
            String thistile = te.xCoord + "_"
                + te.yCoord
                + "_"
                + te.zCoord
                + "_"
    ...
    @SideOnly(Side.CLIENT)
    @Fix(returnSetting = EnumReturnSetting.ON_TRUE)
    public static boolean setLivingAnimations(ModelMagmaCube mmc, EntityLivingBase elb, float p_78086_2_,
        float p_78086_3_, float p_78086_4_) {
    ...
    @SideOnly(Side.CLIENT)
    @Fix(returnSetting = EnumReturnSetting.ON_TRUE)
    public static boolean setLivingAnimations(ModelSkeleton mmc, EntityLivingBase elb, float p_78086_2_,
        float p_78086_3_, float p_78086_4_) {
        if (elb instanceof EntityHallucination) return true;
        return false;
    }
    ...
    // try {
    // Field field = CustomLoadingPlugin.isObfuscated() ? Tessellator.class.getDeclaredField("field_78415_z") :
    // Tessellator.class.getDeclaredField("isDrawing");
    // field.setAccessible(true);
    // return MethodHandles.publicLookup().unreflectGetter(field);
    // } catch (Exception e) {
    // LogHelper.error("Did not find tesselator isDrawing field! Only report this error if you're seeing it on a
    // client!");
    // return null;
    // }
    // }
    ...
    // } catch (Throwable e) {
    // throw new RuntimeException("Could not invoke isDrawing field! Only report this error if you're seeing it on a
    // client!", e);
    // }
    // if (isDrawing)
    // {
    ...
    public static void changeClientTickrate(float newtickrate) {
        ...
        try {
            if (timerField == null) {
                for (Field fld : mc.getClass()
                    .getDeclaredFields()) {
                    if (fld.getType() == net.minecraft.util.Timer.class) { // have to type out the whole class,
                                                                           // otherwise it assumes it's the java.util
                                                                           // timer
                        timerField = fld;
    ...
    public static void changeServerTickrate(float newtickrate) {
        servertickrate = (long) (1000L / newtickrate); // 1000 milis in a second. 20 ticks a second. = 50 milis per
                                                       // tick.
    }
    ...
    public static boolean isBlockIndirectlyGettingPowered(World world, int x, int y, int z) {
        ...
        for (int[] redstoneProvider : providingRedstone) {
            if (redstoneProvider[0] == theID && redstoneProvider[1] == x
                && redstoneProvider[2] == y
                && redstoneProvider[3] == z) {
                toReturn = true;
                break;
            }
            counter++;
        ...
        if (toReturn) {
            ...
            world.getBlock(x, y, z)
                .onNeighborBlockChange(world, x, y, z, Blocks.stonebrick);
            return true;
        }
        return false;
    }
    ...
    @Fix
    public static void updateTick(BlockDynamicLiquid bdl, World p_149674_1_, int p_149674_2_, int p_149674_3_,
        int p_149674_4_, Random p_149674_5_) {
        if (bdl.getMaterial() == Material.lava
            && p_149674_1_.getBlock(p_149674_2_, p_149674_3_ - 1, p_149674_4_) instanceof BlockLiquidEssence) {
    ...
    @Fix(returnSetting = EnumReturnSetting.ON_TRUE)
    public static boolean velocityToAddToEntity(BlockLiquid block, World p_149640_1_, int p_149640_2_, int p_149640_3_,
        int p_149640_4_, Entity e, Vec3 p_149640_6_) {
        if (e instanceof EntityPlayer) {
            if (AffinityHelper.isNotInWaterActually.contains((EntityPlayer) e)) {
                return true;
            }
    ...
    private static final ResourceLocation rodtexture1 = new ResourceLocation(
        "arsmagica2",
        "textures/items/particles/particleswitharcanerod.png");
    private static final ResourceLocation rodtexture2 = new ResourceLocation(
        "arsmagica2",
        "textures/items/particles/particleswithinfernalrod.png");
    ...
    public static boolean getEntityTexture(RenderFish r, EntityFishHook e) {
        if (Minecraft.getMinecraft().thePlayer != null) {
            if (Minecraft.getMinecraft().thePlayer.getHeldItem() != null) {
                Item heldItem = Minecraft.getMinecraft().thePlayer.getHeldItem()
                    .getItem();
    ...
    @Fix(returnSetting = EnumReturnSetting.ON_TRUE)
    public static boolean doRenderShadowAndFire(Render r, Entity ent, double p_76979_2_, double p_76979_4_,
        double p_76979_6_, float p_76979_8_, float p_76979_9_) {
    ...
    public static ResourceLocation getEntityTextureResult(RenderFish r, EntityFishHook e) {
        if (Minecraft.getMinecraft().thePlayer.getHeldItem()
            .getItem() instanceof ItemArcaneFishingRod) {
            return rodtexture1;
        } else {
            return rodtexture2;
        }
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (p_82787_1_.getStackInRowAndColumn(i, j) != null) {
                    if (p_82787_1_.getStackInRowAndColumn(i, j)
                        .getItem() instanceof ItemSpellStaff) {
                        if (!((ItemSpellStaff) p_82787_1_.getStackInRowAndColumn(i, j)
                            .getItem()).isMagiTechStaff()) {
                            craftingCompsStaffs++;
                            if (staffSlotTo == -1) {
                                staffSlotTo = i;
                                staffSlotColumnTo = j;
                    ...
                    } else if (p_82787_1_.getStackInRowAndColumn(i, j)
                        .getItem() instanceof SpellBase) {
                            craftingCompsSpells++;
                            spellSlotFrom = i;
                            spellSlotColumnFrom = j;
                        } else if (p_82787_1_.getStackInRowAndColumn(i, j)
                            .getItem() instanceof BoundArmor) {
                                craftingCompsArmor++;
                                armorSlotTo = i;
                                armorSlotColumnTo = j;
                            }
    ...
    public static ItemStack findMatchingRecipeResult(CraftingManager cm, InventoryCrafting p_82787_1_,
        World p_82787_2_) {

        if (craftingStaffsPossible) {
            ItemStack result = ItemSpellStaff.copyChargeFrom(
                p_82787_1_.getStackInRowAndColumn(staffSlotTo, staffSlotColumnTo)
                    .copy(),
            ...
            ItemStack result = BoundArmor.setSpell(
                p_82787_1_.getStackInRowAndColumn(armorSlotTo, armorSlotColumnTo)
                    .copy(),
                p_82787_1_.getStackInRowAndColumn(spellSlotFrom, spellSlotColumnFrom));
            armorSlotTo = -1;
            armorSlotColumnTo = -1;
