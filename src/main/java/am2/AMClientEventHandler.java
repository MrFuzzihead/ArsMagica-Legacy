...
public class AMClientEventHandler {
    ...
    public void onBlockHighlight(DrawBlockHighlightEvent event) {
        ...
        if (AMCore.proxy.getLocalPlayer()
            .getCurrentArmor(3) != null
            && (AMCore.proxy.getLocalPlayer()
                .getCurrentArmor(3)
                .getItem() instanceof IManaGoggle || ArmorHelper.isInfusionPreset(
                    AMCore.proxy.getLocalPlayer()
                        .getCurrentArmor(3),
            ...
            if (te instanceof IPowerNode) {
                AMCore.proxy
                    .setTrackedLocation(new AMVector3(event.target.blockX, event.target.blockY, event.target.blockZ));
            } else {
        ...
        if (data != null) {
            ...
            for (PowerTypes type : ((IPowerNode) te).getValidPowerTypes()) {
                ...
                AMVector3 offset = new AMVector3(
                    ...
                    event.target.blockZ + 0.5).sub(
                        new AMVector3(
                            (event.player.prevPosX - (event.player.prevPosX - event.player.posX) * event.partialTicks),
                            (event.player.prevPosY - (event.player.prevPosY - event.player.posY) * event.partialTicks)
                                + event.player.getEyeHeight(),
                            (event.player.prevPosZ
                                - (event.player.prevPosZ - event.player.posZ) * event.partialTicks)));
                offset = offset.normalize();
                if (event.target.blockY <= event.player.posY - 0.5) {
                    RenderUtilities.drawTextInWorldAtOffset(
                        String.format("%s%.2f (%.2f%%)", type.chatColor(), pwr, pct),
                        event.target.blockX
                            - (event.player.prevPosX - (event.player.prevPosX - event.player.posX) * event.partialTicks)
                            + 0.5f
                            - offset.x,
                        event.target.blockY + yOff
                            - (event.player.prevPosY - (event.player.prevPosY - event.player.posY) * event.partialTicks)
                            + block.getBlockBoundsMaxY() * 0.8f,
                        event.target.blockZ
    ...
    private void renderPriorityText(DrawBlockHighlightEvent event, TileEntityCrystalMarker te) {
        int meta = event.player.worldObj
            .getBlockMetadata(event.target.blockX, event.target.blockY, event.target.blockZ);
        if (meta == BlockCrystalMarker.META_IN) // no priority for these blocks
            return;

        Block block = event.player.worldObj.getBlock(event.target.blockX, event.target.blockY, event.target.blockZ);
        float yOff = 0.5f;
        String priString = String
            .format(StatCollector.translateToLocal("am2.tooltip.priority"), String.format("%d", te.getPriority()));
        ...
        AMVector3 offset = new AMVector3(
            ...
            event.target.blockZ + boundsOffset.z).sub(
                new AMVector3(
                    (event.player.prevPosX - (event.player.prevPosX - event.player.posX) * event.partialTicks),
                    (event.player.prevPosY - (event.player.prevPosY - event.player.posY) * event.partialTicks)
                        + event.player.getEyeHeight(),
        ...
        AMVector3 drawPos = new AMVector3(
            event.target.blockX + block.getBlockBoundsMaxX()
                - offset.x
                - (event.player.prevPosX - (event.player.prevPosX - event.player.posX) * event.partialTicks),
            event.target.blockY + yOff
                - (event.player.prevPosY - (event.player.prevPosY - event.player.posY) * event.partialTicks)
                + block.getBlockBoundsMaxY() * 0.8f,
            event.target.blockZ + block.getBlockBoundsMaxZ()
                - offset.z
                - (event.player.prevPosZ - (event.player.prevPosZ - event.player.posZ) * event.partialTicks));
        if (event.target.blockY > event.player.posY - 0.5) {
            drawPos.y = (float) (event.target.blockY - yOff
                - (event.player.prevPosY - (event.player.prevPosY - event.player.posY) * event.partialTicks)
                - block.getBlockBoundsMaxY() * 0.2f);
        }
    ...
    public void onItemTooltip(ItemTooltipEvent event) {
        ...
        if (stack != null && stack.getItem() instanceof ItemArmor) {
            ...
            if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
                event.toolTip
                    .add(StatCollector.translateToLocalFormatted("am2.tooltip.armorxp", String.format("%.2f", xp)));
            ...
            if (((ItemBlock) stack.getItem()).field_150939_a == BlocksCommonProxy.manaBattery) {
                if (stack.hasTagCompound()) {
                    ...
                    PowerTypes powerType = PowerTypes
                        .getByID(stack.stackTagCompound.getInteger("mana_battery_powertype"));
                    if (batteryCharge != 0) {
                        // TODO localize this tooltip
                        event.toolTip.add(
                            String.format(
                                "§r§9Contains §5%.2f %s%s §9etherium",
                                batteryCharge,
                                powerType.chatColor(),
                                powerType.name()));
                    }
    ...
    public void onGuiRender(RenderGameOverlayEvent event) {
        if (event.type == ElementType.CROSSHAIRS || event.type == ElementType.TEXT) AMCore.proxy.renderGameOverlay();
    }
    ...
    public void drawCompendiumHUD(Block block, MovingObjectPosition pos, ScaledResolution res) {
        ...
        if (block instanceof ICompendium) {
            String sentry = block.getUnlocalizedName()
                .replace("arsmagica2:", "")
                .replace("tile.", "");
    ...
    public void onPlayerRender(RenderPlayerEvent.Pre event) {
        ...
        if (ExtendedProperties.For(event.entityPlayer)
            .getFlipRotation() > 0) {
            GL11.glPushMatrix();
        ...
        float shrink = ExtendedProperties.For(event.entityPlayer)
            .getShrinkPct();
        if (shrink > 0) {
            GL11.glPushMatrix();
    ...
    public void onPlayerRender(RenderPlayerEvent.Post event) {
        ...
        if (event.entityPlayer == AMCore.proxy.getLocalPlayer()) {
            if (AMCore.proxy.getLocalPlayer()
                .isPotionActive(BuffList.trueSight.id)) {
                if (AMGuiHelper.instance.playerRunesAlpha < 1) AMGuiHelper.instance.playerRunesAlpha += 0.01f;
            } else {
                if (AMGuiHelper.instance.playerRunesAlpha > 0) AMGuiHelper.instance.playerRunesAlpha -= 0.01f;
            }
            ...
            if (AMGuiHelper.instance.playerRunesAlpha > 0) {
                ...
                for (int i = 0; i <= 16; ++i) {
                    int bit = 1 << i;
                    if ((runeCombo & bit) == bit) {
                        RenderUtilities.DrawIconInWorldAtOffset(
                            ItemsCommonProxy.rune.getIconFromDamage(i),
                            xOffset,
                            0.5f,
                            0,
                            0.25f,
    ...
    public void onEntityJoinWorldClient(EntityJoinWorldEvent event) {
        ...
        if (event.entity instanceof EntityPlayer) {
            ...
            List<String> tablist = ((List<GuiPlayerInfo>) Minecraft.getMinecraft().thePlayer.sendQueue.playerInfoList)
                .stream()
                .map(r -> r.name)
            ...
            if (event.entity == Minecraft.getMinecraft().thePlayer && tablist.isEmpty()) { // no players to sync the map
                                                                                           // from, clear it instead
                MysteriumPatchesFixesMagicka.playerModelMap.clear();
            }
        }
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (block == BlocksCommonProxy.particleEmitter) {
            TileEntity te = event.world.getTileEntity(
                event.x + Facing.offsetsXForSide[event.face],
                event.y + Facing.offsetsYForSide[event.face],
                event.z + Facing.offsetsZForSide[event.face]);
