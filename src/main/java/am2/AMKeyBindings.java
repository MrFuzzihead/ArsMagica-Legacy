...
public class AMKeyBindings {
    ...
    public static final KeyBinding ShapeGroupKey = new KeyBinding(
        "key.ShapeGroups",
        Keyboard.KEY_C,
        "key.am2.category");
    public static final KeyBinding SpellBookNextSpellKey = new KeyBinding(
        "key.SpellBookNext",
        Keyboard.KEY_X,
        "key.am2.category");
    public static final KeyBinding SpellBookPrevSpellKey = new KeyBinding(
        "key.SpellBookPrev",
        Keyboard.KEY_Z,
        "key.am2.category");
    public static final KeyBinding AuraCustomizationKey = new KeyBinding(
        "key.AuraCustomization",
        Keyboard.KEY_B,
        "key.am2.category");
    public static final KeyBinding ManaToggleKey = new KeyBinding(
        "key.ToggleManaDisplay",
        Keyboard.KEY_O,
        "key.am2.category");
    public static final KeyBinding AffinityActivationKey = new KeyBinding(
        "key.ActivateAffinityAbility",
        Keyboard.KEY_X,
        "key.am2.category");
    ...
    public void onKeyInput(KeyInputEvent event) {
        EntityPlayer clientPlayer = FMLClientHandler.instance()
            .getClient().thePlayer;
        ...
        if (ShapeGroupKey.isPressed()) {
            ...
            if (curItem.getItem() == ItemsCommonProxy.spellBook
                || curItem.getItem() == ItemsCommonProxy.arcaneSpellbook) {
                // send packet to server
                AMNetHandler.INSTANCE
                    .sendSpellbookSlotChange(player, player.inventory.currentItem, ItemSpellBook.ID_NEXT_SPELL);
            }
            ...
            if (curItem.getItem() == ItemsCommonProxy.spellBook
                ...
                // send packet to server
                AMNetHandler.INSTANCE
                    .sendSpellbookSlotChange(player, player.inventory.currentItem, ItemSpellBook.ID_PREV_SPELL);
            }
        } else if (AuraCustomizationKey.isPressed()) {
            if (PlayerTracker.hasAA(clientPlayer)) {
                Minecraft.getMinecraft()
                    .displayGuiScreen(new AuraCustomizationMenu());
            }
        } else if (AffinityActivationKey.isPressed()) {
            if (AffinityData.For(clientPlayer)
                .isAbilityReady()) {
