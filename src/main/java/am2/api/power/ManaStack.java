...
public class ManaStack {
    ...
    public ManaStack(Mana mana, int amount) {
        if (mana == null) {
            FMLLog.bigWarning(
                "Null mana supplied to manastack. Did you try and create a stack for an unregistered mana?");
            throw new IllegalArgumentException("Cannot create a manastack from a null mana");
        } else if (!ManaRegistry.isManaRegistered(mana)) {
            FMLLog.bigWarning(
                "Failed attempt to create a ManaStack for an unregistered Mana %s (type %s)",
                mana.getName(),
                mana.getClass()
                    .getName());
    ...
    public static boolean areManaStackTagsEqual(ManaStack stack1, ManaStack stack2) {
        return stack1 == null && stack2 == null
            || (stack1 != null && stack2 != null && stack1.isManaStackTagEqual(stack2));
    }
