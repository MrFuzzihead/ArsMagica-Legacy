...
public class EntityWinterGuardian extends AM2Boss {
    ...
    protected void initSpecificAI() {
        this.tasks.addTask(
            1,
            new EntityAICastSpell(
                this,
                NPCSpells.instance.dispel,
                16,
                23,
                50,
                BossActions.CASTING,
                new ISpellCastCallback<EntityWinterGuardian>() {

                    @Override
                    public boolean shouldCast(EntityWinterGuardian host, ItemStack spell) {
                        return host.getActivePotionEffects()
                            .size() > 0;
                    }
                }));
    ...
    public void onUpdate() {
        if (worldObj.getBiomeGenForCoords((int) posX, (int) posZ)
            .getEnableSnow()
            ...
            if (worldObj.isRemote) {
                AMParticle particle = (AMParticle) AMCore.proxy.particleManager.spawn(
                    worldObj,
                    "ember",
                    posX + (rand.nextFloat() * 6 - 3),
                    posY + 2 + (rand.nextFloat() * 2 - 1),
                    posZ + (rand.nextFloat() * 6 - 3));
                if (particle != null) {
        ...
        if (worldObj.isRemote) {
            ...
            if (this.ticksExisted % 100 == 0) {
                List<EntityLivingBase> entities = worldObj
                    .getEntitiesWithinAABB(EntityLivingBase.class, this.boundingBox.expand(2, 2, 2));
    ...
    private void spawnParticles() {
        for (int i = 0; i < AMCore.config.getGFXLevel() * 4; ++i) {
            ...
            AMParticle particle = (AMParticle) AMCore.proxy.particleManager.spawn(
                worldObj,
                rnd < 5 ? "snowflakes" : "ember",
                posX + (rand.nextFloat() * 0.4 - 0.2),
                posY + 2,
                posZ + (rand.nextFloat() * 0.4 - 0.2));
            if (particle != null) {
                if (rnd < 2 || rnd > 8) {
                    ...
                    particle.AddParticleController(
                        new ParticleFleeEntity(particle, this, 0.06f, 2, 2, false).setKillParticleOnFinish(true));
                }
                particle.setIgnoreMaxAge(false);
                particle.setMaxAge(30);
    ...
    protected float modifyDamageAmount(DamageSource source, float damageAmt) {
        if (source instanceof DamageSourceFrost) damageAmt = 0;
        if (source.isFireDamage() || source instanceof DamageSourceFire) damageAmt *= 2;
        return damageAmt;
    }
