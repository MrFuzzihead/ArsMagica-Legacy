...
public class EntityEnderGuardian extends AM2Boss implements IAnimatedEntity {
    ...
    public void onUpdate() {
        ...
        if (spawn == null) spawn = new AMVector3(this);

        wingFlapTime++;
        ticksSinceLastAttack++;

        if (this.motionY < 0) this.motionY *= 0.7999999f;
        ...
        switch (getCurrentAction()) {
            case LONG_CASTING: // roar
                ...
                break;
            case CHARGE:
                if (this.getTicksInCurrentAction() == 0) this.addVelocity(0, 1.5f, 0);
                break;
            default:
        }
    ...
    public void setAttackTarget(EntityLivingBase par1EntityLivingBase) {
        ...
        if (!worldObj.isRemote) {
            ...
            else this.dataWatcher.updateObject(ATTACK_TARGET, -1);
        }
    }
    ...
    public boolean attackEntityFrom(DamageSource par1DamageSource, float par2) {
        ...
        if (par1DamageSource.damageType.equals("outOfWorld")) {
            if (spawn != null) {
                ...
                if (!this.worldObj.isRemote) AMCore.proxy.addDeferredTargetSet(this, null);
            } else {
                this.setDead();
            }
            return false;
        ...
        if (!worldObj.isRemote && par1DamageSource.getSourceOfDamage() != null
            ...
            if (par1DamageSource.damageType == this.lastDamageType) {
                hitCount++;
                if (hitCount > 5) this.heal(par2 / 4);
                return false;
            } else {
    ...
    public EntityLivingBase getAttackTarget() {
        if (!worldObj.isRemote) return super.getAttackTarget();
        else return (EntityLivingBase) worldObj.getEntityByID(this.dataWatcher.getWatchableObjectInt(ATTACK_TARGET));
    }

    @Override
    public void setCurrentAction(BossActions action) {
        this.currentAction = action;
        if (action == BossActions.LONG_CASTING) wingFlapTime = 0;
    }
    ...
    public float getWingFlapSpeed() {
        switch (this.currentAction) {
            ...
            case STRIKE:
                return 0.4f;
            case CHARGE:
                if (ticksInCurrentAction < 15) return 0.25f;
                return 0.75f;
            default:
                return 0.25f;
        }
    ...
    public boolean isPotionActive(int par1) {
        if (par1 == BuffList.spellReflect.id
            && (currentAction == BossActions.SHIELD_BASH || currentAction == BossActions.LONG_CASTING)) return true;
        if (par1 == BuffList.magicShield.id
            && (currentAction == BossActions.SHIELD_BASH || currentAction == BossActions.LONG_CASTING)) return true;
    ...
    public boolean isPotionActive(Potion par1Potion) {
        if (par1Potion == BuffList.spellReflect
            && (currentAction == BossActions.SHIELD_BASH || currentAction == BossActions.LONG_CASTING)) return true;
        if (par1Potion == BuffList.magicShield
            && (currentAction == BossActions.SHIELD_BASH || currentAction == BossActions.LONG_CASTING)) return true;
