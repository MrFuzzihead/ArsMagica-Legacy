...
public class EntityLightningGuardian extends AM2Boss implements IAnimatedEntity {
    ...
    protected void initSpecificAI() {
        ...
        this.tasks.addTask(
            3,
            new EntityAICastSpell(this, NPCSpells.instance.lightningRune, 22, 27, 200, BossActions.CASTING));
        this.tasks.addTask(
            3,
            new EntityAICastSpell(this, NPCSpells.instance.scrambleSynapses, 45, 60, 300, BossActions.SMASH));
    ...
    public void onDeath(DamageSource src) {
        if (this.getAttackTarget() != null) ExtendedProperties.For(this.getAttackTarget())
    ...
    public void onUpdate() {
        ...
        if (this.getAttackTarget() != null) {
            if (this.getCurrentAction() != BossActions.LONG_CASTING) {
                ExtendedProperties.For(getAttackTarget())
                    .setDisableGravity(false);
            }

            if (!this.worldObj.isRemote && this.getDistanceSqToEntity(getAttackTarget()) > 64D
                && this.getCurrentAction() == BossActions.IDLE) {
                this.getNavigator()
                    .tryMoveToEntityLiving(getAttackTarget(), 0.5f);
            }
        }
                ...
                if (ticksInCurrentAction > 50) {
                    for (int i = 0; i < 2 * AMCore.config.getGFXLevel(); ++i) {
                        AMParticle smoke = (AMParticle) AMCore.proxy.particleManager
                            .spawn(worldObj, "smoke", posX, posY + 4, posZ);
                        if (smoke != null) {
                ...
                if (ticksInCurrentAction > 25 && ticksInCurrentAction < 150) {
                    for (int i = 0; i < 2 * AMCore.config.getGFXLevel(); ++i) {
                        ...
                        if (smoke != null) {
                            ...
                            smoke.setParticleScale(1f);
                            smoke.setRGBColorI(
                                ticksInCurrentAction < 85 ? 0xFFFFFF - 0x111111 * ((ticksInCurrentAction - 25) / 4)
                                    : 0x222222);
                            smoke.setMaxAge(20);
                            smoke.AddParticleController(new ParticleHoldPosition(smoke, 10, 1, false));
    ...
    protected float modifyDamageAmount(DamageSource source, float damageAmt) {
        if (source.isMagicDamage() || source.damageType.equals("magic")) damageAmt *= 2;
        if (source.damageType.equals("drown")) damageAmt *= 4;
        if (source.damageType.equals("DamageAMLightning")) damageAmt *= -1;
        return damageAmt;
    }
    ...
    protected void dropFewItems(boolean par1, int par2) {
        if (par1) this
            .entityDropItem(new ItemStack(ItemsCommonProxy.rune, 1, ItemsCommonProxy.rune.META_INF_ORB_GREEN), 0.0f);
