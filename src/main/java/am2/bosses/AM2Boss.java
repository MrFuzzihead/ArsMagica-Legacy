...
public abstract class AM2Boss extends EntityMob implements IArsMagicaBoss, IEntityMultiPart {
    ...
    public void onUpdate() {
        ...
        if (worldObj.isRemote) {
            playerCanSee = AMCore.proxy.getLocalPlayer()
                .canEntityBeSeen(this);
            this.ignoreFrustumCheck = AMCore.proxy.getLocalPlayer()
                .getDistanceToEntity(this) < 32;
        }
        super.onUpdate();
    }
    ...
    public void addPotionEffect(PotionEffect effect) {
        if (effect.getPotionID() == BuffList.silence.id || effect.getPotionID() == Potion.blindness.id
            ...
            || effect.getEffectName()
                .contains("ink")
            || effect.getPotionID() == BuffList.entangled.id) return;
    ...
    public boolean getCanSpawnHere() {
        if (!SpawnBlacklists.getPermanentBlacklistValue(worldObj, this)) return false;
