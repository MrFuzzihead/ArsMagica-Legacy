...
public class EntityAISpinAttack extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        if (cooldownTicks-- > 0 || ((IArsMagicaBoss) host).getCurrentAction() != BossActions.IDLE
            || !((IArsMagicaBoss) host).isActionValid(BossActions.SPINNING)) return false;
    ...
    public void updateTask() {
        ...
        host.getNavigator()
            .tryMoveToEntityLiving(target, moveSpeed);
        List<EntityLivingBase> nearbyEntities = host.worldObj
            .getEntitiesWithinAABB(EntityLivingBase.class, host.boundingBox.expand(2, 2, 2));
