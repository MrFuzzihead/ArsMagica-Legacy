...
public class EntityNatureGuardian extends AM2Boss {
    ...
    protected void initSpecificAI() {
        this.tasks.addTask(
            ...
            new EntityAICastSpell(
                ...
                50,
                BossActions.CASTING,
                new ISpellCastCallback<EntityNatureGuardian>() {
    ...
    public void onUpdate() {
        if (worldObj.isRemote) {
            ...
            if (rand.nextInt(2) == 0) spawnParticles();
        }
        super.onUpdate();
    }
    ...
    private void spawnParticles() {
        AMParticle leaf = (AMParticle) AMCore.proxy.particleManager.spawn(
            worldObj,
            "leaf",
            posX + (rand.nextDouble() * 5) - 2.5f,
            posY + (rand.nextDouble() * 5f),
            posZ + (rand.nextDouble() * 5) - 2.5f);
        if (leaf != null) {
            leaf.setMaxAge(20);
    ...
    protected void dropFewItems(boolean par1, int par2) {
        if (par1) this.entityDropItem(new ItemStack(ItemsCommonProxy.rune, 1, ItemRune.META_INF_ORB_RED), 0.0f);
