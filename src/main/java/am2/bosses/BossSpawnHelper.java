...
public class BossSpawnHelper {
    ...
    public void onVillagerChildKilled(EntityVillager villager) {
        ...
        if (!chalkCircleIsValid(world, x, y, z)) return;
    ...
    private boolean chalkCircleIsValid(World world, int x, int y, int z) {
        // check for candles
        if (world.getBlock(x - 3, y, z) != BlocksCommonProxy.candle) return false;
        if (world.getBlock(x + 3, y, z) != BlocksCommonProxy.candle) return false;
        if (world.getBlock(x, y, z - 3) != BlocksCommonProxy.candle) return false;
        if (world.getBlock(x, y, z + 3) != BlocksCommonProxy.candle) return false;

        // check for chalk circle
        int xOff = -2;
        int zOff = -2;
        while (xOff <= 2) if (world.getBlock(x + xOff++, y, z + zOff) != BlocksCommonProxy.wizardChalk) return false;
        xOff--;
        while (zOff <= 2) if (world.getBlock(x + xOff, y, z + zOff++) != BlocksCommonProxy.wizardChalk) return false;
        zOff--;
        while (xOff >= -2) if (world.getBlock(x + xOff--, y, z + zOff) != BlocksCommonProxy.wizardChalk) return false;
        xOff++;
        while (zOff >= -2) if (world.getBlock(x + xOff, y, z + zOff--) != BlocksCommonProxy.wizardChalk) return false;
        ...
        for (EntityLivingBase ent : queuedBosses.keySet()) {
            ...
            if (!world.isRemote) {
                ...
                onBossSpawn(
                    ent,
                    world,
                    (int) Math.floor(ent.posX),
                    (int) Math.floor(ent.posY),
                    (int) Math.floor(ent.posZ));
            }
        }
        queuedBosses.clear();
    }
    ...
    public void onItemInRing(EntityItem item, Block ringID) {
        if (ringID == BlocksCommonProxy.redstoneInlay) {
            checkForWaterGuardianSpawn(
                item.worldObj,
                (int) Math.floor(item.posX),
                (int) Math.floor(item.posY),
                (int) Math.floor(item.posZ));
        } else if (ringID == BlocksCommonProxy.ironInlay) {
            checkForArcaneGuardianSpawn(
                item.worldObj,
                (int) Math.floor(item.posX),
                (int) Math.floor(item.posY),
                (int) Math.floor(item.posZ));
            checkForEarthGuardianSpawn(
                item.worldObj,
                (int) Math.floor(item.posX),
                (int) Math.floor(item.posY),
                (int) Math.floor(item.posZ));
        } else if (ringID == BlocksCommonProxy.goldInlay) {
            checkForAirGuardianSpawn(
                item.worldObj,
                (int) Math.floor(item.posX),
                (int) Math.floor(item.posY),
                (int) Math.floor(item.posZ));
            checkForFireGuardianSpawn(
                item,
                item.worldObj,
                (int) Math.floor(item.posX),
                (int) Math.floor(item.posY),
                (int) Math.floor(item.posZ));
            checkForEnderGuardianSpawn(
                item.worldObj,
                (int) Math.floor(item.posX),
                (int) Math.floor(item.posY),
                (int) Math.floor(item.posZ));
    ...
    private void checkForWaterGuardianSpawn(World world, int x, int y, int z) {
        ...
        for (Type type : types) {
            if (type == Type.WATER || type == Type.SWAMP
                || type == Type.BEACH
                || type == Type.OCEAN
                || type == Type.RIVER
                || type == Type.WET) {
        ...
        for (int i = -1; i <= 1; ++i)
            for (int j = -1; j <= 1; ++j) if (!world.canBlockSeeTheSky(x + i, y + 1, z + j)) return;

        List<EntityItem> itemsInRange = world.getEntitiesWithinAABB(
            EntityItem.class,
        ...
        for (EntityItem item : itemsInRange) {
            ...
            if (item.getEntityItem()
                .getItem() == Items.boat) hasBoat = true;
            else if (item.getEntityItem()
                .getItem() == Items.water_bucket) hasBucket = true;
        }
    ...
    private void checkForAirGuardianSpawn(World world, int x, int y, int z) {
        ...
        if (itemsInRange.get(0)
            .getEntityItem()
            .getItem() != ItemsCommonProxy.essence
            || itemsInRange.get(0)
                .getEntityItem()
                .getItemDamage() != ItemsCommonProxy.essence.META_AIR)
            return;
    ...
    private void checkForArcaneGuardianSpawn(World world, int x, int y, int z) {
        ...
        if (itemsInRange.get(0)
            ...
            || itemsInRange.get(0)
                .getEntityItem()
                .getItemDamage() != ItemsCommonProxy.essence.META_ARCANE)
            return;
        boolean hasStructure = false;
        TileEntityLectern lectern = null;
        // +z check
        if (world.getBlock(x - 1, y, z + 2) == Blocks.bookshelf
            && world.getBlock(x - 1, y + 1, z + 2) == Blocks.bookshelf
            && world.getBlock(x - 1, y + 2, z + 2) == Blocks.bookshelf
            && world.getBlock(x + 1, y, z + 2) == Blocks.bookshelf
            && world.getBlock(x + 1, y + 1, z + 2) == Blocks.bookshelf
            && world.getBlock(x + 1, y + 2, z + 2) == Blocks.bookshelf) {
        ...
        // +x check
        if (world.getBlock(x + 2, y, z - 1) == Blocks.bookshelf
            && world.getBlock(x + 2, y + 1, z - 1) == Blocks.bookshelf
            && world.getBlock(x + 2, y + 2, z - 1) == Blocks.bookshelf
            && world.getBlock(x + 2, y, z + 1) == Blocks.bookshelf
            && world.getBlock(x + 2, y + 1, z + 1) == Blocks.bookshelf
            && world.getBlock(x + 2, y + 2, z + 1) == Blocks.bookshelf) {
    ...
    private void checkForEarthGuardianSpawn(World world, int x, int y, int z) {
        ...
        for (EntityItem item : itemsInRange) {
            if (item.isDead) continue;
            if (item.getEntityItem()
                .getItem() == Items.emerald) hasEmerald = true;
            else if (item.getEntityItem()
                .getItem() == ItemsCommonProxy.itemOre
                && item.getEntityItem()
                    .getItemDamage() == ItemsCommonProxy.itemOre.META_BLUETOPAZ)
                hasTopaz = true;
            else if (item.getEntityItem()
                .getItem() == ItemsCommonProxy.itemOre
                && item.getEntityItem()
                    .getItemDamage() == ItemsCommonProxy.itemOre.META_CHIMERITE)
                hasChimerite = true;
        }
    ...
    private void checkForFireGuardianSpawn(EntityItem item, World world, int x, int y, int z) {
        if (item.getEntityItem()
            .getItem() != ItemsCommonProxy.essence
            || item.getEntityItem()
                .getItemDamage() != ItemsCommonProxy.essence.META_WATER)
            return;
        boolean hasStructure = false;
    ...
    public void onIceEffigyBuilt(World world, int x, int y, int z) {
        ...
        else if (!world.canBlockSeeTheSky(x + i, y + 1, z + j)) return;
    ...
    public void checkForEnderGuardianSpawn(World world, int x, int y, int z) {
        ...
        for (EntityItem item : itemsInRange) {
            if (item.isDead) continue;
            if (item.getEntityItem()
                .getItem() == Items.ender_eye) hasEyeofEnder = true;
            else if (item.getEntityItem()
                .getItem() == ItemsCommonProxy.essence
                && item.getEntityItem()
                    .getItemDamage() == ItemsCommonProxy.essence.META_ENDER)
        ...
        if (!hasStructure || !hasEnderEssence || !hasEyeofEnder) return;
