...
public class MeteorSpawnHelper {
    ...
    public void tick() {
        if (ticksSinceLastMeteor == 0) {
            ...
            if (rand.nextInt(
                (AMCore.config.getMoonstoneFrequency() / 19)
                    + (1000 * ws.provider.getMoonPhase(ws.provider.getWorldTime())))
                == 0) {
                spawnMeteor();
            }
        } else {
    ...
    public void spawnMeteor() {
        ...
        if (time > 14500 && time < 21500) { // night time range (just past dusk and just before dawn)
            ...
            while ((ExtendedProperties.For(player)
                .getMagicLevel() < AMCore.config.getMeteorMinSpawnLevel())) {
            ...
            meteor.setPosition(
                spawnCoord.x + rand.nextInt(meteorOffsetRadius) - (meteorOffsetRadius / 2),
                ws.getActualHeight(),
                spawnCoord.z + rand.nextInt(meteorOffsetRadius) - (meteorOffsetRadius / 2));
    ...
    private int correctYCoord(World world, int x, int y, int z) {
        ...
        while (y < world.getActualHeight() && world.canBlockSeeTheSky(x, y, z)) y++;

        while (world.isAirBlock(x, y, z) && y > -1) y--;
