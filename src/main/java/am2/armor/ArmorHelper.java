...
public class ArmorHelper {
    ...
    public static void HandleArmorEffects(EntityPlayer player, World world) {
        ...
        for (int i = 0; i < extendedProperties.armorProcCooldowns.length; ++i) {
            if (extendedProperties.armorProcCooldowns[i] > 0) extendedProperties.armorProcCooldowns[i]--;
        }
    }
    ...
    public static void imbueArmor(ItemStack armorStack, String id, boolean ignoreLevelRequirement) {
        ...
        if (armorStack != null && imbuement != null && armorStack.getItem() instanceof ItemArmor) {
            ...
            if (!ignoreLevelRequirement && getArmorLevel(armorStack) < getImbueCost(imbuement.getTier())) return;

            for (int i : imbuement.getValidSlots()) {
                if (i == ((ItemArmor) armorStack.getItem()).armorType) {
                    if (!armorStack.hasTagCompound()) armorStack.setTagCompound(new NBTTagCompound());
                    NBTTagCompound armorProps = (NBTTagCompound) armorStack.stackTagCompound
                        .getTag(AMArmor.NBT_KEY_AMPROPS);
                    if (armorProps == null) armorProps = new NBTTagCompound();
                    String infusionList = armorProps.getString(AMArmor.NBT_KEY_EFFECTS);
                    if (infusionList == null || infusionList == "") infusionList = id;
                    else infusionList += "|" + id;
    ...
    public static void addXPToArmor(float totalAmt, EntityPlayer player) {
        int numPieces = 0;
        for (int i = 0; i < 4; ++i) if (player.getCurrentArmor(i) != null) numPieces++;
    ...
    public static void deductXPFromArmor(float amt, ItemStack armor) {
        if (armor != null && armor.getItem() instanceof ItemArmor) {
            if (!armor.hasTagCompound()) armor.setTagCompound(new NBTTagCompound());
            NBTTagCompound armorProps = (NBTTagCompound) armor.stackTagCompound.getTag(AMArmor.NBT_KEY_AMPROPS);
            if (armorProps == null) armorProps = new NBTTagCompound();
            armorProps
                .setDouble(AMArmor.NBT_KEY_TOTALXP, Math.max(armorProps.getDouble(AMArmor.NBT_KEY_TOTALXP) - amt, 0));
            armorProps.setInteger(
                AMArmor.NBT_KEY_ARMORLEVEL,
                EntityUtilities.getLevelFromXP((float) armorProps.getDouble(AMArmor.NBT_KEY_TOTALXP)));
