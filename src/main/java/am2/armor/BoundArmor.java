    ...
    public void onArmorTick(World world, EntityPlayer player, ItemStack itemStack) {
        if (player.ticksExisted % 100 == 0) {
            ...
            if (getSpellStack(itemStack) != null && p.getContingencyEffect(
                ...
                p.setContingency(
                    itemStack.getUnlocalizedName()
                        .contains("helm") ? ContingencyTypes.DEATH
                            : itemStack.getUnlocalizedName()
                                .contains("chest") ? ContingencyTypes.HEALTH_LOW
                                    : itemStack.getUnlocalizedName()
                                        .contains("leg")
                                            ? ContingencyTypes.DAMAGE_TAKEN
                                            : itemStack.getUnlocalizedName()
                                                .contains("boot") ? ContingencyTypes.FALL : ContingencyTypes.NONE,
                    getSpellStack(itemStack));
            }
        }
    }
