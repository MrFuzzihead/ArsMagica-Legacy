...
public class ArmorEventHandler {
    ...
    public void onEntityLiving(LivingUpdateEvent event) {
        ...
        if (player.inventory.armorInventory[0] != null
            ...
            float speedInAirLast = ReflectionHelper
                .getPrivateValue(EntityPlayer.class, player, "speedInAir", "field_71102_ce");
    ...
    public void onEntityDeath(LivingDeathEvent event) {
        if (event.source.getSourceOfDamage() instanceof EntityPlayer) doXPInfusion(
            (EntityPlayer) event.source.getSourceOfDamage(),
            1,
            Math.min(20, event.entityLiving.getMaxHealth()));
    ...
    private void doXPInfusion(EntityPlayer player, float xpMin, float xpMax) {
        float amt = (float) ((player.worldObj.rand.nextFloat() * xpMin + (xpMax - xpMin))
            * AMCore.config.getArmorXPInfusionFactor());
    ...
    public void onPlayerRenderPre(RenderPlayerEvent.Pre event) { // true invis
        ...
        if (MysteriumPatchesFixesMagicka.playerModelMap.get(event.entityPlayer.getCommandSenderName()) != null
            && !MysteriumPatchesFixesMagicka.playerModelMap.get(event.entityPlayer.getCommandSenderName())
                .startsWith("maid")) {
            // custom mobs
            ...
            if (rle instanceof RendererLivingEntity) {
                ...
                ((RendererLivingEntity) rle).doRender(
                    elb,
                    d0 - RenderManager.renderPosX,
                    d1 - RenderManager.renderPosY - event.entityPlayer.yOffset,
    ...
    public void onHandRender(RenderHandEvent event) {
        if (MysteriumPatchesFixesMagicka.isPlayerEthereal(Minecraft.getMinecraft().thePlayer)) event.setCanceled(true);
    }
    ...
    public void onBlockHighlight(DrawBlockHighlightEvent event) {
        if (MysteriumPatchesFixesMagicka.isPlayerEthereal(event.player)) {
            ...
            int meta = Minecraft.getMinecraft().theWorld
                .getBlockMetadata(event.target.blockX, event.target.blockY, event.target.blockZ);
            if (!block.hasTileEntity(meta) || !(Minecraft.getMinecraft().theWorld
                .getTileEntity(event.target.blockX, event.target.blockY, event.target.blockZ) instanceof IInventory)) {
                event.setCanceled(true);
            }
        }
    }
    ...
    public void onInteract(PlayerInteractEvent event) {
        if (MysteriumPatchesFixesMagicka.isPlayerEthereal(event.entityPlayer)) {
            if (event.action != PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK) event.setCanceled(true);
            else {
                ...
                if (!block.hasTileEntity(meta)
                    || !(event.world.getTileEntity(event.x, event.y, event.z) instanceof IInventory)) {
                    event.setCanceled(true);
                }
            }
        }
    ...
    public void breakSpeed(PlayerEvent.BreakSpeed event) {
        if (MysteriumPatchesFixesMagicka.isPlayerEthereal(event.entityPlayer)) event.newSpeed = 0;
    }
    ...
    public void onPlayerRender(RenderPlayerEvent.Post event) {
        if (event.entityPlayer == Minecraft.getMinecraft().thePlayer) { // these additions only render clientside/SP
            if (event.entityPlayer.getCurrentArmor(3) != null) {
                if (event.entityPlayer.getCurrentArmor(3)
                    .getItem() == ItemsCommonProxy.archmageHood) {
                    ...
                    float f5 = 0.0625F;
                    float f2 = 0; // interpolateRotation(p_76986_1_.prevRenderYawOffset, p_76986_1_.renderYawOffset,
                                  // p_76986_9_);
                    ...
                    Minecraft.getMinecraft()
                        ...
                        .bindTexture(
                            new ResourceLocation(
                                "arsmagica2:" + ResourceManager.getMobTexturePath("bosses/fire_guardian.png")));
            ...
            if (event.entityPlayer.getCurrentArmor(2) != null) {
                if (event.entityPlayer.getCurrentArmor(2)
                    .getItem() == ItemsCommonProxy.archmageArmor) {
                    EntityPlayer p_76986_1_ = event.entityPlayer;
                    int i1 = Minecraft.getMinecraft().gameSettings.limitFramerate;
                    float p_76986_9_ = System.nanoTime() + (long) (1000000000 / i1);
                    float f5 = 0.0625F;
                    ...
                    float f4 = 0;
                    float f13 = 0;
                    float f6 = 0; // p_76986_1_.prevLimbSwingAmount + (p_76986_1_.limbSwingAmount -
                                  // p_76986_1_.prevLimbSwingAmount) * p_76986_9_;
                    ...
                    Minecraft.getMinecraft()
                        ...
                        .bindTexture(
                            new ResourceLocation(
                                "arsmagica2:" + ResourceManager.getMobTexturePath("bosses/earth_guardian.png")));
            ...
            if (event.entityPlayer.getCurrentArmor(1) != null) {
                if (event.entityPlayer.getCurrentArmor(1)
                    .getItem() == ItemsCommonProxy.archmageLeggings) {
                    EntityPlayer p_76986_1_ = event.entityPlayer;
                    int i1 = Minecraft.getMinecraft().gameSettings.limitFramerate;
                    float p_76986_9_ = System.nanoTime() + (long) (1000000000 / i1);
                    float f5 = 0.0625F;
                    ...
                    Minecraft.getMinecraft()
                        ...
                        .bindTexture(
                            new ResourceLocation(
                                "arsmagica2:" + ResourceManager.getMobTexturePath("bosses/water_guardian.png")));
                    ModelLibrary.instance.earthArmor.saveValues = true;
