...
public class RecipeHandlerEnervator extends TemplateRecipeHandler {
    ...
    public void loadCraftingRecipes(ItemStack result) {
        for (Object key : getRecipes().keySet()) {
            ...
            if (getRecipes().get(key) != null
                && NEIServerUtils.areStacksSameType((ItemStack) getRecipes().get(key), result)
                || ((ItemStack) getRecipes().get(key)).stackTagCompound == null
                    && NEIServerUtils.areStacksSameTypeCrafting(((ItemStack) getRecipes().get(key)), result)
                    && ((ItemStack) getRecipes().get(key)).getItem() != Items.skull)
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        for (Object key : getRecipes().keySet()) {
            ...
            RecipeHandlerEnervator.CachedEvervatorRecipe crecipe = getCachedRecipe(
                (ItemStack) key,
                ((ItemStack) getRecipes().get(key)));
