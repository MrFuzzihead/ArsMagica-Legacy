...
public class InventoryUtilities {
    ...
    public static int decrementStackQuantity(IInventory inventory, int slotIndex, int quantity) {
        ...
        if (stack != null) {
            if (stack.stackSize < quantity) quantity = stack.stackSize;
    ...
    public static boolean mergeIntoInventory(IInventory inventory, ItemStack toMerge, int quantity) {
        ...
        if (quantity > toMerge.stackSize) quantity = toMerge.stackSize;

        int qty = quantity;
        int emptySlot = -1;
        for (int i = 0; i < inventory.getSizeInventory(); ++i) {
            if (!inventory.isItemValidForSlot(i, toMerge)) continue;
            ...
            if (compareItemStacks(inventoryStack, toMerge, true, false, true, true)) {
                if (inventoryStack.stackSize == inventoryStack.getMaxStackSize()) continue;
        ...
        if (inventory instanceof ISidedInventory) {
            ...
            for (int i = 0; i < slots.length && stack != null && stack.stackSize > 0; ++i) {
                ...
                if (InventoryUtilities.canInsertItemToInventory(sidedInventory, stack, slots[i], side)) {
                    ...
                    if (prvStack == null) {
                        // if the stack in the slot is null then get the max value that can be moved and transfer the
                        // stack to the inventory
                        ...
                        // if the stack in the slot can be merged with the stack we are trying to move get the max items
                        // that can exist in the slot
    ...
    public static boolean inventoryHasItem(IInventory inventory, ItemStack search, int quantity) {
        ...
        for (int i = 0; i < inventory.getSizeInventory(); ++i) {
            ...
            if (compareItemStacks(inventoryStack, search, true, false, true, true)) {
                ...
                if (qtyFound >= quantity) return true;
            }
        }
        return false;
    }
    ...
    public static int getFirstBlockInInventory(IInventory inventory) {
        for (int i = 0; i < inventory.getSizeInventory(); ++i) {
            ItemStack inventoryStack = inventory.getStackInSlot(i);
            if (inventoryStack == null) continue;
            if (inventoryStack.getItem() instanceof ItemBlock) return i;
        }
        return -1;
    }
    ...
    public static boolean inventoryHasRoomFor(IInventory inventory, ItemStack stack, int qty) {
        for (int i = 0; i < inventory.getSizeInventory(); ++i) {
            ...
            if (compareItemStacks(invStack, stack, true, false, true, true)
                && invStack.getMaxStackSize() - invStack.stackSize >= qty) return true;
        }
        return false;
    }
    ...
    public static int getInventorySlotIndexFor(IInventory inventory, Item item, int metadata) {
        for (int i = 0; i < inventory.getSizeInventory(); ++i) {
            ...
            if (stack != null && stack.getItem() == item
                && (stack.getItemDamage() == metadata || metadata == Short.MAX_VALUE)) return i;
        }
        return -1;
    }
    ...
    public static int getInventorySlotIndexFor(IInventory inventory, ItemStack search) {
        for (int i = 0; i < inventory.getSizeInventory(); ++i) {
            ItemStack stack = inventory.getStackInSlot(i);
            if (compareItemStacks(stack, search, false, false, true, true)) return i;
        }
        return -1;
    }
    ...
    public static int getLikeItemCount(IInventory inventory, ItemStack stack) {
        ...
        for (int i = 0; i < inventory.getSizeInventory(); ++i) {
            ItemStack invStack = inventory.getStackInSlot(i);
            if (compareItemStacks(invStack, stack, true, false, true, true)) totalCount += invStack.stackSize;
        }
    ...
    public static GetFirstStackStartingFromSlotResult getFirstStackStartingFromSlot(IInventory inventory,
        ItemStack itemStack, int slot) {
    ...
    public static GetFirstStackStartingFromSlotResult getFirstStackStartingFromSlot(IInventory inventory,
        ItemStack itemStack, int slot, int side) {
        if (inventory instanceof ISidedInventory) {
    ...
    public static TileEntityChest getAdjacentChest(TileEntityChest chest) {
        ...
        else if (chest.adjacentChestXPos != null) adjacent = chest.adjacentChestXPos;
        else if (chest.adjacentChestZPos != null) adjacent = chest.adjacentChestZPos;
        else if (chest.adjacentChestZNeg != null) adjacent = chest.adjacentChestZNeg;
    ...
    public static ItemStack replaceItem(ItemStack originalStack, Item newItem) {
        ...
        if (originalStack.hasTagCompound()) stack.setTagCompound(originalStack.getTagCompound());
        return stack;
    }

    public static boolean compareItemStacks(ItemStack a, ItemStack b, boolean matchMeta, boolean matchStackSize,
        boolean matchNBT, boolean allowAnyMeta) {
        if (a == null || b == null) return false;

        if (a.getItem() != b.getItem()) return false;

        if (allowAnyMeta && !(a.getItemDamage() == b.getItemDamage() || a.getItemDamage() == AMCore.ANY_META
            || b.getItemDamage() == AMCore.ANY_META)) {
            return false;
        } else if (matchMeta && a.getItemDamage() != b.getItemDamage()) {
            return false;
        }

        if (matchStackSize && a.stackSize != b.stackSize) return false;
