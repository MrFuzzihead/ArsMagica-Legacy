...
public class EntityUtilities {
    ...
    public static void makeSummon_PlayerFaction(EntityCreature entityliving, EntityPlayer player,
        boolean storeForRevert) {
        if (isAIEnabled(entityliving) && !(entityliving instanceof IBossDisplayData)
            ...
            entityliving.targetTasks.addTask(
                2,
                new EntityAINearestAttackableTarget(
                    ...
                    true,
                    false,
                    SummonEntitySelector.instance));
            entityliving.targetTasks
                .addTask(2, new EntityAINearestAttackableTarget(entityliving, EntitySlime.class, 0, true));
            entityliving.targetTasks
                .addTask(2, new EntityAINearestAttackableTarget(entityliving, EntityGhast.class, 0, true));

            if (!entityliving.worldObj.isRemote && entityliving.getAttackTarget() != null
                && entityliving.getAttackTarget() instanceof EntityPlayer)
            ...
            entityliving.getEntityData()
                .setBoolean(isSummonKey, true);
            ExtendedProperties.For(player)
                .addSummon(entityliving);
        }
    }
    ...
    public static void makeSummon_MonsterFaction(EntityCreature entityliving, boolean storeForRevert) {
        if (isAIEnabled(entityliving) && !(entityliving instanceof IBossDisplayData)) {
            ...
            entityliving.targetTasks.addTask(1, new EntityAIHurtByTarget(entityliving, true));
            entityliving.targetTasks
                .addTask(2, new EntityAINearestAttackableTarget(entityliving, EntityPlayer.class, 0, true));
            if (!entityliving.worldObj.isRemote && entityliving.getAttackTarget() != null
                && entityliving.getAttackTarget() instanceof EntityMob)
                AMCore.proxy.addDeferredTargetSet(entityliving, null);
    ...
    public static void revertAI(EntityCreature entityliving) {
        ...
        if (owner instanceof EntityLivingBase) {
            ExtendedProperties.For((EntityLivingBase) owner)
                .removeSummon();
            if (ExtendedProperties.For((EntityLivingBase) owner)
                .isManaLinkedTo(entityliving)) {
                ExtendedProperties.For((EntityLivingBase) owner)
                    .updateManaLink(entityliving);
                ExtendedProperties.For((EntityLivingBase) owner)
                    .forceSync();
            }
        }
        ...
        if (storedTasks.containsKey(entityliving.getEntityId())) {
            ...
            if (storedAITasks.get(entityliving.getEntityId()) != null) {
                ...
                for (Object task : entityliving.tasks.taskEntries) {
                    EntityAITaskEntry base = (EntityAITaskEntry) task;
                    if (base.action instanceof EntityAIAttackOnCollide
                        || base.action instanceof EntityAISummonFollowOwner) {
                        toRemove.add(base);
                    }
                }
    ...
    public static boolean isSummon(EntityLivingBase entityliving) {
        return entityliving.getEntityData()
            .getBoolean(isSummonKey);
    }

    public static void setOwner(EntityLivingBase entityliving, EntityLivingBase owner) {
        if (owner == null) {
            entityliving.getEntityData()
                .removeTag(summonOwnerKey);
            return;
        }

        entityliving.getEntityData()
            .setInteger(summonOwnerKey, owner.getEntityId());
        if (entityliving instanceof EntityCreature) {
            float speed = entityliving.getAIMoveSpeed();
            if (speed <= 0) speed = 1.0f;
            ((EntityCreature) entityliving).tasks
                .addTask(1, new EntityAISummonFollowOwner((EntityCreature) entityliving, speed, 10, 20));
        }
    }

    public static void setTileSpawned(EntityLivingBase entityliving, TileEntitySummoner summoner) {
        entityliving.getEntityData()
            .setInteger(summonTileXKey, summoner.xCoord);
        entityliving.getEntityData()
            .setInteger(summonTileYKey, summoner.yCoord);
        entityliving.getEntityData()
            .setInteger(summonTileZKey, summoner.zCoord);
    }

    public static boolean isTileSpawnedAndValid(EntityLivingBase entityliving) {
        int tileX = entityliving.getEntityData()
            .getInteger(summonTileXKey);
        int tileY = entityliving.getEntityData()
            .getInteger(summonTileYKey);
        int tileZ = entityliving.getEntityData()
            .getInteger(summonTileZKey);
    ...
    public static void setSummonDuration(EntityLivingBase entity, int duration) {
        entity.getEntityData()
            .setInteger(summonDurationKey, duration);
    }
    ...
    public static boolean decrementSummonDuration(EntityLivingBase ent) {
        if (!isTileSpawnedAndValid(ent)) {
            int duration = ent.getEntityData()
                .getInteger(summonDurationKey);
    ...
    public static int getRuneCombo(EntityPlayer player) {
        int hash = player.getCommandSenderName()
            .toLowerCase()
            .hashCode();
        return hash & 0xFFFF;
    }
    ...
    public static EntityPlayer getPlayerForCombo(World world, int combo) {
        ...
        for (WorldServer ws : MinecraftServer.getServer().worldServers) {
            for (Object o : ws.playerEntities) {
                ...
                if (getRuneCombo(player) == combo) return player;
            }
        }
        return null;
    }
    ...
    public static float modifySoundPitch(Entity e, float p) {
        if (e instanceof EntityLivingBase && ((EntityLivingBase) e).isPotionActive(BuffList.shrink)) {
            return p + ExtendedProperties.For((EntityLivingBase) e)
                .getShrinkPct();
        }
        return p;
    }
