...
public class EntityThrownRock extends EntityLiving {
    ...
    public void setHeading(double movementX, double movementY, double movementZ, double projectileSpeed,
    ...
    public void onUpdate() {
        ...
        if (getIsShootingStar()) {
            motionY -= 0.1f;
            if (motionY < -2f) motionY = -2f;
        }

        if (worldObj.isRemote) {
            if (getIsMoonstoneMeteor()) {
                AMParticle fire = (AMParticle) AMCore.proxy.particleManager
                    .spawn(worldObj, "explosion_2", posX, posY, posZ);
                if (fire != null) {
                    fire.setMaxAge(20);
                    fire.setRGBColorF(1, 1, 1);
                    fire.setParticleScale(2.0f);
                    fire.AddParticleController(new ParticleHoldPosition(fire, 20, 1, false));
                    fire.AddParticleController(
                        new ParticleColorShift(fire, 1, false).SetShiftSpeed(0.1f)
                        ...
                        for (ISpellModifier mod : mods) {
                            if (mod instanceof Colour) {
                                byte[] meta = SpellUtils.instance
                                    .getModifierMetadataFromStack(getSpellStack(), mod, 0, ordinalCount++);
                ...
                for (float i = 0; i < Math.abs(motionY); i += 0.1f) {
                    AMParticle star = (AMParticle) AMCore.proxy.particleManager
                        .spawn(worldObj, "ember", posX + motionX * i, posY + motionY * i, posZ + motionZ * i);
                    if (star != null) {
                        star.setMaxAge(22);
    ...
    protected void HitObject(MovingObjectPosition movingobjectposition) {
        ...
        if (getIsShootingStar()) {
            AMNetHandler.INSTANCE.sendStarImpactToClients(
                posX,
                posY + ((movingobjectposition.typeOfHit == MovingObjectType.ENTITY)
                    ? -movingobjectposition.entityHit.getEyeHeight()
                    : 1.5f),
                posZ,
                worldObj,
                this.getSpellStack());
            List<EntityLivingBase> ents = worldObj
                .getEntitiesWithinAABB(EntityLivingBase.class, boundingBox.expand(5, 5, 5));
            for (EntityLivingBase e : ents) {
                if (e == throwingEntity) continue;
                if (this.getEntitySenses()
                    .canSee(e))
                    SpellHelper.instance
                        .attackTargetSpecial(null, e, DamageSources.causeEntityMagicDamage(throwingEntity), damage);
            }
        } else {
            ...
            if (movingobjectposition.entityHit instanceof EntityLivingBase) {
                ...
                if (this.getIsMoonstoneMeteor()) {
                    ...
                    this.worldObj.newExplosion(
                        ...
                        0.8f,
                        false,
                        AMCore.config.moonstoneMeteorsDestroyTerrain());

                    int numOres = rand.nextInt(6) + 1;

                    for (int i = 0; i < numOres; ++i) {
                        generateSurfaceOreAtOffset(
                            worldObj,
                            (int) Math.floor(this.target.x),
                            (int) Math.floor(this.target.y),
                            (int) Math.floor(this.target.z),
                            i == 0);
                    }
    ...
    private void generateSurfaceOreAtOffset(World world, int x, int y, int z, boolean force) {
        ...
        while (!world.isAirBlock(x, y, z) && y < world.getActualHeight()) y++;

        if (rand.nextInt(4) < 2 || force)
            world.setBlock(x, y, z, BlocksCommonProxy.AMOres, BlockAMOre.META_MOONSTONE_ORE, 2);
        else world.setBlock(x, y, z, BlocksCommonProxy.cosmicRock);
    }
