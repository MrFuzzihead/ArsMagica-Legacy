...
public class EntityManaVortex extends Entity {
    ...
    public void onUpdate() {
        ...
        if (getTicksToExist() - this.ticksExisted <= 5 && !hasGoneBoom) {
            ...
            if (!worldObj.isRemote) {
                List players = worldObj.getEntitiesWithinAABB(
                    ...
                    this.boundingBox
                        .expand(3 + Math.floor(this.ticksExisted / 50), 2, 3 + Math.floor(this.ticksExisted / 50)));
                ...
                for (Object o : playerArray) {
                    ...
                    MovingObjectPosition mop = this.worldObj.rayTraceBlocks(
                        ...
                        Vec3.createVectorHelper(e.posX, e.posY + e.getEyeHeight(), e.posZ),
                        false);
                    if (mop == null) e.attackEntityFrom(DamageSources.causeEntityPhysicalDamage(this), damage);
                }
            } else {
                for (int i = 0; i < 360; i += AMCore.config.FullGFX() ? 5 : AMCore.config.LowGFX() ? 10 : 20) {
                    AMParticle effect = (AMParticle) AMCore.instance.proxy.particleManager
                        .spawn(worldObj, "ember", this.posX, this.posY, this.posZ);
                    if (effect != null) {
        ...
        if (getTicksToExist() - this.ticksExisted > 30) {
            ...
            List<EntityLivingBase> players = worldObj.getEntitiesWithinAABB(
                EntityLivingBase.class,
                this.boundingBox
                    .expand(3 + Math.floor(this.ticksExisted / 50), 2, 3 + Math.floor(this.ticksExisted / 50)));
            Object[] playerArray = players.toArray();
            ...
            for (Object o : playerArray) {
                ...
                if (worldObj.isRemote) {
                    ...
                    AMParticle effect = (AMParticle) AMCore.instance.proxy.particleManager
                        .spawn(worldObj, "ember", e.posX, e.posY + (e.getEyeHeight() / 2), e.posZ);
                    if (effect != null) {
                        effect.AddParticleController(
                            new ParticleArcToEntity(effect, 1, this, false).generateControlPoints()
                                .setKillParticleOnFinish(true));
                        effect.setRGBColorF(0.24f, 0.24f, 0.8f);
                        effect.setIgnoreMaxAge(true);
                    }
                }
                float manaStolen = ExtendedProperties.For(e)
                    .getMaxMana() * 0.01f;
                float curMana = ExtendedProperties.For(e)
                    .getCurrentMana();

                if (manaStolen > curMana) manaStolen = curMana;

                this.dataWatcher.updateObject(
                    DW_MANA_STOLEN,
                    this.dataWatcher.getWatchableObjectFloat(DW_MANA_STOLEN) + manaStolen);
                ExtendedProperties.For(e)
                    .setCurrentMana(
                        ExtendedProperties.For(e)
                            .getCurrentMana() - manaStolen);
