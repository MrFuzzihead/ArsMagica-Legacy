...
public class EntityWinterGuardianArm extends EntityLiving {
    ...
    public void setDead() {
        if (getThrowingEntity() != null) {
            if (getThrowingEntity() instanceof EntityWinterGuardian) {
                ...
                if (!((EntityPlayer) getThrowingEntity()).inventory
                    .addItemStackToInventory(ItemsCommonProxy.winterArmEnchanted.copy())) {
        ...
        if (isInWater()) {
            ...
            for (int i = 0; i < 2; ++i) {
                AMParticle particle = (AMParticle) AMCore.proxy.particleManager.spawn(
                    worldObj,
                    "ember",
                    posX + rand.nextFloat() * 0.2 - 0.1,
                    posY + 1.2,
                    posZ + rand.nextFloat() * 0.2 - 0.1);
                if (particle != null) {
                    ...
                    particle.AddParticleController(
                        new ParticleMoveOnHeading(
                            particle,
                            Math.toDegrees(this.rotationPitch),
                            Math.toDegrees(this.rotationYaw),
                            0.2f,
                            1,
                            false));
                }
            ...
            if (getThrowingEntity() != null && this.entityHit == null) {
                ...
                if (movingobjectposition.entityHit instanceof EntityLivingBase) {
                    ...
                    ((EntityLivingBase) movingobjectposition.entityHit)
                        .addPotionEffect(new PotionEffect(Potion.weakness.id, 60, 2));
                    ((EntityLivingBase) movingobjectposition.entityHit)
                        .addPotionEffect(new PotionEffect(Potion.digSlowdown.id, 60, 2));
                    ((EntityLivingBase) movingobjectposition.entityHit)
                        .addPotionEffect(new BuffEffectFrostSlowed(60, 2));
                }
            }
        }
    }
    ...
    private EntityLivingBase getThrowingEntity() {
        if (throwingEntity == null) {
            throwingEntity = (EntityLivingBase) this.worldObj
                .getEntityByID(this.dataWatcher.getWatchableObjectInt(DW_THROWING_ENTITY));
        }
