...
public class EntityLightMage extends EntityCreature {
    ...
    public ItemStack getHeldItem() {
        ...
        if (cm == 0) return diminishedHeldItem;
        else if (cm == 1) return this.normalHeldItem;
        else return this.augmentedHeldItem;
    }
    ...
    private void initAI() {
        ...
        this.tasks
            .addTask(1, new EntityAIAvoidEntity(this, EntityManaVortex.class, 10, MovementSpeed(), ActionSpeed()));

        this.tasks.addTask(3, new EntityAIAllyManaLink(this));

        this.tasks.addTask(
            4,
            new EntityAIRangedAttackSpell(this, MovementSpeed(), 20, NPCSpells.instance.lightMage_DiminishedAttack));
        ...
        this.targetTasks.addTask(
            ...
            new EntityAINearestAttackableTarget(
                ...
                true,
                false,
                LightMageEntitySelector.instance));
    }
    ...
    private int getAverageNearbyPlayerMagicLevel() {
        ...
        List<EntityPlayer> players = worldObj
            .getEntitiesWithinAABB(EntityPlayer.class, this.boundingBox.expand(250, 250, 250));
        if (players.size() == 0) return 0;
        int avgLvl = 0;
        for (EntityPlayer player : players) {
            avgLvl += ExtendedProperties.For(player)
                .getMagicLevel();
        }
    ...
    public boolean getCanSpawnHere() {
        if (!SpawnBlacklists.entityCanSpawnHere(this.posX, this.posZ, worldObj, this)) return false;
        ...
        if (avgLevel == 0) {
            ExtendedProperties.For(this)
                .setMagicLevelWithMana(10);
            if (rand.nextInt(100) < 10) {
                this.tasks.addTask(
                    3,
                    new EntityAIRangedAttackSpell(
                        this,
                        MovementSpeed(),
                        40,
                        NPCSpells.instance.lightMage_NormalAttack));
                this.dataWatcher.updateObject(DW_MAGE_BOOK, 1);
            }
        } else {
            ExtendedProperties.For(this)
                .setMagicLevelWithMana(10 + rand.nextInt(avgLevel));
            ...
            if (levelRand > 60) {
                this.tasks.addTask(
                    ...
                    new EntityAIRangedAttackSpell(
                        ...
                        MovementSpeed(),
                        100,
                        NPCSpells.instance.lightMage_AugmentedAttack));
        ...
        if (!worldObj.isRemote) ExtendedProperties.For(this)
            .setFullSync();
        hasUpdated = true;
    ...
    protected void dropFewItems(boolean par1, int par2) {
        if (par1 && getRNG().nextDouble() < 0.2) for (int j = 0; j < getRNG().nextInt(3); ++j)
    ...
    protected boolean interact(EntityPlayer player) {
        ...
        if (player.getHeldItem() != null && player.getHeldItem()
            .getItem() instanceof ItemNameTag) return false;
            ...
            if (EntityUtilities.isSummon(this)) {
                player.addChatMessage(
                    new ChatComponentText(
                        String.format("\247o%s", StatCollector.translateToLocal("am2.npc.partyleave"))));
                EntityUtilities.revertAI(this);
            } else {
                if (ExtendedProperties.For(player)
                    .getCanHaveMoreSummons()) {
                    if (ExtendedProperties.For(player)
                        .getMagicLevel() - 5 >= ExtendedProperties.For(this)
                            .getMagicLevel()) {
                        player.addChatMessage(
                            new ChatComponentText(
                                String.format("\247o%s", StatCollector.translateToLocal("am2.npc.partyjoin"))));
                        ...
                        player.addChatMessage(
                            new ChatComponentText(
                                String.format("\247o%s", StatCollector.translateToLocal("am2.npc.partyrefuse"))));
                    }
                } else {
                    player.addChatMessage(
                        new ChatComponentText(
                            String.format("\247o%s", StatCollector.translateToLocal("am2.npc.partyfull"))));
                }
            }
        } else {
            player.addChatMessage(
                new ChatComponentText(
                    String.format("\247o%s", StatCollector.translateToLocal("am2.npc.nopartyskill"))));
        }
        return false;
    }
    ...
    public String getTexture() {
        return ResourceManager.getMobTexturePath(
            String.format("light_mages/light_mage_%d.png", this.dataWatcher.getWatchableObjectInt(DW_MAGE_SKIN)));
    }
}
