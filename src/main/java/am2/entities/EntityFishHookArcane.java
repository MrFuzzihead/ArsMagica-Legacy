...
public class EntityFishHookArcane extends EntityFishHook {
    ...
    public static final List ender_fishing = Arrays.asList(
        new WeightedRandomFishable[] { new WeightedRandomFishable(new ItemStack(Items.ender_eye), 10),
            new WeightedRandomFishable(new ItemStack(Items.ender_pearl), 10),
        ...
        if (this.field_146055_aB > 0) {
            ...
            if (!this.worldObj.isRemote) {
                ...
                if (this.field_146042_b.isDead || !this.field_146042_b.isEntityAlive()
                    || itemstack == null
                    || itemstack.getItem() != ItemsCommonProxy.itemArcaneFishingRod
    ...
    private ItemStack getItemToGive() {
        ...
        long time = DimensionManager.getWorld(0)
            .getWorldTime() % 24000; // even if we fish in end, we look at overworld's time
        if (time < 12500 || time > 23500) succeed = false; // night
        int phase = BossSpawnHelper.instance
            .getMoonPhaseProxiedProperly(DimensionManager.getWorld(0).provider.getWorldTime());
