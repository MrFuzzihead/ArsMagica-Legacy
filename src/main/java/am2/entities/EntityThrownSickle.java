...
public class EntityThrownSickle extends EntityLiving {
    ...
    public void setDead() {
        if (getThrowingEntity() != null && getThrowingEntity() instanceof EntityNatureGuardian) {
            ...
            if (!worldObj.isRemote) if (!((EntityPlayer) getThrowingEntity()).inventory
                .addItemStackToInventory(ItemsCommonProxy.natureScytheEnchanted.copy())) {
            ...
            if (getThrowingEntity() != null
                ...
                if (getThrowingEntity() instanceof EntityPlayer) {
                    if (movingobjectposition.entityHit instanceof EntityPlayer
                        && (getThrowingEntity().worldObj.isRemote || !MinecraftServer.getServer()
                            .isPVPEnabled()))
                        return;
                    movingobjectposition.entityHit
                        .attackEntityFrom(DamageSources.causeEntityCactusDamage(getThrowingEntity(), true), 10);
                } else {
                    movingobjectposition.entityHit
                        .attackEntityFrom(DamageSources.causeEntityCactusDamage(getThrowingEntity(), true), 12);
                }
            }
        } else if (movingobjectposition.typeOfHit == MovingObjectType.BLOCK && this.getThrowingEntity() != null
            && this.getThrowingEntity() instanceof EntityPlayer) {
                int radius = 1;
                for (int i = -radius; i <= radius; ++i) {
                    for (int j = -radius; j <= radius; ++j) {
                        for (int k = -radius; k <= radius; ++k) {
                            Block nextBlock = worldObj.getBlock(
                                movingobjectposition.blockX + i,
                                movingobjectposition.blockY + j,
                                movingobjectposition.blockZ + k);
                            if (nextBlock == null) continue;
                            if (nextBlock instanceof BlockLeaves || nextBlock instanceof BlockFlower
                                || nextBlock instanceof BlockCrops) {
                                if (ForgeEventFactory.doPlayerHarvestCheck(
                                    DummyEntityPlayer.fromEntityLiving(getThrowingEntity()),
                                    nextBlock,
                                    true))
                                    if (!worldObj.isRemote) worldObj.func_147478_e(
                                        movingobjectposition.blockX + i,
                                        movingobjectposition.blockY + j,
                                        movingobjectposition.blockZ + k,
                                        true);
    ...
    private EntityLivingBase getThrowingEntity() {
        if (throwingEntity == null) {
            ...
            if (e instanceof EntityLivingBase) throwingEntity = (EntityLivingBase) e;
        }
