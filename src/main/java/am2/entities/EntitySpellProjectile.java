...
public class EntitySpellProjectile extends Entity {
    ...
    public EntitySpellProjectile(World world, EntityLivingBase entityLiving, double projectileSpeed, double x, double y,
        double z) {
        super(world);
        this.noClip = true;
        setSize(0.25F, 0.25F);
    ...
    public void setSpellProjectileHeading(double movementX, double movementY, double movementZ, double projectileSpeed,
    ...
    public void setTargetWater() {
        if (!this.worldObj.isRemote) this.dataWatcher.updateObject(DW_TARGETGRASS, (byte) 1);
    }
    ...
    public void moveTowards(Entity entity, float maxYaw, float maxPitch) {
        ...
        if (entity instanceof EntityLivingBase) {
            ...
            deltaY = entitylivingbase.posY + (double) entitylivingbase.getEyeHeight()
                - (this.posY + (double) this.getEyeHeight());
        } else {
            deltaY = (entity.boundingBox.minY + entity.boundingBox.maxY) / 2.0D
                - (this.posY + (double) this.getEyeHeight());
        }
    ...
    public void onUpdate() {
        ...
        if (entity != null) {
            if (entity instanceof EntityDragonPart && ((EntityDragonPart) entity).entityDragonObj != null
                && ((EntityDragonPart) entity).entityDragonObj instanceof EntityLivingBase) {
        ...
        if (movingobjectposition != null) {
            ...
            if (movingobjectposition.typeOfHit == MovingObjectType.ENTITY && movingobjectposition.entityHit != null
                ...
                if (ent.isPotionActive(BuffList.spellReflect.id) && !pierce) {
                    ...
                    if (!worldObj.isRemote) ent.removePotionEffect(BuffList.spellReflect.id);

                    setBounces(originalBounceCount);

                    if (worldObj.isRemote) {
                        for (int i = 0; i < 13; ++i) {
                            AMParticle effect = (AMParticle) AMCore.instance.proxy.particleManager.spawn(
                                worldObj,
                                "hr_lensflare",
                                ent.posX + rand.nextDouble() - 0.5,
                                ent.posY + ent.getEyeHeight() + rand.nextDouble() - 0.5,
                                ent.posZ + rand.nextDouble() - 0.5);
                            if (effect != null) {
                                ...
                                effect
                                    .setRGBColorF(0.5f + rand.nextFloat() * 0.5f, 0.2f, 0.5f + rand.nextFloat() * 0.5f);
            ...
            if (movingobjectposition.typeOfHit == MovingObjectType.BLOCK) {
                ...
                AxisAlignedBB bb = block.getCollisionBoundingBoxFromPool(
                    worldObj,
                    movingobjectposition.blockX,
                    movingobjectposition.blockY,
                    movingobjectposition.blockZ);
                if (bb == null && !SpellUtils.instance
                    .modifierIsPresent(SpellModifiers.TARGET_NONSOLID_BLOCKS, getEffectStack(), 0)) doHit = false;
            }
        ...
        for (rotationPitch = (float) (Math.atan2(motionY, f)); rotationPitch - prevRotationPitch
        ...
        if (!AMCore.config.NoGFX() && worldObj.isRemote && this.getShootingEntity() instanceof EntityPlayer) {
            if (this.particleType == null || this.particleType.isEmpty()) particleType = AMParticleIcons.instance
                .getSecondaryParticleForAffinity(SpellUtils.instance.mainAffinityFor(getEffectStack()));
            AMParticle particle = (AMParticle) AMCore.proxy.particleManager
                .spawn(worldObj, particleType, this.posX, this.posY, this.posZ);
            if (particle != null) {
    ...
    protected void HitObject(MovingObjectPosition movingobjectposition, boolean pierce) {
        ...
        if (movingobjectposition.entityHit != null) {
            if ((movingobjectposition.entityHit == getShootingEntity() && !casterCast) || getShootingEntity() == null)
                return;
            ...
            if (e instanceof EntityLivingBase && getShootingEntity() != null
                && !this.entityHits.contains(movingobjectposition.entityHit.getEntityId())) {
                SpellHelper.instance
                    .applyStageToEntity(this.getEffectStack(), getShootingEntity(), this.worldObj, e, 0, true);
                if (isGlyphCast()) {
                    SpellHelper.instance.applyStackStage(
                        Glyph.getNewStack(getEffectStack(), 1),
                        getShootingEntity(),
                        (EntityLivingBase) e,
                        movingobjectposition.hitVec.xCoord,
                        movingobjectposition.hitVec.yCoord,
                    ...
                    SpellHelper.instance.applyStackStage(
                        SpellUtils.instance.popStackStage(getEffectStack()),
                        getShootingEntity(),
                        (EntityLivingBase) e,
                        movingobjectposition.hitVec.xCoord,
                        movingobjectposition.hitVec.yCoord,
            ...
            AMVector3 blockLoc = new AMVector3(
                movingobjectposition.blockX,
                movingobjectposition.blockY,
                movingobjectposition.blockZ);
            if (getShootingEntity() != null && !this.blockhits.contains(blockLoc)) {
                SpellHelper.instance.applyStageToGround(
                    getEffectStack(),
                    getShootingEntity(),
                    worldObj,
                    movingobjectposition.blockX,
