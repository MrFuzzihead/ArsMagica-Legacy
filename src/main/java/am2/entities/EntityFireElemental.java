    ...
    public void onLivingUpdate() {
        ...
        if (!this.worldObj.isRemote) {
            try {
                ...
                LogHelper.warn(
                    "This means that a fundamentally broken mod that modifies datawatcher IDs is present in your instance.");
                LogHelper.warn(
                    "Find it and remove it - it will cause issues with multiple mods, not just this one. Up until recently this");
                LogHelper.warn(
                    "was caused by ASJCore, however the new versions of it no longer cause this issue: well done Socol.");
    ...
    public void onUpdate() {
        ...
        if (cookTargetID != 0) {
            List<EntityItem> items = worldObj
                .getEntitiesWithinAABB(EntityItem.class, this.boundingBox.expand(cookRadius, cookRadius, cookRadius));
            ...
            if (inanimate != null && worldObj.isRemote) {
                AMParticle effect = (AMParticle) AMCore.instance.proxy.particleManager
                    .spawn(worldObj, "fire", posX, posY + getEyeHeight(), posZ);
                if (effect != null) {
                    effect.setIgnoreMaxAge(true);
                    effect.AddParticleController(
                        new ParticleApproachPoint(
                            effect,
                            inanimate.posX + (rand.nextFloat() - 0.5),
                            inanimate.posY + (rand.nextFloat() - 0.5),
                            inanimate.posZ + (rand.nextFloat() - 0.5),
                            0.1f,
                            0.1f,
                            1,
        ...
        if (worldObj.isRemote && rand.nextInt(100) > 75 && !isBurning())
            for (int i = 0; i < AMCore.config.getGFXLevel(); i++) worldObj.spawnParticle(
                "largesmoke",
