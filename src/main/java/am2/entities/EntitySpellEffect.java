...
public class EntitySpellEffect extends Entity {
    ...
    public void SetCasterAndStack(EntityLivingBase caster, ItemStack spellScroll) {
        ...
        if (spellStack != null) this.dataWatcher.updateObject(WATCHER_STACK, spellStack);
    }
    ...
    public void onUpdate() {
        ...
        if (dummycaster != null && dummycaster instanceof DummyEntityPlayer) dummycaster.onUpdate();
    ...
    @Override
    public void setDead() {
        if (dummycaster instanceof DummyEntityPlayer) dummycaster.setDead();
        super.setDead();
    }
            ...
            if (!AMCore.config.NoGFX()) {
                ...
                if ((AMCore.config.FullGFX() && this.ticksExisted % 2 == 0) || this.ticksExisted % 8 == 0) {
                    for (int i = 0; i < 4; ++i) {
                        ...
                        AMParticle effect = (AMParticle) AMCore.proxy.particleManager.spawn(
                            worldObj,
                            AMParticleIcons.instance
                                .getParticleForAffinity(SpellUtils.instance.mainAffinityFor(spellStack)),
                            x,
                            posY,
                            z);
                        if (effect != null) {
                            ...
                            if (AMCore.config.LowGFX()) {
                                effect.AddParticleController(
                                    new ParticleOrbitPoint(effect, posX, posY, posZ, 2, false)
        ...
        if (ticksToEffect <= 0) {
            ...
            if (this.dataWatcher.getWatchableObjectInt(WATCHER_GRAVITY) < 0 && !firstApply)
                SpellHelper.instance.applyStackStage(
                    ...
                    false,
                    this.ticksExisted);
            else SpellHelper.instance.applyStackStage(
                spellStack,
                dummycaster,
                null,
                posX,
    ...
    private void rainOfFireUpdate() {
        ...
        if (worldObj.isRemote) {
            ...
            for (Entity e : possibleTargets) {
                if (e != dummycaster) {
                    if (e instanceof EntityDragonPart
                        && ((EntityDragonPart) e).entityDragonObj instanceof EntityLivingBase)
                        e = (EntityLivingBase) ((EntityDragonPart) e).entityDragonObj;
                    ...
                    if (SpellHelper.instance
                        .attackTargetSpecial(null, e, DamageSources.causeEntityFireDamage(dummycaster), damage)
                        && !(e instanceof EntityPlayer)) e.hurtResistantTime = 10;
            ...
            if (this.dataWatcher.getWatchableObjectByte(WATCHER_ROF_IGNITE) == 1 && rand.nextInt(10) < 2) {
                ...
                if (worldObj.isAirBlock(pX, pY, pZ)) worldObj.setBlock(pX, pY, pZ, Blocks.fire);
            }
        }
    }
    ...
    private void blizzardUpdate() {
        ...
        if (worldObj.isRemote) {
            ...
            if (particle != null) {
                ...
                particle
                    .AddParticleController(new ParticleFleePoint(particle, new AMVector3(x, y, z), 0.1f, 3f, 1, false));
            }
            ...
            for (Entity e : possibleTargets) {
                if (e != dummycaster) {
                    ...
                    double lastVelZ = e.motionZ;
                    if (SpellHelper.instance
                        .attackTargetSpecial(null, e, DamageSources.causeEntityFrostDamage(dummycaster), damage)
                        && !(e instanceof EntityPlayer)) e.hurtResistantTime = 15;
                    e.addVelocity(-(e.motionX - lastVelX), -(e.motionY - lastVelY), -(e.motionZ - lastVelZ));
                }
            }
            ...
            if (rand.nextInt(10) < 2) {
                ...
                int pZ = (int) (posZ - radius + rand.nextInt((int) Math.ceil(radius) * 2));
                if (worldObj.isAirBlock(pX, pY, pZ) && !worldObj.isAirBlock(pX, pY - 1, pZ)
                    && worldObj.getBlock(pX, pY, pZ)
                        .isOpaqueCube())
    ...
    private void wallUpdate() {
        if (worldObj.isRemote) {
            ...
            for (float i = 0; i < dist; i += 0.5f) {
                ...
                AMParticle effect = (AMParticle) AMCore.proxy.particleManager.spawn(
                    worldObj,
                    AMParticleIcons.instance.getParticleForAffinity(SpellUtils.instance.mainAffinityFor(spellStack)),
                    x,
                    posY,
                    z);
                if (effect != null) {
                ...
                effect = (AMParticle) AMCore.proxy.particleManager.spawn(
                    worldObj,
                    AMParticleIcons.instance.getParticleForAffinity(SpellUtils.instance.mainAffinityFor(spellStack)),
                    x,
                    posY,
            ...
            if (ticksToEffect <= 0) {
                ...
                for (Entity e : possibleTargets) {
                    ...
                    if (e instanceof EntityLivingBase && hDistance < 0.75f && vDistance < 2) {
                        ...
                        // uncomment and comment out below line to revert to direct target only, but mark wave/wall as
                        // terminus
                        // SpellHelper.instance.applyStageToEntity(spellStack, dummycaster, worldObj, e, 0, false);
                        SpellHelper.instance.applyStackStage(
                            spellStack,
    ...
    private AMVector3[] getAllBlockLocationsBetween(AMVector3 a, AMVector3 b) {
        ...
        while (stepX != 0 || stepZ != 0) {
            if ((stepX < 0 && curPos.x <= b.x) || (stepX > 0 && curPos.x >= b.x)) stepX = 0;
            if ((stepZ < 0 && curPos.z <= b.z) || (stepZ > 0 && curPos.z >= b.z)) stepZ = 0;
    ...
    public void setRainOfFire(boolean ignite) {
        ...
        if (ignite) this.dataWatcher.updateObject(WATCHER_ROF_IGNITE, (byte) 1);
    }
