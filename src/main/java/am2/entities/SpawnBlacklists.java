...
public class SpawnBlacklists {
    ...
    private static final ArrayListMultimap<Integer, Class<? extends EntityLivingBase>> blacklistedDimensionSpawns = ArrayListMultimap
        .create();
    private static final ArrayListMultimap<Integer, Class<? extends EntityLivingBase>> blacklistedBiomeSpawns = ArrayListMultimap
        .create();
    ...
    public static void addBlacklistedDimensionSpawn(String entityClass, Integer dimensionID) {
        ...
        try {
            clazz = Class.forName(entityClass)
                .asSubclass(EntityLivingBase.class);
            ...
            LogHelper.info(
                "Unable to parse class name %s from IMC!  This needs to be corrected by the other mod author!",
                entityClass);
        }
    }
    ...
    public static boolean entityCanSpawnHere(double x, double z, World world, EntityLivingBase entity) {
        if (blacklistedDimensionSpawns.containsEntry(world.provider.dimensionId, entity.getClass())) return false;
        BiomeGenBase biome = world.getBiomeGenForCoords((int) x, (int) z);
        if (blacklistedBiomeSpawns.containsEntry(biome.biomeID, entity.getClass())) return false;
    ...
    public static void addBlacklistedDimensionForWorldgen(int dimensionID) {
        if (dimensionID == 0 || dimensionID == -1) return;
    ...
    public static void addButcheryBlacklist(Class<? extends EntityAnimal> clazz) {
        if (!butcheryBlacklist.contains(clazz)) butcheryBlacklist.add(clazz);
    }

    public static void addProgenyBlacklist(Class<? extends EntityAnimal> clazz) {
        if (!progenyBlacklist.contains(clazz)) progenyBlacklist.add(clazz);
    }
