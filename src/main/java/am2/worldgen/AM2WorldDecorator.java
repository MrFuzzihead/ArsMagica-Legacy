...
public class AM2WorldDecorator implements IWorldGenerator {
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,
        ...
        if (!SpawnBlacklists.worldgenCanHappenInDimension(world.provider.dimensionId)) return;
    ...
    public void generateOverworld(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,
        IChunkProvider chunkProvider) {
        boolean isDimensionBlacklisted = dimensionBlacklist.contains(world.provider.dimensionId);
        ...
        iteratorBiomeTypes: for (Type type : biomeTypes) {
            switch (type) {
                case BEACH:
                case SWAMP:
                case JUNGLE:
        ...
        if (!(AMCore.config.BlacklistAffectTrees() && isDimensionBlacklisted)) {
            if (random.nextInt(witchChance) == 0) {
                generateTree(
                    random.nextInt(AMCore.config.spawnHugeTrees() ? 6 : 1) == 0 ? new WitchwoodTreeHuge(true)
                        : new WitchwoodTreeEvenMoreHuge(true),
                    world,
                    random,
                    chunkX,
                    chunkZ);
        ...
        if (!(AMCore.config.BlacklistAffectPools() && isDimensionBlacklisted)) {
            ...
            if ((BiomeDictionary.isBiomeOfType(biome, Type.MAGICAL)
                || BiomeDictionary.isBiomeOfType(biome, Type.FOREST)) && random.nextInt(4) == 0
                && TerrainGen.populate(chunkProvider, world, random, chunkX, chunkZ, true, LAKE)
                && (AMCore.config.spawnEtherMode() == 0 || AMCore.config.spawnEtherMode() == 1)) {
                int lakeGenX = (chunkX * 16) + random.nextInt(16) + 8;
                int lakeGenY = random.nextInt(128);
                int lakeGenZ = (chunkZ * 16) + random.nextInt(16) + 8;
                (new WorldGenEssenceLakes(BlocksCommonProxy.liquidEssence))
                    .generate(world, random, lakeGenX, lakeGenY, lakeGenZ);
            }
        }
    }
