...
public class ClientTickHandler {
    ...
    private void gameTick_Start() {
        ...
        if (!CompendiumEntryTypes.instance.hasInitialized()) CompendiumEntryTypes.instance.initTextures();
        ...
        if (Minecraft.getMinecraft()
            ...
            if (worldName == null || !worldName.equals(
                Minecraft.getMinecraft()
                    ...
                    .getWorldName()
                    .replace(" ", "_"))) {
                worldName = Minecraft.getMinecraft()
                    .getIntegratedServer()
                    .getWorldName()
                    .replace(" ", "_");
                firstTick = true;
    ...
    public void onGuiOpen(GuiOpenEvent event) { // you're a ghost, how are you looking inside yourself
        if (MysteriumPatchesFixesMagicka.isPlayerEthereal(Minecraft.getMinecraft().thePlayer)
            && event.gui instanceof GuiInventory) event.setCanceled(true);
    }

    @SubscribeEvent
    public void onPlaySound(PlaySoundEvent17 event) {
        if (Minecraft.getMinecraft() != null) {
            if (Minecraft.getMinecraft().thePlayer != null && (AMPacketProcessorClient.deaf > 0)) event.result = null; // prevent
                                                                                                                       // sound
                                                                                                                       // playing
        }
    }
    ...
    private void spawnPowerPathVisuals() {
        ...
        if (canSeePaths(player)) {
                    ...
                    for (PowerTypes type : paths.keySet()) {
                        ...
                        for (LinkedList<AMVector3> individualPath : pathList) {
                            for (int i = 0; i < individualPath.size() - 1; ++i) {
                                ...
                                TileEntity teStart = Minecraft.getMinecraft().theWorld
                                    .getTileEntity((int) start.x, (int) start.y, (int) start.z);
                                TileEntity teEnd = Minecraft.getMinecraft().theWorld
                                    .getTileEntity((int) end.x, (int) end.y, (int) end.z);

                                if (!(teEnd instanceof IPowerNode)) break;

                                double startX = start.x
                                    + ((teStart instanceof IPowerNode) ? ((IPowerNode) teStart).particleOffset(0)
                                        : 0.5f);
                                double startY = start.y
                                    + ((teStart instanceof IPowerNode) ? ((IPowerNode) teStart).particleOffset(1)
                                        : 0.5f);
                                double startZ = start.z
                                    + ((teStart instanceof IPowerNode) ? ((IPowerNode) teStart).particleOffset(2)
                                        : 0.5f);
            ...
            while (it.hasNext()) {
                ...
                if (arc == null || arc.isDead) it.remove();
                else arc.setDead();
            }
    ...
    private boolean checkForTKMove(ItemStack stack) {
        if (stack.getItem() instanceof ItemSpellBook) {
            ...
            if (activeStack != null) stack = activeStack;
        }
    ...
    private void renderTick_Start() {
        if (!Minecraft.getMinecraft().inGameHasFocus) AMGuiHelper.instance.guiTick();
    }
    ...
    public void renderOverlays() {
        ...
        if (Minecraft.getMinecraft().thePlayer != null && Minecraft.getMinecraft().theWorld != null
            && (Minecraft.getMinecraft().inGameHasFocus || guiScreen instanceof GuiHudCustomization)) {
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        if (event.phase == TickEvent.Phase.START) {
            ...
            if (Minecraft.getMinecraft().theWorld != null) spawnPowerPathVisuals();
        }
        ...
        if (Minecraft.getMinecraft().thePlayer != null) {
            ...
            if (Minecraft.getMinecraft().thePlayer.inventory.armorInventory[3] != null
                && Minecraft.getMinecraft().thePlayer.inventory.armorInventory[3].getItem()
                    == ItemsCommonProxy.archmageHood) {
                if (!Minecraft.getMinecraft().thePlayer.isPotionActive(Potion.nightVision)) {
                    Minecraft.getMinecraft().thePlayer
                        .addPotionEffect(new PotionEffect(Potion.nightVision.id, Integer.MAX_VALUE));
                    Minecraft.getMinecraft().thePlayer.getActivePotionEffect(Potion.nightVision)
                        .setPotionDurationMax(true);
                    appliedEffect = true;
                } else if (Minecraft.getMinecraft().thePlayer.isPotionActive(Potion.nightVision)
                    && Minecraft.getMinecraft().thePlayer.getActivePotionEffect(Potion.nightVision)
                        .getDuration() < (Integer.MAX_VALUE / 10)) {
                            Minecraft.getMinecraft().thePlayer
                                .addPotionEffect(new PotionEffect(Potion.nightVision.id, Integer.MAX_VALUE));
                            Minecraft.getMinecraft().thePlayer.getActivePotionEffect(Potion.nightVision)
                                .setPotionDurationMax(true);
            ...
            if (Minecraft.getMinecraft().thePlayer.getCurrentArmor(0) != null) {
                if (Minecraft.getMinecraft().thePlayer.getCurrentArmor(0)
                    .getItem() == ItemsCommonProxy.archmageBoots) {
                    if (Minecraft.getMinecraft().thePlayer.worldObj.isAirBlock(
                        (int) Minecraft.getMinecraft().thePlayer.posX,
                        (int) Minecraft.getMinecraft().thePlayer.posY - 2,
                        (int) Minecraft.getMinecraft().thePlayer.posZ) && !Minecraft.getMinecraft().thePlayer.noClip) {
                        for (int i = 0; i < AMCore.config.getGFXLevel(); ++i) {
                            AMParticle cloud = (AMParticle) AMCore.proxy.particleManager.spawn(
                                Minecraft.getMinecraft().thePlayer.worldObj,
                                "sparkle2",
                                Minecraft.getMinecraft().thePlayer.posX,
                                Minecraft.getMinecraft().thePlayer.posY - 1.7,
    ...
    private boolean canSeePaths(EntityPlayer player) {
        ...
        if (delayEquipmentCheck == 20) {
            ...
            hasPathsVisuals = player.getCurrentArmor(3) != null && (player.getCurrentArmor(3)
                .getItem() instanceof IManaGoggle
