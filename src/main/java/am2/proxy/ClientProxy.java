...
public class ClientProxy extends CommonProxy {
    ...
    public void init() {
        ...
        MinecraftForgeClient
            .registerItemRenderer(Item.getItemFromBlock(BlocksCommonProxy.caster), CustomItemRenderer.instance);
    ...
    public void openSkillTreeUI(World world, EntityPlayer player) {
        if (world.isRemote) {
            Minecraft.getMinecraft()
                .displayGuiScreen(new GuiSkillTrees(player));
        }
    }
    ...
    public void openParticleBlockGUI(World world, EntityPlayer player, TileEntityParticleEmitter te) {
        if (world.isRemote) {
            Minecraft.getMinecraft()
                .displayGuiScreen(new GuiParticleEmitter(te));
        }
    }
    ...
    public boolean setMouseDWheel(int dwheel) {
        ...
        if (store) {
            clientTickHandler.setDWheel(
                dwheel / 120,
                Minecraft.getMinecraft().thePlayer.inventory.currentItem,
                Minecraft.getMinecraft().thePlayer.isUsingItem());
            return true;
        } else {
    ...
    private boolean checkForTKMove(ItemStack stack) {
        ...
        if (stack.getItem() instanceof ItemSpellBase && stack.hasTagCompound()
            && Minecraft.getMinecraft().thePlayer.isUsingItem()) {
    ...
    public void unlockCompendiumEntry(String id) {
        if (ArcaneCompendium.instance.isCategory(id)) unlockCompendiumCategory(id);
        else ArcaneCompendium.instance.unlockEntry(id);
    }
                ...
                if (data != null) {
                    ...
                    for (PowerTypes type : ((IPowerNode) te).getValidPowerTypes()) {
                        ...
                        RenderUtilities.drawTextInWorldAtOffset(
                            ...
                            target.blockY + yOff
                                - (player.prevPosY - (player.prevPosY - player.posY) * partialTicks)
