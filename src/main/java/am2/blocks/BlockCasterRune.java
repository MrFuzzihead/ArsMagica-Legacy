...
public class BlockCasterRune extends PoweredBlock {
   ...
   public void registerBlockIcons(IIconRegister iconRegister) {
      this.blockIcon = ResourceManager.RegisterTexture("CasterRune", iconRegister);
      this.sideIcon = ResourceManager.RegisterTexture("CasterRuneSide", iconRegister);
   }

   @SideOnly(Side.CLIENT)
   public IIcon getIcon(int side, int meta) {
      return side == 1 ? this.blockIcon : this.sideIcon;
   }
   ...
   public void onEntityWalking(World world, int x, int y, int z, Entity entity) {
      ...
      if (te != null && te instanceof TileEntityCasterRune) {
         ((TileEntityCasterRune)te).castSpell(entity);
      }
