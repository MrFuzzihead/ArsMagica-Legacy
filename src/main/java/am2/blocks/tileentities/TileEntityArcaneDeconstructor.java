...
public class TileEntityArcaneDeconstructor extends TileEntityAMPower
    implements IInventory, ISidedInventory, IKeystoneLockable {
    ...
    public void updateEntity() {
        ...
        if (worldObj.isRemote) {
            if (particleCounter == 0 || particleCounter++ > 20) { // fixed particle lingering
                particleCounter = 1;
                radiant = (AMParticle) AMCore.proxy.particleManager
                    .spawn(worldObj, "radiant", xCoord + 0.5f, yCoord + 0.5f, zCoord + 0.5f);
                if (radiant != null) {
        ...
        if (checkStack.getItem() == ItemsCommonProxy.spell) {
                    ...
                    for (Object o : componentParts) {
                        ...
                        if (stack != null) {
                            if (stack.getItem() == ItemsCommonProxy.bindingCatalyst) {
                                stack.setItemDamage(
                                    ((Binding) SkillManager.instance.getSkill("Binding")).getBindingType(checkStack));
                            }
            ...
            if (recipeParts != null && recipe.getRecipeOutput() != null) {
                if (recipe.getRecipeOutput()
                    .getItem() == checkStack.getItem()
                    && recipe.getRecipeOutput()
                        .getItemDamage() == checkStack.getItemDamage()
    ...
    private ItemStack objectToItemStack(Object o) {
        ...
        if (o instanceof ItemStack) output = (ItemStack) o;
        else if (o instanceof Item) output = new ItemStack((Item) o);
        else if (o instanceof Block) output = new ItemStack((Block) o);
        else if (o instanceof ArrayList) output = objectToItemStack(((ArrayList<?>) o).get(0));
    ...
    public ItemStack[] getRunesInKey() {
        return new ItemStack[] { inventory[13], inventory[14], inventory[15] };
    }
