...
public class TileEntityKeystoneRecepticle extends TileEntityAMPower
    implements IInventory, IMultiblockStructureController, IKeystoneLockable {
    ...
    public void onPlaced() {
        if (!worldObj.isRemote) {
            AMChunkLoader.INSTANCE
                .requestStaticChunkLoad(this.getClass(), this.xCoord, this.yCoord, this.zCoord, this.worldObj);
        }
    }
    ...
    public void invalidate() {
        ...
        if (!worldObj.isRemote) {
            AMChunkLoader.INSTANCE
                .releaseStaticChunkLoad(this.getClass(), this.xCoord, this.yCoord, this.zCoord, this.worldObj);
        }
    ...
    public void setActive(long key, boolean redPortal) {
        ...
        if (PowerNodeRegistry.For(worldObj)
            .getHighestPowerType(this) == PowerTypes.DARK) {
            myMeta |= 8;
        } else if (PowerNodeRegistry.For(worldObj)
            .getHighestPowerType(this) == PowerTypes.LIGHT) {
                myMeta |= 4;
            }
        ...
        if (this.isActive) {
            ...
            if (entities.size() == 1 && worldObj.canBlockSeeTheSky(xCoord, yCoord, zCoord)) {
                ...
                if (entity instanceof EntityPlayer) {
                    ...
                    if (player.isPotionActive(BuffList.haste) && player.isPotionActive(Potion.moveSpeed.id)
                        && player.isSprinting()) {
                        // if (worldObj.isRemote)
    ...
    public boolean canActivate() {
        ...
        allGood &= PowerNodeRegistry.For(this.worldObj)
            .checkPower(this);
    ...
    private void doTeleport(Entity entity) {
        ...
        if (this.isRedPortal()) {
                ...
                if (player.dimension != AMCore.config.getMMFDimensionID()) {
                    player.mcServer.getConfigurationManager()
                        .transferPlayerToDimension(
                            player,
                            AMCore.config.getMMFDimensionID(),
                            new AMTeleporter(
                                player.mcServer.worldServerForDimension(AMCore.config.getMMFDimensionID())));
                } else {
                    player.mcServer.getConfigurationManager()
                        .transferPlayerToDimension(
                            player,
                            0,
                            new AMTeleporter(player.mcServer.worldServerForDimension(0)));
                }
            }
        } else {
            AMVector3 newLocation = AMCore.instance.proxy.blocks
                .getNextKeystonePortalLocation(this.worldObj, xCoord, yCoord, zCoord, false, this.key);
            ...
            if (AMCore.config.getHazardousGateways()) {
                ...
                float charge = PowerNodeRegistry.For(this.worldObj)
                    .getHighestPower(this);
                ...
                this.worldObj
                    .playSoundEffect(newLocation.x, newLocation.y, newLocation.z, "mob.endermen.portal", 1.0F, 1.0F);
                return;
            }
    ...
    public void readFromNBT(NBTTagCompound nbttagcompound) {
        ...
        AMCore.instance.proxy.blocks.registerKeystonePortal(
            ...
            yCoord,
            zCoord,
            nbttagcompound.getInteger("keystone_receptacle_dimension_id"));
