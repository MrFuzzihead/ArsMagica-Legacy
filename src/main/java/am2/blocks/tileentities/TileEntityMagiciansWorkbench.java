...
public class TileEntityMagiciansWorkbench extends TileEntity implements IKeystoneLockable, ISidedInventory {
    ...
    public void setUpgradeStatus(byte flag, boolean set) {
        if (set) upgradeState |= flag;
        else upgradeState &= ~flag;
    ...
    public void rememberRecipe(ItemStack output, ItemStack[] recipeItems, boolean is2x2) {
        for (RememberedRecipe recipe : rememberedRecipes) {
            if (recipe.output.isItemEqual(output)) return;
        }
        if (!popRecipe()) {
            return;
        }

        for (ItemStack stack : recipeItems) if (stack != null) stack.stackSize = 1;
    ...
    private boolean popRecipe() {
        ...
        if (rememberedRecipes.size() < 8) return true;
    ...
    public void setRecipeLocked(int index, boolean locked) {
        if (index >= 0 && index < rememberedRecipes.size()) rememberedRecipes.get(index).isLocked = locked;
        ...
        for (RememberedRecipe recipe : rememberedRecipes) {
            try {
                ...
                for (int i = 0; i < recipe.components.length; ++i) {
                    ...
                    if (recipe.components[i] != null) recipe.components[i].writeToNBT(component);
