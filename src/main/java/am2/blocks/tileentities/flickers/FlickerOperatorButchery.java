...
public class FlickerOperatorButchery implements IFlickerFunctionality {
    ...
    public boolean DoOperation(World worldObj, IFlickerController habitat, boolean powered) {
        ...
        int radius = 6;
        List<EntityAnimal> creatures = worldObj.getEntitiesWithinAABB(
            EntityAnimal.class,
            AxisAlignedBB.getBoundingBox(
                ((TileEntity) habitat).xCoord - radius,
                ((TileEntity) habitat).yCoord - radius,
                ((TileEntity) habitat).zCoord - radius,
                ((TileEntity) habitat).xCoord + radius + 1,
                ((TileEntity) habitat).yCoord + radius + 1,
                ((TileEntity) habitat).zCoord + radius + 1));
        for (EntityAnimal creature : creatures) {
            Class<? extends EntityAnimal> clazz = creature.getClass();
            if (!SpawnBlacklists.canButcheryAffect(clazz)) continue;
            Integer count = entityCount.get(clazz);
            if (count == null) count = 0;
            if (!creature.isChild()) count++;
            ...
            if (count > 2) {
                if (worldObj.isRemote) {
                    AMParticle particle = (AMParticle) AMCore.proxy.particleManager.spawn(
                        worldObj,
                        "ghost",
                        ((TileEntity) habitat).xCoord + 0.5,
                        ((TileEntity) habitat).yCoord + 0.7,
                        ((TileEntity) habitat).zCoord + 0.5);
                    if (particle != null) {
    ...
    public Object[] getRecipe() {
        return new Object[] { "PBC", "FGL", "RER", Character.valueOf('P'), new ItemStack(Items.porkchop),
            ...
            new ItemStack(ItemsCommonProxy.flickerJar, 1, Affinity.LIFE.ordinal()), Character.valueOf('R'),
            new ItemStack(ItemsCommonProxy.rune, 1, ItemsCommonProxy.rune.META_RED), Character.valueOf('E'),
            new ItemStack(ItemsCommonProxy.evilBook) };
    }
