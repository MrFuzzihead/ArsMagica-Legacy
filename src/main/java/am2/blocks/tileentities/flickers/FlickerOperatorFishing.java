...
public class FlickerOperatorFishing implements IFlickerFunctionality {
    ...
    private static final List<WeightedRandomFishable> common_items = Arrays.asList(
    ...
    private static final List<WeightedRandomFishable> rare_items = Arrays.asList(
    ...
    private static final List<WeightedRandomFishable> uncommon_items = Arrays.asList(
    ...
    private static final List<WeightedRandomFishable> nether_items = Arrays.asList(
        new WeightedRandomFishable(new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_PYROGENICSEDIMENT), 10),
        new WeightedRandomFishable(new ItemStack(Items.blaze_powder), 25),
        new WeightedRandomFishable(new ItemStack(Items.nether_wart), 25),
        new WeightedRandomFishable(new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_SUNSTONEFRAGMENT), 5));
    private static final List<WeightedRandomFishable> ender_items = Arrays.asList(
        new WeightedRandomFishable(new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_COSMICDUST), 10),
        new WeightedRandomFishable(new ItemStack(Items.ender_pearl), 25),
        new WeightedRandomFishable(new ItemStack(Items.ender_eye), 25),
        new WeightedRandomFishable(new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_MOONSTONEFRAGMENT), 5));
    ...
    public boolean DoOperation(World worldObj, IFlickerController controller, boolean powered, Affinity[] flickers) {
        ...
        if (!powered || !checkSurroundings(worldObj, te.xCoord, te.yCoord, te.zCoord, flickers)
            || !worldObj.isBlockIndirectlyGettingPowered(te.xCoord, te.yCoord, te.zCoord)) return false;

        transferOrEjectItem(worldObj, pickFishingItem(worldObj), te.xCoord, te.yCoord, te.zCoord);
    ...
    private boolean checkSurroundings(World world, int x, int y, int z, Affinity[] flickers) {
        if (world.provider.dimensionId == -1) {
            for (Affinity aff : flickers) {
                if (aff == Affinity.FIRE) return true; // nether
            }
        }
        if (world.provider.dimensionId == 1 && y < 5) {
            for (Affinity aff : flickers) {
                if (aff == Affinity.ENDER) return true; // end
            }
        }
        for (int i = -1; i <= 1; ++i) {
            for (int j = -1; j >= -2; --j) {
    ...
    private ItemStack pickFishingItem(World world) {
        ...
        if (dropChance < minorUpgradeChance) {
            ...
            if (dropChance < majorUpgradeChance) {
                ...
                return ((WeightedRandomFishable) WeightedRandom.getRandomItem(rand, uncommon_items))
                    .func_150708_a(rand);
            }
        }
    }
    ...
    public int TimeBetweenOperation(boolean powered, Affinity[] flickers) {
        int time = 2000;
        for (Affinity aff : flickers) {
            if (aff == Affinity.LIGHTNING) time *= 0.8;
        }
        return time;
    }
    ...
    public Object[] getRecipe() {
        return new Object[] { " F ", "N W", " R ", 'F', Items.fish, 'W',
            new ItemStack(ItemsCommonProxy.flickerJar, 1, Affinity.WATER.ordinal()), 'N',
            new ItemStack(ItemsCommonProxy.flickerJar, 1, Affinity.NATURE.ordinal()), 'R',
            ItemsCommonProxy.itemInfernalFishingRod };
    }
