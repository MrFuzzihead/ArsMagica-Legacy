...
public class FlickerOperatorRegistry implements IFlickerRegistry {
    ...
    public boolean registerFlickerOperator(IFlickerFunctionality singleton, int mask) {
        if (registeredOperators.containsKey(mask)) {
            LogHelper.warn(
                "An addon attempted to register a flicker operator (%s) with a mask (%d) that is already in use.  The operator was NOT registered!",
                singleton.getClass()
                    .getName(),
                mask);
            return false;
        ...
        LogHelper.debug(
            "Registered Flicker operator %s to mask %d",
            singleton.getClass()
                .getName(),
            mask);
        return true;
    ...
    public void registerFlickerOperator(IFlickerFunctionality singleton, Affinity... affinities) {
        int mask = 0;
        for (Affinity aff : affinities) mask |= aff.getAffinityMask();
