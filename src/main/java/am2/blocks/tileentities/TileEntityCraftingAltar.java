...
public class TileEntityCraftingAltar extends TileEntityAMPower implements IMultiblockStructureController {
    ...
    final BlockDec[] augMaterials = new BlockDec[] { new BlockDec(Blocks.glass, 0), new BlockDec(Blocks.coal_block, 0),
        new BlockDec(Blocks.redstone_block, 0), new BlockDec(Blocks.iron_block, 0), new BlockDec(Blocks.lapis_block, 0),
        new BlockDec(Blocks.gold_block, 0), new BlockDec(Blocks.diamond_block, 0),
        new BlockDec(Blocks.emerald_block, 0), new BlockDec(BlocksCommonProxy.AMOres, META_MOONSTONE_BLOCK),
    ...
    void generateDefaultAugments(MultiblockStructureDefinition definition) {
        ...
        for (int i = 0; i < augMaterials.length; i++) {
            definition
                .addAllowedBlock(augMatl_primary[i], -1, 0, -2, augMaterials[i].getBlock(), augMaterials[i].getMeta());
            definition
                .addAllowedBlock(augMatl_primary[i], -1, 0, 2, augMaterials[i].getBlock(), augMaterials[i].getMeta());
            definition
                .addAllowedBlock(augMatl_primary[i], 1, 0, -2, augMaterials[i].getBlock(), augMaterials[i].getMeta());
            definition
                .addAllowedBlock(augMatl_primary[i], 1, 0, 2, augMaterials[i].getBlock(), augMaterials[i].getMeta());
            definition
                .addAllowedBlock(augMatl_primary[i], 0, -4, 0, augMaterials[i].getBlock(), augMaterials[i].getMeta());
        }
    ...
    private int getNumPartsInSpell() {
        int parts = 0;
        if (outputCombo != null) parts = outputCombo.length;

        if (shapeGroupGuide != null) {
            for (int[] ints : shapeGroupGuide) {
                if (ints != null) parts += ints.length;
            }
        }
        return parts;
    }
    ...
    private boolean currentDefinitionIsWithinStructurePower() {
        ...
        for (ArrayList<KeyValuePair<ISpellPart, byte[]>> part : shapeGroups) count += part.size();
    ...
    public void updateEntity() {
        ...
        if (!(worldObj.getTileEntity(
            ...
            zCoord + podiumLocation.getZ()) instanceof TileEntityLectern)) return; // crash fix
        if (isCrafting) {
            checkForEndCondition();
            updatePowerRequestData();
            if (!worldObj.isRemote && !currentDefinitionIsWithinStructurePower() && this.ticksExisted > 100) {
                worldObj.newExplosion(null, xCoord + 0.5, yCoord - 1.5, zCoord + 0.5, this.maxEffects, false, true); // the
                                                                                                                     // better
                                                                                                                     // the
                                                                                                                     // altar,
                                                                                                                     // the
                    ...
                    if (!worldObj.isRemote) {
                        ...
                        for (int i = 0; i < 5 * AMCore.config.getGFXLevel(); ++i) {
                            AMParticle particle = (AMParticle) AMCore.proxy.particleManager
                                .spawn(worldObj, "radiant", item.posX, item.posY, item.posZ);
                            if (particle != null) {
    ...
    private void updateLecternInformation() {
        ...
        if (!(worldObj.getTileEntity(
            ...
            yCoord + podiumLocation.getY(),
            zCoord + podiumLocation.getZ()) instanceof TileEntityLectern)) return; // crash fix
        TileEntityLectern lectern = (TileEntityLectern) worldObj.getTileEntity(
            xCoord + podiumLocation.getX(),
            yCoord + podiumLocation.getY(),
        ...
        if (lectern != null) {
            if (lectern.hasStack()) {
                ...
                if (lecternStack.hasTagCompound()) {
                    spellGuide = lecternStack.getTagCompound()
                        .getIntArray("spell_combo");
                    outputCombo = lecternStack.getTagCompound()
                        .getIntArray("output_combo");
                    currentSpellName = lecternStack.getDisplayName();

                    int numShapeGroups = lecternStack.getTagCompound()
                        .getInteger("numShapeGroups");
                    shapeGroupGuide = new int[numShapeGroups][];

                    for (int i = 0; i < numShapeGroups; ++i) {
                        shapeGroupGuide[i] = lecternStack.getTagCompound()
                            .getIntArray("shapeGroupCombo_" + i);
                    }
                }
    ...
    public boolean switchIsOn() {
        ...
        Block block = worldObj
            .getBlock(xCoord + switchLocation.getX(), yCoord + switchLocation.getY(), zCoord + switchLocation.getZ());
        boolean b = false;
        if (block == Blocks.lever) {
            for (int i = 0; i < 6; ++i) {
                b = (Blocks.lever.isProvidingStrongPower(
                    worldObj,
        ...
        if (stack != null && stack.getItem() instanceof ItemEssence && stack.getItemDamage() > ItemEssence.META_MAX) {
            if (switchIsOn()) {
                ...
                if (this.currentMainPowerTypes != PowerTypes.NONE && PowerNodeRegistry.For(this.worldObj)
                    ...
                    currentConsumedPower += (int) PowerNodeRegistry.For(worldObj)
                        .consumePower(
                            this,
                ...
                if (currentConsumedPower >= stack.stackSize) {
                    PowerNodeRegistry.For(this.worldObj)
                        .setPower(this, this.currentMainPowerTypes, 0);
                    if (!worldObj.isRemote) addItemToRecipe(
                        new ItemStack(ItemsCommonProxy.essence, stack.stackSize, ItemEssence.META_MAX + flags));
    ...
    private void pickPowerType(ItemStack stack) {
        ...
        for (PowerTypes type : PowerTypes.all()) {
            float tmpAmt = PowerNodeRegistry.For(worldObj)
                .getPower(this, type);
    ...
    private int getInstability() {
        ...
        for (Object objectPlayer : worldObj.playerEntities) {
            if (getDistanceSqXZ((EntityPlayer) objectPlayer, xCoord, zCoord) < range * range) // only XZ check to
                                                                                              // prevent stupidest
                                                                                              // altair hack
            {
                countPlayers++;
    ...
    private void randomInstabilityEffect(int fail) {
        ...
        for (Object objectPlayer : worldObj.playerEntities) {
            ...
            double newDistance = getDistanceSqXZ(entityplayer1, xCoord, zCoord); // only XZ check to prevent stupidest
                                                                                 // altair hack
        ...
        if (random.nextBoolean()) {
            ...
            for (int x = -7; x <= 7; x++) {
                for (int z = -7; z <= 7; z++) {
                    for (int y = -3; y <= 3; y++) {
                        Block blockBelow = worldObj
                            .getBlock((int) player.posX + x, (int) player.posY + y - 1, (int) player.posZ + z);
                        if (blockBelow.isNormalCube() && worldObj
                            .isAirBlock((int) player.posX + x, (int) player.posY + y, (int) player.posZ + z)) {
                            if (random.nextBoolean()) worldObj.setBlock(
                                (int) player.posX + x,
                                (int) player.posY + y,
                                (int) player.posZ + z,
                                Blocks.fire);
    ...
    private boolean matchCurrentRecipe() {
        ...
        if (part instanceof Summon) handleSummonShape();
        if (part instanceof Binding) handleBindingShape();
    ...
    private ArrayList<KeyValuePair<ISpellPart, byte[]>> getShapeGroupToAddTo() {
        for (int i = 0; i < shapeGroupGuide.length; ++i) {
            ...
            int addedLength = shapeGroups.get(i)
                .size();
            if (addedLength < guideLength) return shapeGroups.get(i);
        }
    ...
    private void handleSummonShape() {
        if (currentAddedItems.size() > 2) addedPhylactery = currentAddedItems.get(currentAddedItems.size() - 2);
    }

    private void handleBindingShape() {
        if (currentAddedItems.size() == 8) addedBindingCatalyst = currentAddedItems.get(currentAddedItems.size() - 1);
    }

    private List<EntityItem> lookForValidItems() {
        if (!isCrafting) return new ArrayList<>();
        double radius = worldObj.isRemote ? 2.1 : 2;
        return (List<EntityItem>) this.worldObj.getEntitiesWithinAABB(
            EntityItem.class,
    ...
    private void checkStructure() {
        ...
        if (valid) {
            ...
            mainmatlGroups = primary
                .getMatchedGroups(MultiblockStructureDefinition.MAINGROUP_MUTEX, worldObj, xCoord, yCoord, zCoord);
    ...
    private void checkForStartCondition() {
        ...
        List<EntityItem> items = this.worldObj.getEntitiesWithinAABB(
            EntityItem.class,
            AxisAlignedBB.getBoundingBox(xCoord - 2, yCoord - 3, zCoord - 2, xCoord + 2, yCoord, zCoord + 2));
        if (items.size() == 1) {
            EntityItem item = items.get(0);
            if (item != null && !item.isDead
                && item.getEntityItem()
                    .getItem() == ItemsCommonProxy.rune
                && item.getEntityItem()
            ...
            if (item != null && !item.isDead && item.getEntityItem() != null && checkEndItem(item.getEntityItem())) {
                if (!worldObj.isRemote) {
                    ...
                    worldObj.playSound(
                        ...
                        yCoord,
                        zCoord,
                        "arsmagica2:misc.craftingaltar.create_spell",
                        1.0f,
                        1.0f,
                        true);
                }
    ...
    private void createSpellItem() {
        ...
        if (!craftStack.hasTagCompound()) craftStack.stackTagCompound = new NBTTagCompound();
    ...
    private boolean compareItemStacks(ItemStack target, ItemStack input) {
        ...
        return target.getItem() == input.getItem()
            && (target.getItemDamage() == input.getItemDamage() || target.getItemDamage() == Short.MAX_VALUE)
            && target.stackSize == input.stackSize;
    }
    ...
    private NBTTagCompound ISpellPartListToNBT(ArrayList<KeyValuePair<ISpellPart, byte[]>> list) {
        ...
        for (int d = 0; d < list.size(); ++d) {
            ids[d] = SkillManager.instance.getShiftedPartID(
                list.get(d)
                    .getKey());
            meta[d] = list.get(d)
                .getValue();
