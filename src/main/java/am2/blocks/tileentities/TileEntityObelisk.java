...
public class TileEntityObelisk extends TileEntityAMPower implements IMultiblockStructureController, IInventory {
    ...
    protected void checkNearbyBlockState() {
        ...
        for (StructureGroup group : groups) {
            if (group == pillars) pillarsFound = true;
            else if (group == wizardChalkCircle) wizChalkFound = true;
        ...
        if (wizChalkFound) powerMultiplier = 1.25f;

        if (pillarsFound) powerMultiplier *= capsLevel;
    }
    ...
    private void sendCookUpdateToClients() {
        if (!worldObj.isRemote) {
            AMNetHandler.INSTANCE.sendObeliskUpdate(
                this,
                new AMDataWriter().add(PK_BURNTIME_CHANGE)
    ...
    public void handlePacket(byte[] data) {
        ...
        if (rdr.ID == PK_BURNTIME_CHANGE) this.burnTimeRemaining = rdr.getInt();
    }
    ...
    public void updateEntity() {
        ...
        if (isActive()) {
            if (surroundingCheckTicks % 100 == 0) {
                ...
                if (!worldObj.isRemote && PowerNodeRegistry.For(this.worldObj)
                    .checkPower(this, this.capacity * 0.1f)) {
                    List<EntityPlayer> nearbyPlayers = worldObj.getEntitiesWithinAABB(
                        EntityPlayer.class,
            ...
            float powerAmt = PowerNodeRegistry.For(worldObj)
                .getPower(this, PowerTypes.NEUTRAL);
            float powerAdded = inventory[0] != null
                ? ObeliskFuelHelper.instance.getFuelBurnTime(inventory[0]) * (powerBase * powerMultiplier)
                : 0;
            ...
            if (burnTimeRemaining <= 0 && powerAmt < chargeThreshold) {
                ...
                if (burnTimeRemaining > 0) {
                    ...
                    if (this.inventory[0].getItem()
                        .hasContainerItem(this.inventory[0]))
                        this.inventory[0] = new ItemStack(
                            this.inventory[0].getItem()
                                .getContainerItem());
                    else InventoryUtilities.decrementStackQuantity(this, 0, 1);
            ...
            if (burnTimeRemaining > 0) {
                ...
                PowerNodeRegistry.For(worldObj)
                    .insertPower(this, PowerTypes.NEUTRAL, powerBase * powerMultiplier);

                if (burnTimeRemaining % 20 == 0) sendCookUpdateToClients();
            }
        } else {
            surroundingCheckTicks = 1;
        }
    ...
    public ItemStack getStackInSlot(int i) {
        if (i < 0 || i >= this.getSizeInventory()) return null;
        return inventory[i];
    }
