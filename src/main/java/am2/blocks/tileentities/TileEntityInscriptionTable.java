...
public class TileEntityInscriptionTable extends TileEntity implements IInventory {
    ...
    public void updateEntity() {
        if (worldObj.isRemote && getUpgradeState() >= 3) candleUpdate();

        if (this.numStageGroups > MAX_STAGE_GROUPS) this.numStageGroups = MAX_STAGE_GROUPS;
    }
    ...
    private void candleUpdate() {
        ...
        if (isRenderingLeft()) {
            if (ticksToNextParticle == 0 || ticksToNextParticle == 15) {
                ...
                AMParticle effect = (AMParticle) AMCore.proxy.particleManager
                    .spawn(worldObj, "fire", particleX, yCoord + 1.32, particleZ);
                if (effect != null) {
                ...
                if (worldObj.rand.nextInt(100) > 80) {
                    AMParticle smoke = (AMParticle) AMCore.proxy.particleManager
                        .spawn(worldObj, "smoke", particleX, yCoord + 1.4, particleZ);
                    if (smoke != null) {
            ...
            if (ticksToNextParticle == 10 || ticksToNextParticle == 25) {
                ...
                AMParticle effect = (AMParticle) AMCore.proxy.particleManager
                    .spawn(worldObj, "fire", particleX, yCoord + 1.26, particleZ);
                if (effect != null) {
                    effect.setParticleScale(0.025f, 0.1f, 0.025f);
                    effect.AddParticleController(new ParticleHoldPosition(effect, 29, 1, false));
                    effect.setIgnoreMaxAge(false);
        ...
        switch (rdr.ID) {
            case FULL_UPDATE:
                ...
                for (int i = 0; i < partLength; ++i) {
                    ...
                    if (part instanceof ISpellPart) this.currentRecipe.add((ISpellPart) part);
                }
                ...
                for (int i = 0; i < numGroups; ++i) {
                    ...
                    for (int n : partData) {
                        ...
                        if (part instanceof ISpellPart) group.add((ISpellPart) part);
                    }
    ...
    private byte[] GetUpdatePacketForServer() {
        ...
        for (ISpellPart part : this.currentRecipe) {
            int id = part.getID();
            if (part instanceof ISpellComponent) id += SkillManager.COMPONENT_OFFSET;
            else if (part instanceof ISpellModifier) id += SkillManager.MODIFIER_OFFSET;
            writer.add(id);
        }
    ...
    public void addSpellPartToStageGroup(int groupIndex, ISpellPart part) {
        ...
        if (!currentSpellIsReadOnly && group.size() < 4 && !(part instanceof ISpellComponent)) {
            group.add(part);
            if (this.worldObj.isRemote) this.sendDataToServer();
            countModifiers();
        }
    }
    ...
    public void removeMultipleSpellParts(int startIndex, int length) {
        if (!currentSpellIsReadOnly) {
            for (int i = 0; i <= length; ++i) this.getCurrentRecipe()
                .remove(startIndex);
            countModifiers();
            if (this.worldObj.isRemote) this.sendDataToServer();
        }
    ...
    public ItemStack writeRecipeAndDataToBook(ItemStack bookstack, EntityPlayer player, String title) {
        if (bookstack.getItem() == Items.written_book && this.currentRecipe != null) {
            if (!currentRecipeIsValid().valid) return bookstack;

            if (!bookstack.hasTagCompound()) bookstack.setTagCompound(new NBTTagCompound());
            else if (bookstack.getTagCompound()
                .getBoolean("spellFinalized")) // don't overwrite a completed spell
                return bookstack;

            LinkedHashMap<String, Integer> materialsList = new LinkedHashMap<String, Integer>();

            materialsList.put(
                ItemsCommonProxy.rune
                    .getItemStackDisplayName(new ItemStack(ItemsCommonProxy.rune, 1, ItemRune.META_BLANK)),
                1);
            ...
            for (ArrayList<ISpellPart> shapeGroup : shapeGroups) {
                if (shapeGroup == null || shapeGroup.isEmpty()) continue;
            ...
            for (ISpellPart part : allRecipeItems) {
                ...
                if (recipeItems == null) {
                    LogHelper
                        .error("Unable to write recipe to book.  Recipe items are null for part %d!", part.getID());
                    return bookstack;
                }
                for (int i = 0; i < recipeItems.length; ++i) {
                    ...
                    if (o instanceof ItemStack) {
                        ...
                        if (((String) o).startsWith("P:")) {
                            ...
                            for (int powerID : ids) {
                                ...
                                materialkey.append(type.name())
                                    .append("/");
                            }

                            if (materialkey.toString()
                                .equals("Essence (")) {
                                ++i;
                                continue;
                            }
                            ...
                            if (materialkey.toString()
                                .startsWith("Essence (")) {
                                materialkey = new StringBuilder(
                                    materialkey.substring(0, materialkey.lastIndexOf("/")) + ")");
                                qty = (Integer) o;
                                int flag = 0;
                                for (int f : ids) {
                                    flag |= f;
                    ...
                    if (recipeStack != null) componentRecipeList.add(recipeStack);
                }
            }

            materialsList.put(
                ItemsCommonProxy.spellParchment.getItemStackDisplayName(new ItemStack(ItemsCommonProxy.spellParchment)),
                1);
            ...
            if (currentSpellName.isEmpty()) currentSpellName = "Spell Recipe";
    ...
    public void clearCurrentRecipe() {
        this.currentRecipe.clear();
        for (ArrayList<ISpellPart> group : shapeGroups) group.clear();
        currentSpellName = "";
    ...
    public void reverseEngineerSpell(ItemStack stack) {
        ...
        for (int i = 0; i < numShapeGroups; ++i) {
            ...
            for (int partID : parts) {
                ...
                if (entry instanceof ISpellPart) this.shapeGroups.get(i)
                    .add((ISpellPart) entry);
            }
        }
    ...
    public int getShapeGroupSize(int groupIndex) {
        return this.shapeGroups.get(groupIndex)
            .size();
    }
