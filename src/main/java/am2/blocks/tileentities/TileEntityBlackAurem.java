...
public class TileEntityBlackAurem extends TileEntityObelisk implements IMultiblockStructureController {
    ...
    public void updateEntity() {
        ...
        while (it.hasNext()) {
            ...
            if (EntityUtilities.isSummon(ent)) continue;
                ...
                if (distanceVertical < -2) {
                    ...
                    if (!worldObj.isRemote) {
                        if (ent.attackEntityFrom(DamageSources.darkNexus, 4)) {
                            if (ent.getHealth() <= 0) {
                                ent.setDead();
                                float power = ((int) Math.ceil((ent.getMaxHealth() * (ent.ticksExisted / 20)) % 5000))
                                    * this.powerMultiplier;
                                PowerNodeRegistry.For(this.worldObj)
                                    .insertPower(this, PowerTypes.DARK, power);
                            }
                        }
                    }
                }
        ...
        if (surroundingCheckTicks % 100 == 0) {
             ...
             * IllEffectsManager.instance.ApplyRandomBadThing(this, IllEffectSeverity.values()[maxSev],
             * BadThingTypes.DARKNEXUS);
             * }
             */
        }
    ...
    private void updateNearbyEntities() {
        ...
        for (EntityLivingBase entity : nearbyEntities) {
            if (entity.isEntityInvulnerable() || entity instanceof IBossDisplayData
                ...
                || entity instanceof EntityBroom) continue;
            if (!cachedEntities.contains(entity)) cachedEntities.add(entity);
        }
