...
public class TileEntityEssenceRefiner extends TileEntityAMPower
    implements IInventory, IKeystoneLockable, ISidedInventory {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (isRefining()) {
                setActiveTexture();
                if (PowerNodeRegistry.For(this.worldObj)
                    .checkPower(this, TICK_REFINE_COST)) {
                    remainingRefineTime--;
                    if (remainingRefineTime % 10 == 0) worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
                    if (remainingRefineTime <= 0) {
                        remainingRefineTime = 0;
                        if (!worldObj.isRemote) {
                            refineItem();
        ...
        if (isRefining()) {
            if (!worldObj.isRemote && (worldObj.getBlockMetadata(xCoord, yCoord, zCoord) & 0x8) != 0x8) {
                worldObj.setBlockMetadataWithNotify(
                    ...
                    yCoord,
                    zCoord,
                    worldObj.getBlockMetadata(xCoord, yCoord, zCoord) | 0x8,
                    2);
            }
        } else {
            if (!worldObj.isRemote && (worldObj.getBlockMetadata(xCoord, yCoord, zCoord) & 0x8) == 0x8) {
    ...
    private boolean canRefine() {
        ...
        ItemStack itemstack = RecipesEssenceRefiner.essenceRefinement()
            .GetResult(getCraftingGridContents(), null);
        if (itemstack == null) {
            return false;
        }
        ...
        if (inventory[OUTPUT_INDEX].stackSize < getInventoryStackLimit()
            && inventory[OUTPUT_INDEX].stackSize < inventory[OUTPUT_INDEX].getMaxStackSize()) {
            return true;
        }
    ...
    private void decrementCraftingGridSlot(int slot) {
        if (inventory[slot].getItem()
            .hasContainerItem()) {
            inventory[slot] = new ItemStack(
                inventory[slot].getItem()
                    .getContainerItem());
        } else {
