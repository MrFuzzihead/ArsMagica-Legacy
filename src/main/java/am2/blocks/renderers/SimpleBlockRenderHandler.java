...
public class SimpleBlockRenderHandler implements ISimpleBlockRenderingHandler {
    ...
    public void renderInventoryBlock(Block block, int metadata, int modelID, RenderBlocks renderer) {
        ...
        if (isInlay) GL11.glTranslatef(0, 0.5f, 0);
    ...
    private void renderWizardChalk(IBlockAccess world, int x, int y, int z, Block block, int modelId,
    ...
    private void renderCraftingAltar(Block block, Block mimic, int x, int y, int z, int metadata,
        RenderBlocks renderer) {
        ...
        if (icon == null) icon = block.getIcon(2, 0);
        renderer.renderFaceXPos(block, x, y, z, icon);

        icon = mimic.getIcon(3, metadata);
        if (icon == null) icon = block.getIcon(3, 0);
        renderer.renderFaceXNeg(block, x, y, z, icon);

        icon = mimic.getIcon(4, metadata);
        if (icon == null) icon = block.getIcon(4, 0);
        renderer.renderFaceZPos(block, x, y, z, icon);

        icon = mimic.getIcon(5, metadata);
        if (icon == null) icon = block.getIcon(5, 0);
    ...
    private void RenderBrokenPowerLink(RenderBlocks renderer, int x, int y, int z) {
        ...
        if ((x == 0 && y == 0 && z == 0) || (player != null && player.getCurrentArmor(3) != null
    ...
    private void RenderInlayBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        RenderBlocks renderer) {
    ...
    private void renderEverstoneBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        RenderBlocks renderer) {
    ...
    private void renderManaBattery(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        RenderBlocks renderer) {
        GL11.glColor4f(1, 1, 1, 1);
        ...
        if (te != null) {
            ...
            renderer.blockAccess = this.blockAccess;
            this.blockAccess.setControllingTileEntity(te);
            this.blockAccess.setFakeBlockAndMeta(
