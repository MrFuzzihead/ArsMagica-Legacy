...
public class CompendiumEntryItem extends CompendiumEntry {
    ...
    protected void parseEx(Node node) {
        ...
        for (int i = 0; i < childNodes.getLength(); ++i) {
            ...
            if (child.getNodeName()
                .equals("metarange")) {
                String[] metarange = child.getTextContent()
                    .split(",");
    ...
    protected GuiArcaneCompendium getCompendiumGui(String searchID, int meta) {
        ...
        if (split.length == 2) {
            ...
            if (item != null) {
                if (meta == -1) return new GuiArcaneCompendium(item);
                else return new GuiArcaneCompendium(searchID + "@" + meta, item, meta);
            }
        } else {
        ...
        if (split.length == 2) {
            ...
            for (Item item : AMCore.proxy.items.getArsMagicaItems()) {
                ...
                String itemID = item.getUnlocalizedName()
                    ...
                    .replace("arsmagica2:", "");
                if (itemID.equals(searchID)) {
                    if (meta == -1) return new ItemStack(item);
                    else return new ItemStack(item, 1, meta);
                }
            }
        }
