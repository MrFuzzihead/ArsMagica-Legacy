...
public class ArcaneCompendium implements ILoreHelper {
    ...
    public static Achievement compendiumData = (new Achievement(
        "am2_ach_data",
        "compendiumData",
        0,
        0,
        ItemsCommonProxy.arcaneCompendium,
        null));
    public static Achievement componentUnlock = (new Achievement(
        "am2_ach_unlock",
        "componentUnlock",
        0,
        0,
        ItemsCommonProxy.spellParchment,
        null));
    ...
    private String getWorldName() {
        if (ClientTickHandler.worldName == null) return null;
    ...
    public void loadUnlockData() {
        ...
        try {
            if (Minecraft.getMinecraft().thePlayer != null
                && ExtendedProperties.For(Minecraft.getMinecraft().thePlayer) != null) {
                hasLoaded = true;
                LogHelper.debug("Loading Unlock Data!");
                ExtendedProperties.For(Minecraft.getMinecraft().thePlayer)
                    .requestEntriesUpdateFromServer();
            }
        } catch (Exception e) {
    ...
    public void init(Language lang) {
        ...
        MCVersion = (String) ReflectionHelper
            .getPrivateValue(Minecraft.class, Minecraft.getMinecraft(), "field_110447_Z", "launchedVersion");
    ...
    private void loadDocument(InputStream stream) {
        ...
        try {
            ...
            Node parentNode = document.getChildNodes()
                .item(0);
            ...
            for (int i = 0; i < categories.getLength(); ++i) {
                ...
                if (category.getAttributes() != null) {
                    Node zeroItemText = category.getAttributes()
                        .getNamedItem("zeroItemText");
                ...
                for (int j = 0; j < entries.getLength(); ++j) {
                    ...
                    CompendiumEntryType type = CompendiumEntryTypes
                        .getForSection(category.getNodeName(), entry.getNodeName());
    ...
    private InputStream getPackagedCompendium(Language lang) {
        ResourceLocation rLoc = new ResourceLocation(
            "arsmagica2",
            String.format("docs/ArcaneCompendium_%s.xml", lang.getLanguageCode()));
        ...
        throw new MissingResourceException(
            "No packaged version of the compendium was found.  You may have a corrupted download.",
            "compendium",
            "Arcane Compendium");
    }
    ...
    private void unlockEntry(CompendiumEntry entry, boolean unlockRelated) {
        ...
        if (entry.isLocked()) {
            ...
            if (ExtendedProperties.For(Minecraft.getMinecraft().thePlayer)
                .getMagicLevel() > 0) Minecraft.getMinecraft().guiAchievement.func_146256_a(compendiumData);
            saveUnlockData();
        }
    }
    ...
    public void unlockEntry(String key) {
        ...
        if (verbose && key.equals("all")) {
            ...
            Minecraft.getMinecraft().thePlayer
                .addChatMessage(new ChatComponentText(StatCollector.translateToLocal("am2.tooltip.unlockSuccess")));
            return;
        }
            ...
            if (entry == null) {
                ...
                if (verbose) {
                    String message = String
                        .format(StatCollector.translateToLocal("am2.tooltip.compEntryNotFound"), key);
    ...
    public void unlockRelatedItems(ItemStack stack) {
        ...
        if (stack.getItem()
            .getUnlocalizedName() == null) return;
        ...
        if (stack.getItemDamage() > -1) itemID += "@" + stack.getItemDamage();
    ...
    public void setLockedState(boolean b) {
        for (CompendiumEntry entry : this.compendium.values()) entry.setIsLocked(b);
        saveUnlockData();
    }

    public boolean isCategory(String id) {
        for (CompendiumEntryType type : this.getCategories()) if (type.getCategoryName()
            .equals(id)) return true;
        return false;
    }

    @Override
    public void AddCompenidumEntry(Object entryItem, String entryKey, String entryName, String entryDesc, String parent,
        boolean allowReplace, String... relatedKeys) {
        if (entryItem == null) {
        ...
        if (existingEntry != null && !allowReplace) {
            LogHelper.warn(
                "Compendium entry with key %s exists, and allowReplace is false.  The entry was not added.",
                entryKey);
            return;
        }
        ...
        try { // this method doesn't *actually* get used, but it can be used for overwriting entries, as a result, this
              // part needs to be included
            if (Minecraft.getMinecraft().thePlayer != null
                && ExtendedProperties.For(Minecraft.getMinecraft().thePlayer) != null) {
                String state = "U";
                if (newEntry.isLocked) state += "L";
                if (newEntry.isNew) state += "N";
                ExtendedProperties.For(Minecraft.getMinecraft().thePlayer)
                    .setCompendiumEntry(entryKey, state);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
