...
public class ExtendedProperties implements IExtendedProperties, IExtendedEntityProperties {
    ...
    public float getMaxMana() {
        ...
        if (this.entity instanceof EntityPlayer) {
            if (SkillData.For((EntityPlayer) this.entity)
                .isEntryKnown(
                    SkillTreeManager.instance.getSkillTreeEntry(SkillManager.instance.getSkill("ManaCapacityIII")))) {
                max += 15000;
            } else if (SkillData.For((EntityPlayer) this.entity)
                .isEntryKnown(
                    SkillTreeManager.instance.getSkillTreeEntry(SkillManager.instance.getSkill("ManaCapacityII")))) {
                        max += 7000;
                    } else
                if (SkillData.For((EntityPlayer) this.entity)
                    .isEntryKnown(
                        SkillTreeManager.instance.getSkillTreeEntry(SkillManager.instance.getSkill("ManaCapacityI")))) {
                            max += 2000;
                        }
        }
        ...
        try {
            if (this.entity instanceof EntityPlayer) {
                if (this.entity.getActivePotionEffect(BuffList.manaBoost) != null) {
                    max *= (float) (1 + (0.25 * (this.entity.getActivePotionEffect(BuffList.manaBoost)
                        .getAmplifier() + 1)));
                }
            }
        } catch (NullPointerException ignored) {} // just in case
        try {
    ...
    public byte[] getUpdateData() {
        ...
        if ((this.updateFlags & UPD_BETA_PARTICLES) == UPD_BETA_PARTICLES && entity instanceof EntityPlayer
            && PlayerTracker.hasAA((EntityPlayer) entity)) {
    ...
    public boolean getCanHaveMoreSummons() {
        ...
        if (entity instanceof EntityLifeGuardian) return true;
    ...
    public boolean setMagicLevelWithMana(int level) {
        ...
        setMagicLevel(level);
        if (getMaxMana()
            > ((float) (Math.pow(level, 1.5f) * (85f * ((float) level / maxMagicLevel)) + 500f) / 2F) + 17000) {
            setMaxMana(((float) (Math.pow(level, 1.5f) * (85f * ((float) level / maxMagicLevel)) + 500f) / 2F) + 23000); // keep
                                                                                                                         // world
                                                                                                                         // fusion
                                                                                                                         // ritual
                                                                                                                         // effect
        } else {
            setMaxMana((float) (Math.pow(level, 1.5f) * (85f * ((float) level / maxMagicLevel)) + 500f) / 2F); // max
                                                                                                               // mana
                                                                                                               // without
                                                                                                               // bosses,
                                                                                                               // talents
    ...
    public void setCurrentFatigue(float currentFatigue) {
        ...
        if (currentFatigue >= getMaxFatigue()) {
            ...
            if (AMCore.config.getBurnoutEffects() && this.entity instanceof EntityPlayer
                && !((EntityPlayer) this.entity).capabilities.isCreativeMode) {
                Random random = new Random();
                if (currentFatigue > 50) { // lvl 5+
                    int roll = random.nextInt(4);
                    if (roll == 0) {
                        if (currentFatigue < 250) {
                            if (!this.entity.worldObj.isRemote) this.entity
                                .addPotionEffect(new PotionEffect(Potion.blindness.id, random.nextInt(100), 1));
                        }
                    } else if (roll == 1) {
                        ...
                        this.entity.worldObj
                            .spawnParticle("reddust", this.entity.posX, this.entity.posY, this.entity.posZ, f, f1, f2);
                    } else if (roll == 2) {
                        if (currentFatigue < 250) {
                            if (!this.entity.worldObj.isRemote)
                        ...
                        if (currentFatigue < 500) {
                            if (!this.entity.worldObj.isRemote) {
                                this.entity
                                    .addPotionEffect(new PotionEffect(Potion.digSlowdown.id, random.nextInt(160), 1));
                                this.entity
                                    .addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, random.nextInt(160), 1));
                            }
                        }
                    }
                }
                ...
                if (currentFatigue > 500) { // lvl 50+
                    ...
                    if (roll == 0) {
                        ...
                        this.entity.worldObj.addWeatherEffect(
                        ...
                        for (int i = 0; i < 100; i++) {
                            ...
                            if (this.entity.worldObj.isAirBlock((int) x, (int) y, (int) z)
                                && this.entity.worldObj.isAirBlock((int) x, (int) y - 1, (int) z)
                                && this.entity.worldObj.isAirBlock((int) x, (int) y + 1, (int) z)) {
    ...
    public void setMagicLevel(int magicLevel) {
        ...
        ticksForFullRegen = (int) Math
            .round(baseTicksForFullRegen * (0.75 - (0.25 * ((double) getMagicLevel() / maxMagicLevel))));
    ...
    public void updateManaLink(EntityLivingBase entity) {
        ...
        if (!this.manaLinks.contains(mle)) this.manaLinks.add(mle);
        else this.manaLinks.remove(mle);
    }
    ...
    public void setIsFlipped(boolean flipped) {
        if (flipped) this.bitFlag |= BIT_FLIPPED;
        else this.bitFlag &= ~BIT_FLIPPED;

        if (!entity.worldObj.isRemote) this.setUpdateFlag(UPD_BITFLAG);
    }

    public void setIsShrunk(boolean shrunk) {
        if (shrunk) this.bitFlag |= BIT_SHRUNK;
        else this.bitFlag &= ~BIT_SHRUNK;
    ...
    public void setEntityReference(EntityLivingBase entity) {
        ...
        if (isCritical) ticksToRegen = 5;
        else ticksToRegen = 20;
        ...
        if (entity.worldObj != null && entity.worldObj.isRemote
            ...
            if (entity != localPlayer) AMNetHandler.INSTANCE.requestAuras((EntityPlayer) entity);
        }
    ...
    public boolean handleDataPacket(byte[] data) {
        ...
        if ((flags & UPD_MANALINK) == UPD_MANALINK) {
            ...
            for (int i = 0; i < numLinks; ++i) {
                ...
                if (e instanceof EntityLivingBase) updateManaLink((EntityLivingBase) e);
            }
        }
    ...
    public void updateAuraData(int index, int behaviour, float scale, float alpha, boolean colorRandom,
        boolean colorDefault, int color, int delay, int quantity, float speed) {
    ...
    public void loadNBTData(NBTTagCompound compound) {
        ...
        if (armorProcCooldowns == null) armorProcCooldowns = new int[4];
        ...
        for (int i = 0; i < 5; i++) {
            try {
                this.contingencyStacks[i] = ItemStack
                    .loadItemStackFromNBT((NBTTagCompound) compound.getTag("contingency_effect" + i));
    ...
    private void syncCompendiumEntries(String name, String value) { // this is separate from the rest of syncing to ease
                                                                    // load on network
        AMDataWriter writer = new AMDataWriter();
    ...
    public void onSyncCompendiumDataPacket(byte[] remaining) {
        ...
        while (it.hasNext()) {
            ...
            entry.isLocked = pair.getValue()
                .contains("L");
            entry.isNew = pair.getValue()
                .contains("N");
        }
    }
    ...
    private void setDefaultCompendiumEntryValues() {
        // set them for client
        ArcaneCompendium.instance.init(
    ...
    public boolean isEntryLocked(String name) {
        return compendium_entries.get(name)
            .contains("L");
    }
    ...
    public Iterator<Map.Entry<String, String>> getCompendiumIterator() {
        return compendium_entries.entrySet()
            .iterator();
    }
        ...
        if (entity.worldObj != null && entity.worldObj.isRemote) {
            for (ManaLinkEntry entry : this.manaLinks) {
                ...
                if (e != null && e.getDistanceSqToEntity(entity) < entry.range && e.ticksExisted % 90 == 0) {
                    AMLineArc arc = (AMLineArc) AMCore.proxy.particleManager
                        .spawn(entity.worldObj, "textures/blocks/oreblockbluetopaz.png", e, entity);
                    if (arc != null) {
    ...
    public void addBurnout(float burnout) {
        if (entity.isPotionActive(BuffList.burnoutReduction)) burnout *= 0.75f;
    ...
    public void handleExtendedPropertySync() {
        ...
        if (!entity.worldObj.isRemote && this.getHasUpdate()) {
            ...
            AMNetHandler.INSTANCE.sendPacketToAllClientsNear(
                ...
                entity.posZ,
                32,
                AMPacketIDs.SYNC_EXTENDED_PROPS,
                data);
        }
        ...
        if (entity.worldObj.isRemote) {
            if (this.detectPossibleDesync()) {
                AMNetHandler.INSTANCE.sendPacketToServer(
                    AMPacketIDs.POSSIBLE_CLIENT_EXPROP_DESYNC,
                    new AMDataWriter().add(entity.getEntityId())
                        .generate());
            }
        }
    }
    ...
    public void addMagicXP(float amt) {
        ...
        if (magicXP >= xpToLevel) {
            ...
            if (this.entity instanceof EntityPlayer && magicLevel % 2 == 0) {
                ...
                if (magicLevel <= 20) SkillData.For(ent)
                    .incrementSpellPoints(SkillPointTypes.BLUE);
                else if (magicLevel <= 40) SkillData.For(ent)
                    .incrementSpellPoints(SkillPointTypes.GREEN);
                else if (magicLevel <= 50) SkillData.For(ent)
                    .incrementSpellPoints(SkillPointTypes.RED);
    ...
    public void procContingency(int num, EntityLivingBase attacker) {
        ...
        SpellHelper.instance.applyStackStage(
            spell,
            entity,
            attacker == null ? entity : attacker,
            entity.posX,
            entity.posY,
            entity.posZ,
            0,
            ...
            false,
            0);
        AMNetHandler.INSTANCE.sendSpellApplyEffectToAllAround(
            entity,
            attacker == null ? entity : attacker,
            entity.posX,
            entity.posY,
    ...
    public void manaBurnoutTick() {
        ...
        if (this.lightningsLeft > 0 && ticksLeft <= 0) {
            ...
            for (int i = 0; i < 10; i++) AMCore.proxy.particleManager.BoltFromPointToPoint(
                this.entity.worldObj,
                this.entity.posX + random.nextInt(14) - 7,
                this.entity.posY + random.nextInt(6) - 3,
                this.entity.posZ + random.nextInt(14) - 7,
                this.entity.posX,
                this.entity.posY,
                this.entity.posZ,
                1,
        ...
        if (ticksSinceLastRegen >= ticksToRegen) {
            ...
            if (getCurrentMana() < actualMaxMana) {
                if (entity instanceof EntityPlayer && ((EntityPlayer) entity).capabilities.isCreativeMode) {
                    ...
                    float regenTicks = (float) (ticksForFullRegen * entity.getAttributeMap()
                        ...
                        if (SkillData.For(player)
                            .isEntryKnown(
                                SkillTreeManager.instance
                                    .getSkillTreeEntry(SkillManager.instance.getSkill("ManaRegenIII")))) {
                            regenTicks *= 0.7f;
                        } else if (SkillData.For(player)
                            .isEntryKnown(
                                SkillTreeManager.instance
                                    .getSkillTreeEntry(SkillManager.instance.getSkill("ManaRegenII")))) {
                                        regenTicks *= 0.85f;
                                    } else
                            if (SkillData.For(player)
                                .isEntryKnown(
                                    SkillTreeManager.instance
                                        .getSkillTreeEntry(SkillManager.instance.getSkill("ManaRegenI")))) {
                                            regenTicks *= 0.95f;
                                        }
    ...
    public void flipTick() {
        ...
        if (entity instanceof EntityPlayer) {
            ...
            if (boots == null || boots.getItem() != ItemsCommonProxy.enderBoots) setIsFlipped(false);
        }

        prevFlipRotation = flipRotation;
        if (flipped && flipRotation < 180) flipRotation += 15;
        else if (!flipped && flipRotation > 0) flipRotation -= 15;
    }
    ...
    public boolean isManaLinkedTo(EntityLivingBase entity) {
        for (ManaLinkEntry entry : manaLinks) {
            if (entry.entityID == entity.getEntityId()) return true;
        }
        return false;
    }
    ...
    private static class ManaLinkEntry {
        ...
        public float getAdditionalCurrentMana(World world, EntityLivingBase host) {
            ...
            if (e == null || e.getDistanceSqToEntity(host) > range) return 0;
        ...
        public float deductMana(World world, EntityLivingBase host, float amt) {
            ...
            if (!world.isRemote) For(e).forceSync();
            return amt;
        }
        ...
        public boolean equals(Object obj) {
            if (obj instanceof ManaLinkEntry) return ((ManaLinkEntry) obj).entityID == this.entityID;
            return false;
        }
    }
