...
public class AoE implements ISpellShape {
    ...
    public SpellCastResult beginStackStage(ItemSpellBase item, ItemStack stack, EntityLivingBase caster,
        ...
        double radius = SpellUtils.instance
            .getModifiedDouble_Add(1, stack, caster, target, world, 0, SpellModifiers.RADIUS);
        ...
        if (side == 0 || side == 1) { // top/bottom
            if (world.isRemote) spawnAoEParticles(
                stack,
                caster,
                world,
                x + 0.5f,
                y + ((side == 1) ? 0.5f : (target != null ? target.getEyeHeight() : -2.0f)),
                z + 0.5f,
                (int) radius);
            int gravityMagnitude = SpellUtils.instance.countModifiers(SpellModifiers.GRAVITY, stack, 0);
            return applyStageHorizontal(
                stack,
                caster,
                world,
                (int) Math.floor(x),
    ...
    private void spawnAoEParticles(ItemStack stack, EntityLivingBase caster, World world, double x, double y, double z,
        int radius) {
            ...
            if (effect != null) {
                ...
                effect.AddParticleController(
                    new ParticleLeaveParticleTrail(effect, pfxName, false, 5, 1, false)
                        .addControllerToParticleList(
                            new ParticleFadeOut(effect, 1, false).setFadeSpeed(0.1f)
                                .setKillParticleOnFinish(true))
    ...
    private SpellCastResult applyStageHorizontal(ItemStack stack, EntityLivingBase caster, World world, int x, int y,
        int z, int face, int radius, int gravityMagnitude, boolean giveXP) {
        ...
        for (int i = -radius; i <= radius; ++i) {
            for (int j = -radius; j <= radius; ++j) {
                ...
                SpellCastResult result = SpellHelper.instance.applyStageToGround(
                    stack,
                    caster,
                    world,
                    x + i,
    ...
    private SpellCastResult applyStageVerticalX(ItemStack stack, EntityLivingBase caster, World world, int x, int y,
        int z, int face, int radius, boolean giveXP) {
        for (int i = -radius; i <= radius; ++i) {
            for (int j = -radius; j <= radius; ++j) {
                Block block = world.getBlock(x, y + i, z + j);
                if (block == Blocks.air) continue;
                SpellCastResult result = SpellHelper.instance
                    .applyStageToGround(stack, caster, world, x, y + i, z + j, face, x, y + i, z + j, 0, giveXP);
                if (result != SpellCastResult.SUCCESS) return result;
            }
        }
        return SpellCastResult.SUCCESS;
    ...
    private SpellCastResult applyStageVerticalZ(ItemStack stack, EntityLivingBase caster, World world, int x, int y,
        int z, int face, int radius, boolean giveXP) {
        for (int i = -radius; i <= radius; ++i) {
            for (int j = -radius; j <= radius; ++j) {
                ...
                if (block == Blocks.air) continue;
                SpellCastResult result = SpellHelper.instance
                    .applyStageToGround(stack, caster, world, x + j, y + i, z, face, x + j, y + i, z, 0, giveXP);
                if (result != SpellCastResult.SUCCESS) return result;
            }
        }
        return SpellCastResult.SUCCESS;
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_MOONSTONE),
    ...
    public float manaCostMultiplier(ItemStack spellStack) {
        ...
        for (int i = 0; i < stages; ++i) {
            if (SpellUtils.instance.getShapeForStage(spellStack, i)
                .getID() != this.getID()) continue;
            ...
            for (ISpellModifier modifier : mods) {
                if (modifier.getAspectsModified()
                    .contains(SpellModifiers.RADIUS)) {
                    radiusMods++;
                }
            }
        }
