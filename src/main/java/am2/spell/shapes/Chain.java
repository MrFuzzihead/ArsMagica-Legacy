...
public class Chain implements ISpellShape {
    ...
    public SpellCastResult beginStackStage(ItemSpellBase item, ItemStack stack, EntityLivingBase caster,
        ...
        double range = SpellUtils.instance
            .getModifiedDouble_Mul(4, stack, caster, target, world, 0, SpellModifiers.RANGE);
        int num_targets = SpellUtils.instance
            .getModifiedInt_Add(3, stack, caster, target, world, 0, SpellModifiers.PROCS);
        ...
        if (mop != null && mop.typeOfHit == MovingObjectType.ENTITY && mop.entityHit != null) {
            ...
            if (e instanceof EntityLivingBase) {
                do {
                    ...
                    List<EntityLivingBase> nearby = world
                        .getEntitiesWithinAABB(EntityLivingBase.class, e.boundingBox.expand(range, range, range));
        ...
        for (EntityLivingBase e : targets) {
            ...
            SpellHelper.instance
                .applyStackStage(newItemStack, caster, e, e.posX, e.posY, e.posZ, 0, world, true, giveXP, 0);

            if (world.isRemote) {
                if (prevEntity == null)
                    spawnChainParticles(world, x, y, z, e.posX, e.posY + e.getEyeHeight(), e.posZ, stack);
                else spawnChainParticles(
                    world,
                    prevEntity.posX,
                    prevEntity.posY + e.getEyeHeight(),
                    prevEntity.posZ,
                    e.posX,
    ...
    private void spawnChainParticles(World world, double startX, double startY, double startZ, double endX, double endY,
        ...
        if (aff == Affinity.LIGHTNING) {
            AMCore.proxy.particleManager
                .BoltFromPointToPoint(world, startX, startY, startZ, endX, endY, endZ, 1, color);
        } else {
            if (color == -1) color = aff.color;
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_SUNSTONE), Items.lead,
            Items.iron_ingot, Blocks.tripwire_hook, Items.string };
    }
