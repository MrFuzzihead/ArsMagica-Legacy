...
public class Glyph implements ISpellShape {
    ...
    public SpellCastResult beginStackStage(ItemSpellBase item, ItemStack stack, EntityLivingBase caster,
        ...
        if (caster.isSneaking()) { // set glyphs
                    ...
                    if (SkillData.For((EntityPlayer) caster)
                        .isEntryKnown(
                            SkillTreeManager.instance
                                .getSkillTreeEntry(SkillManager.instance.getSkill("ExtraGlyphs")))) {
            ...
            for (int i = 0; i < AMCore.config.getGFXLevel() * 5; ++i) {
                AMParticle symbols = (AMParticle) AMCore.proxy.particleManager
                    .spawn(world, "symbols2", x + 0.5F, y + 1.5f, z + 0.5f);
                if (symbols != null) {
                    ...
                    if (SpellUtils.instance.modifierIsPresent(SpellModifiers.COLOR, stack, 0)) {
                        ...
                        for (ISpellModifier mod : mods) {
                            if (mod instanceof Colour) {
                                ...
                                symbols
                                    .setRGBColorI((int) mod.getModifier(SpellModifiers.COLOR, null, null, null, meta));
                            }
                        }
                    }
            ...
            if (ep.hasExtraVariable("SPELLPOS")) {
                if (SkillData.For((EntityPlayer) caster)
                    .isEntryKnown(
                        SkillTreeManager.instance.getSkillTreeEntry(SkillManager.instance.getSkill("ExtraGlyphs")))) {
                ...
                SpellHelper.instance
                    .applyStackStage(newItemStack, caster, null, actualX, actualY, actualZ, 0, world, true, giveXP, 0);
                AMNetHandler.INSTANCE
                    .sendSpellApplyEffectToAllAround(caster, caster, actualX, actualY, actualZ, world, stack);
                if (!world.isRemote) {
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemRune.META_WHITE),
            new ItemStack(ItemsCommonProxy.rune, 1, ItemRune.META_BLACK),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_CHIMERITE),
            new ItemStack(ItemsCommonProxy.essence, 1, ItemEssence.META_AIR), Items.feather, Items.arrow,
            Items.golden_axe, Items.ender_eye, new ItemStack(Blocks.trapped_chest) };
    }
