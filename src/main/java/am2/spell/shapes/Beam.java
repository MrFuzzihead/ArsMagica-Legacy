...
public class Beam implements ISpellShape {
    ...
    public SpellCastResult beginStackStage(ItemSpellBase item, ItemStack stack, EntityLivingBase caster,
        ...
        if (mop == null) {
            ...
            if (shouldApplyEffect) {
                result = SpellHelper.instance.applyStageToGround(
                    stack,
                    caster,
                    world,
                    mop.blockX,
        ...
        if (world.isRemote && beamHitVec != null) {
            ...
            if (beam != null) {
                ...
                if (affinity == Affinity.LIGHTNING) {
                    AMCore.proxy.particleManager.BoltFromEntityToPoint(
                        ...
                        beamHitVec.zCoord,
                        1,
                        color == -1 ? affinity.color : color);
                } else {
                    beam = (AMBeam) AMCore.proxy.particleManager.BeamFromEntityToPoint(
                        world,
                        caster,
                        beamHitVec.xCoord,
                        beamHitVec.yCoord,
                        ...
                        color == -1 ? affinity.color : color);
                    if (beam != null) {
                        if (AMCore.proxy.getProxyUtils()
                            .isLocalPlayerInFirstPerson()) beam.setFirstPersonPlayerCast();
            ...
            for (int i = 0; i < AMCore.config.getGFXLevel() + 1; ++i) {
                AMParticle particle = (AMParticle) AMCore.proxy.particleManager.spawn(
                    world,
                    AMParticleIcons.instance.getParticleForAffinity(affinity),
                    beamHitVec.xCoord,
                    beamHitVec.yCoord,
                    beamHitVec.zCoord);
                if (particle != null) {
                    ...
                    particle.AddParticleController(
                        new ParticleMoveOnHeading(
                            ...
                            world.rand.nextDouble() * 360,
                            world.rand.nextDouble() * 0.2 + 0.02f,
                            1,
                            false));
                }
            }
        ...
        if (result != null && spellVec != null) {
            ...
            return SpellHelper.instance.applyStackStage(
                newItemStack,
                caster,
                target,
                spellVec.xCoord,
    ...
    public Object[] getRecipeItems() {
        return new Object[] { ItemsCommonProxy.standardFocus,
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_BLUETOPAZ),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_BLUETOPAZ),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_PURIFIEDVINTEUM), BlocksCommonProxy.aum,
