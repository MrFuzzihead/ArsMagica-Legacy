...
public class Rune implements ISpellShape {
    ...
    public SpellCastResult beginStackStage(ItemSpellBase item, ItemStack stack, EntityLivingBase caster,
        EntityLivingBase target, World world, double x, double y, double z, int side, boolean consumeMBR,
        int useCount) {
        ...
        if (!BlocksCommonProxy.spellRune.placeAt(
            world,
            mop.blockX,
            mop.blockY + 1,
            mop.blockZ,
            SpellUtils.instance.mainAffinityFor(stack)
                .ordinal())) {
        ...
        if (!world.isRemote) {
            world.setTileEntity(
                ...
                mop.blockY + 1,
                mop.blockZ,
                BlocksCommonProxy.spellRune.createTileEntity(world, 0));
            BlocksCommonProxy.spellRune
                .setSpellStack(world, mop.blockX, mop.blockY + 1, mop.blockZ, SpellUtils.instance.popStackStage(stack));
    ...
    public Object[] getRecipeItems() {
        ...
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemRune.META_BLUE),
