...
public class SpellUtils implements ISpellUtils {
    ...
    public double getModifiedDouble_Mul(double defaultValue, ItemStack stack, EntityLivingBase caster, Entity target,
        World world, SpellModifiers check) {
        return getModifiedDouble_Mul(defaultValue, stack, caster, target, world, 0, check);
    }

    public int getModifiedInt_Mul(int defaultValue, ItemStack stack, EntityLivingBase caster, Entity target,
        World world, SpellModifiers check) {
        return getModifiedInt_Mul(defaultValue, stack, caster, target, world, 0, check);
    }

    public double getModifiedDouble_Mul(SpellModifiers check, ItemStack stack, EntityLivingBase caster, Entity target,
        World world) {
        return getModifiedDouble_Mul(check, stack, caster, target, world, 0);
    }

    public int getModifiedInt_Mul(SpellModifiers check, ItemStack stack, EntityLivingBase caster, Entity target,
        World world) {
        return getModifiedInt_Mul(check, stack, caster, target, world, 0);
    }

    public double getModifiedDouble_Add(double defaultValue, ItemStack stack, EntityLivingBase caster, Entity target,
        World world, SpellModifiers check) {
        return getModifiedDouble_Add(defaultValue, stack, caster, target, world, 0, check);
    }

    public int getModifiedInt_Add(int defaultValue, ItemStack stack, EntityLivingBase caster, Entity target,
        World world, SpellModifiers check) {
        return getModifiedInt_Add(defaultValue, stack, caster, target, world, 0, check);
    }

    public double getModifiedDouble_Add(SpellModifiers check, ItemStack stack, EntityLivingBase caster, Entity target,
        World world) {
        return getModifiedDouble_Add(check, stack, caster, target, world, 0);
    }

    public int getModifiedInt_Add(SpellModifiers check, ItemStack stack, EntityLivingBase caster, Entity target,
        World world) {
    ...
    public double getModifiedDouble_Mul(double defaultValue, ItemStack stack, EntityLivingBase caster, Entity target,
        ...
        if (caster instanceof EntityPlayer) {
            if (SkillData.For((EntityPlayer) caster)
                .isEntryKnown(
                    SkillTreeManager.instance.getSkillTreeEntry(SkillManager.instance.getSkill("AugmentedCasting")))) {
                modifiedValue *= 1.1f;
            }
        }

        ModifierCalculatedEvent event = new ModifierCalculatedEvent(
            stack,
            caster,
            check,
            defaultValue,
    ...
    public SpellRequirements modifySpellRequirementsByAffinity(ItemStack stack, EntityLivingBase caster,
        SpellRequirements reqs) {
        HashMap<Affinity, Float> affinities = this.AffinityFor(stack);
        AffinityData affData = AffinityData.For(caster);

        if (affData == null) return reqs;

        float manaCost = reqs.manaCost;

        for (Affinity aff : affinities.keySet()) {
            float depth = affData.getAffinityDepth(aff);
            // calculate the modifier based on the player's affinity depth as well as how much of the spell is that
            // affinity
    ...
    private SpellRequirements calculateSpellRequirements(ItemStack stack, EntityLivingBase caster) {
        ...
        for (int i = stages - 1; i >= 0; --i) {
            ...
            for (ISpellComponent component : components) {
                ...
                if (componentReagents != null) reagents.addAll(Arrays.asList(componentReagents));
    ...
    private SpellRequirements parseSpellRequirements(ItemStack stack, EntityLivingBase caster) {
        float burnoutPct = (ExtendedProperties.For(caster)
            .getCurrentFatigue()
    ...
    private boolean spellRequirementsPresent(ItemStack stack, EntityLivingBase caster) {
        ...
        return stack.stackTagCompound.hasKey(BaseManaCostIdentifier)
            && stack.stackTagCompound.hasKey(BaseBurnoutIdentifier)
            && stack.stackTagCompound.hasKey(BaseReagentsIdentifier);
    }
    // ==============================================================================
    ...
    /**
     * Gets the shape group parts from the passed in ItemStack, based on the stack's internal state of current shape
     * group
     */
    public int[] getShapeGroupParts(ItemStack stack) {
        if (!stack.hasTagCompound() || !stack.stackTagCompound.hasKey(CurShapeGroup_Identifier)) return new int[0];
    ...
    public int[] getShapeGroupParts(ItemStack stack, int index) {
        if (!stack.hasTagCompound()) return new int[0];
    ...
    public void addShapeGroup(int[] shapeGroupParts, byte[][] metaDatas, ItemStack stack) {
        ...
        stack.stackTagCompound
            .setIntArray(String.format("%s%d", ShapeGroup_Identifier, numShapeGroups - 1), shapeGroupParts);
        for (int i = 0; i < metaDatas.length; ++i) {
            stack.stackTagCompound
                .setByteArray(String.format("%s%d%d", ShapeGroupMeta_Identifier, numShapeGroups - 1, i), metaDatas[i]);
        }
    }

    /**
     * Takes an item stack, looks at the current shape group assigned to it, and constructs a "classic" spell definition
     * from the shape group and the rest of the main spell def
     *
     * @param stack The item stack contaning the entire spell def, including all start shape groups
     * @return An ItemStack that contains a classic spell definition that the casting system can handle. This is what
     *         should be passed to the spell helper class.
     */
    public ItemStack constructSpellStack(ItemStack stack) {
        if (stack == null) return null;
        if (!stack.hasTagCompound() || !stack.stackTagCompound.hasKey(CurShapeGroup_Identifier)) return stack.copy();
        ...
        for (int i : shapeGroup) {
            ...
            if (entry instanceof ISpellShape) {
                ...
                byte[] meta = stack.stackTagCompound.getByteArray(
                    String.format("%s%d%d", ShapeGroupMeta_Identifier, currentShapeGroup, shapeGroupElementCount));
                if (meta == null) meta = new byte[0];
                newStages.get(newStages.size() - 1).definition
                    .addModifier(SkillManager.instance.getShiftedPartID(entry), meta);
            }
            shapeGroupElementCount++;
        }

        // check the old spell definition - if it starts with MissingShape, then inject the components and modifiers
        // into the last stage from the shape group
        if (numStages(stack) > 0 && !newStages.isEmpty()) {
            ...
            if (firstShape == SkillManager.instance.missingShape.getID()) {
                ...
                for (int i : components) last.definition.addComponent(i);

                HashMap<Integer, Integer> ordinals = new HashMap<Integer, Integer>();
                for (ISpellModifier modifier : modifiers) {
                    int ordinal = 0;
                    if (ordinals.containsKey(modifier.getID())) ordinal = ordinals.get(modifier.getID());
                    last.definition.addModifier(
                        modifier.getID() + SkillManager.MODIFIER_OFFSET,
                        getModifierMetadataFromStack(stack, modifier, 0, ordinal));
                }
            }
        }
        ...
        for (SpellStageDefinition stage : newStages) {
            addSpellStageToScroll(
                classicStack,
                stage.shape,
                stage.definition.getComponents(),
                stage.definition.getModifiers());
        ...
        for (int i = 0; i < numStages(stack); ++i) {
            ...
            for (int c : components) def.definition.addComponent(c);
            ISpellModifier[] modifiers = getModifiersForStage(stack, i);
            HashMap<Integer, Integer> ordinals = new HashMap<Integer, Integer>();
            for (ISpellModifier modifier : modifiers) {
                int ordinal = 0;
                if (ordinals.containsKey(modifier.getID())) ordinal = ordinals.get(modifier.getID());
                def.definition.addModifier(
                    SkillManager.instance.getShiftedPartID(modifier),
                    getModifierMetadataFromStack(stack, modifier, i, ordinal));
            ...
            addSpellStageToScroll(
                classicStack,
                def.shape,
                def.definition.getComponents(),
                def.definition.getModifiers());
        }
    ...
    public ItemStack popStackStage(ItemStack stack) {
        ...
        for (int i = 1; i < stages; ++i) {
            workingStack.stackTagCompound.setIntArray(
                Component_Prefix + (i - 1),
                workingStack.stackTagCompound.getIntArray(Component_Prefix + i));
            workingStack.stackTagCompound
                .setIntArray(Modifier_Prefix + (i - 1), workingStack.stackTagCompound.getIntArray(Modifier_Prefix + i));
            workingStack.stackTagCompound
                .setInteger(Shape_Prefix + (i - 1), workingStack.stackTagCompound.getInteger(Shape_Prefix + i));
        }
    ...
    public int numStages(ItemStack stack) {
        ...
        return stack.stackTagCompound.hasKey(Stages_Identifier) ? stack.stackTagCompound.getInteger(Stages_Identifier)
            : stack.stackTagCompound.getInteger("ShapeOrdinal_");
    }
    ...
    public ISpellComponent[] getComponentsForStage(ItemStack stack, int stage) {
        if (stack == null || !stack.hasTagCompound()) return new ISpellComponent[0];
        ...
        for (int i : componentIDs) {
            ...
            components[count++] = component instanceof ISpellComponent ? (ISpellComponent) component
                : SkillManager.instance.missingComponent;
        }
        return components;
    }

    public ISpellModifier[] getModifiersForStage(ItemStack stack, int stage) {
        if (stack == null || !stack.hasTagCompound()) return new ISpellModifier[0];
        ...
        for (int i : modifierIDs) {
            ...
            modifiers[count++] = modifier instanceof ISpellModifier ? (ISpellModifier) modifier
                : SkillManager.instance.missingModifier;
        }
        return modifiers;
    }
    ...
    public ISpellShape getShapeForStage(ItemStack stack, int stage) {
        ...
        if (SkillTreeManager.instance.isSkillDisabled(shape)) return SkillManager.instance.missingShape;
    ...
    public boolean casterHasAllReagents(EntityLivingBase caster, ArrayList<ItemStack> reagents) {
        if (caster instanceof EntityPlayer) {
            ...
            if (player.capabilities.isCreativeMode || reagents == null || reagents.isEmpty()) return true;
            InventoryPlayer inventory = player.inventory;
            // return Stream.of(inventory.mainInventory).map(ItemStack::getItem).allMatch( item ->
            // reagents.stream().map(ItemStack::getItem).anyMatch(item1 -> item1.equals(item)));
            return reagents.stream()
                .filter(Objects::nonNull)
                .map(ItemStack::getItem)
                .allMatch(
                    item -> Stream.of(inventory.mainInventory)
                        .filter(Objects::nonNull)
                        .map(ItemStack::getItem)
    ...
    public boolean casterHasMana(EntityLivingBase caster, float mana) {
        if (caster instanceof EntityPlayer && ((EntityPlayer) caster).capabilities.isCreativeMode) return true;
        return ExtendedProperties.For(caster)
            .getCurrentMana()
            + ExtendedProperties.For(caster)
                .getBonusCurrentMana()
            + getStoredItemMana(caster) >= mana;
    }
    ...
    public void addSpellStageToScroll(ItemStack scrollStack, int shape, int[] components,
        ListMultimap<Integer, byte[]> modifiers) {
        ...
        int[] modifierarray = new int[modifiers.values()
            .size()];
        int index = 0;
    ...
    public Affinity mainAffinityFor(ItemStack stack) {
        ...
        if (!stack.hasTagCompound()) return Affinity.NONE;
    ...
    public void doAffinityShift(EntityLivingBase caster, ISpellComponent component, ISpellShape governingShape,
        ...
        for (Affinity affinity : affList) {
            ...
            if (SkillData.For((EntityPlayer) caster)
                .isEntryKnown(
                    SkillTreeManager.instance.getSkillTreeEntry(SkillManager.instance.getSkill("AffinityGains")))) {
                shift *= 1.1f;
                xp *= 0.9f;
            }
            ItemStack chestArmor = ((EntityPlayer) caster).getCurrentArmor(2);
            if (ArmorHelper.isInfusionPreset(chestArmor, GenericImbuement.magicXP)) xp *= 1.25f;

            if (shift > 0) {
                AffinityChangingEvent event = new AffinityChangingEvent((EntityPlayer) caster, affinity, shift);
                MinecraftForge.EVENT_BUS.post(event);
                if (!event.isCanceled()) aff.incrementAffinity(affinity, event.amount);
            }
            if (xp > 0) {
                xp *= (float) caster.getAttributeMap()
                    .getAttributeInstance(ArsMagicaApi.xpGainModifier)
                    .getAttributeValue();
                // float level = (float)ExtendedProperties.For(caster).getMagicLevel();
                // float burnoutscaled = (ExtendedProperties.For(caster).getCurrentFatigue() /
                // ExtendedProperties.For(caster).getMaxFatigue());
    ...
    public void writeModifierMetadataToStack(ItemStack stack, ISpellModifier modifier, int stage, int ordinal,
        byte[] meta) {
        if (!stack.hasTagCompound()) return;
    ...
    public boolean componentIsPresent(ItemStack stack, Class clazz, int stage) {
        ...
        for (ISpellComponent comp : components) {
            if (comp.getClass() == clazz) return true;
        }
        return false;
    }
    ...
    public boolean spellIsChanneled(ItemStack stack) {
        ...
        if (numShapeGroups(stack) == 0 || !(shape instanceof MissingShape)) {
            ...
            if (finalShape != null) return finalShape.isChanneled();
        }
        return false;
    }

    public ItemStack createSpellStack(ArrayList<ArrayList<KeyValuePair<ISpellPart, byte[]>>> shapeGroups,
        ArrayList<KeyValuePair<ISpellPart, byte[]>> spell) {
        ArrayList<KeyValuePair<ISpellPart, byte[]>> recipeCopy = (ArrayList<KeyValuePair<ISpellPart, byte[]>>) spell
            .clone();

        if (!recipeCopy.isEmpty() && !(recipeCopy.get(0)
            .getKey() instanceof ISpellShape))
        ...
        while (!recipeCopy.isEmpty()) {
            ...
            if (part.getKey() instanceof ISpellShape) {
                ...
                SpellUtils.instance
                    .addSpellStageToScroll(stack, shape.getID(), ArrayListToIntArray(components), modifiers);
            }
        }
        ...
        for (ArrayList<KeyValuePair<ISpellPart, byte[]>> shapeGroup : shapeGroups) {
            ...
            for (int n = 0; n < shapeGroup.size(); ++n) {
                sgp[n] = SkillManager.instance.getShiftedPartID(
                    shapeGroup.get(n)
                        .getKey());
                sgp_m[n] = shapeGroup.get(n)
                    .getValue();
            }
        ...
        for (int i = 0; i < numStages(checkStack); ++i) {
            ...
            if (st > silkTouchLevel) silkTouchLevel = st;
            if (fn > fortuneLevel) fortuneLevel = fn;
        }
        ...
        if (silkTouchLevel > 0) AMEnchantmentHelper.silkTouchStack(stack, silkTouchLevel);
    ...
    public String getSpellMetadata(ItemStack stack, String key) {
        if (!stack.hasTagCompound() || !stack.stackTagCompound.hasKey(Global_Spell_Meta)) return "";
