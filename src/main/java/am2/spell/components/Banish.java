...
public class Banish implements ISpellComponent {
    ...
    public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
        ...
        if (casterP.hasExtraVariable("karma")) {
            if (casterP.getExtraVariable("karma")
                .equalsIgnoreCase("bad")) {
                if (((EntityLivingBase) target).isPotionActive(BuffList.astralDistortion.id)) {
                    if (target instanceof EntityPlayer) ((EntityPlayer) target).addChatMessage(
                ...
                ((EntityPlayer) caster).addChatMessage(
                    new ChatComponentText(
                        "Evil world-bending forces refuse to budge, as you have not broken inner harmony."));
                return true;
            }
        } else if (caster instanceof EntityPlayer) {
            ((EntityPlayer) caster).addChatMessage(
    ...
    public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target,
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            if (particle != null) {
                ...
                if (rand.nextBoolean()) particle.AddParticleController(
                    new ParticleOrbitEntity(particle, target, 0.1f, 1, false)
                        .SetTargetDistance(rand.nextDouble() + 0.5));
                else particle.AddParticleController(
                    new ParticleOrbitPoint(particle, x, y, z, 1, false).SetOrbitSpeed(0.1f)
                        .SetTargetDistance(rand.nextDouble() + 0.5));
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemsCommonProxy.rune.META_BLACK),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemEssence.META_NIGHTMARE),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_CELESTIALFISH), Items.compass,
            Items.ender_pearl };
    }
