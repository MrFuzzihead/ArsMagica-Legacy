    ...
    public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
        if (target instanceof EntityLivingBase) {
            if (((EntityLivingBase) target).getCreatureAttribute() == EnumCreatureAttribute.UNDEAD) {
                int healing = SpellUtils.instance
                    .getModifiedInt_Mul(10, stack, caster, target, world, 0, SpellModifiers.HEALING);
                target.setFire(2);
                return SpellHelper.instance.attackTargetSpecial(
                    stack,
                    target,
                    DamageSources.causeEntityHolyDamage(caster),
                    healing * (0.5f + 2 * AffinityData.For(caster)
                        .getAffinityDepth(Affinity.LIFE)));
            } else {
                int healing = SpellUtils.instance
                    .getModifiedInt_Mul(2, stack, caster, target, world, 0, SpellModifiers.HEALING);
                if (ExtendedProperties.For((EntityLivingBase) target)
                    .getCanHeal()) {
                    ((EntityLivingBase) target).heal(healing);
                    ExtendedProperties.For((EntityLivingBase) target)
                        .setHealCooldown(60);
                    return true;
                }
            }
            ...
            for (int i = 0; i < 25; ++i) {
                ...
                if (particle != null) {
                    ...
                    particle.AddParticleController(
                        new ParticleOrbitEntity(particle, target, 0.5f, 2, false).setIgnoreYCoordinate(true)
