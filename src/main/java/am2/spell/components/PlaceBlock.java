...
public class PlaceBlock implements ISpellComponent {
    ...
    private BlockDec getPlaceBlock(ItemStack stack) {
        if (stack.hasTagCompound() && stack.stackTagCompound.hasKey(KEY_BLOCKID)) {
            return new BlockDec(
                Block.getBlockById(stack.stackTagCompound.getInteger(KEY_BLOCKID)),
                stack.stackTagCompound.getInteger(KEY_META));
        }
        return null;
    }
    ...
    private void setPlaceBlock(ItemStack stack, Block block, int meta) {
        ...
        if (!stack.stackTagCompound.hasKey("Lore")) stack.stackTagCompound.setTag("Lore", new NBTTagList());
        ...
        tagList.appendTag(
            new NBTTagString(
                String.format(
                    StatCollector.translateToLocal("am2.tooltip.placeBlockSpell"),
    ...
    public boolean applyEffectBlock(ItemStack stack, World world, int blockx, int blocky, int blockz, int blockFace,
        ...
        if (spellStack == null || spellStack.getItem() != ItemsCommonProxy.spell
            || !SpellUtils.instance
                .componentIsPresent(spellStack, PlaceBlock.class, SpellUtils.instance.numStages(spellStack) - 1))
            return false;
