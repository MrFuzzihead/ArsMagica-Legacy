...
public class Recall implements ISpellComponent, IRitualInteraction {
    ...
    public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
        ...
        if (caster.isPotionActive(BuffList.astralDistortion.id)
            || ((EntityLivingBase) target).isPotionActive(BuffList.astralDistortion.id)) {
            if (caster instanceof EntityPlayer) ((EntityPlayer) caster)
        ...
        if (!casterProperties.getMarkSet()) {
            if (caster instanceof EntityPlayer && !world.isRemote) ((EntityPlayer) caster)
                .addChatMessage(new ChatComponentText(StatCollector.translateToLocal("am2.tooltip.noMark")));
            return false;
        } else if (casterProperties.getMarkDimension() != caster.dimension) {
            if (caster instanceof EntityPlayer && !world.isRemote) ((EntityPlayer) caster)
                .addChatMessage(new ChatComponentText(StatCollector.translateToLocal("am2.tooltip.diffDimMark")));
            return false;
        }
        if (!world.isRemote) {
            ((EntityLivingBase) target).setPositionAndUpdate(
                casterProperties.getMarkX(),
                casterProperties.getMarkY(),
                casterProperties.getMarkZ());
        }
        return true;
    }

    private boolean handleRitualReagents(ItemStack[] ritualRunes, World world, int x, int y, int z,
        ...
        if (!hasVinteumDust && ritualRunes.length == 3) {
            ...
            if (vector == null || vector.equals(new AMVector3(x, y, z))) {
                if (caster instanceof EntityPlayer && !world.isRemote) ((EntityPlayer) caster).addChatMessage(
                    new ChatComponentText(StatCollector.translateToLocal("am2.tooltip.noMatchingGate")));
                return false;
            } else {
        ...
        for (int i = 0; i < 25; ++i) {
            ...
            if (particle != null) {
                ...
                particle.AddParticleController(
                    new ParticleExpandingCollapsingRingAtPoint(particle, x, y - 1, z, 0.1, 3, 0.3, 1, false)
                        .setCollapseOnce());
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemRune.META_ORANGE), Items.compass,
            new ItemStack(Items.map, 1, Short.MAX_VALUE), Items.ender_pearl };
    }
    ...
    public ItemStack[] getReagents() {
        return new ItemStack[] { new ItemStack(ItemsCommonProxy.rune, 1, AMCore.ANY_META),
