...
public class Summon implements ISpellComponent {
    ...
    public EntityLiving summonCreature(ItemStack stack, EntityLivingBase caster, EntityLivingBase target, World world,
        ...
        try {
            entity = (EntityLiving) clazz.getConstructor(World.class)
                .newInstance(world);
        } catch (Throwable t) {
            t.printStackTrace();
            return null;
        ...
        int duration = SpellUtils.instance
            .getModifiedInt_Mul(4800, stack, caster, target, world, 0, SpellModifiers.DURATION);
    ...
    public Object[] getRecipeItems() {
        ...
        return new Object[] { new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_CHIMERITE),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_PURIFIEDVINTEUM),
            BlocksCommonProxy.cerublossom, ItemsCommonProxy.mobFocus,
    ...
    public void setSummonType(ItemStack stack, ItemStack phylacteryStack) {
        if (phylacteryStack.getItemDamage() == ItemsCommonProxy.crystalPhylactery.META_FULL
    ...
    public Class getSummonType(ItemStack stack) {
        ...
        if (s == null || s == "") s = "Skeleton"; // default! default! default!
    ...
    private Class checkForSpecialSpawns(ItemStack stack, Class clazz) {
        if (clazz == EntityChicken.class) {
            if (SpellUtils.instance.modifierIsPresent(SpellModifiers.DAMAGE, stack, 0)
                && SpellUtils.instance.componentIsPresent(stack, Haste.class, 0)) {
                return EntityBattleChicken.class;
            }
        } else if (clazz == EntityCow.class) {
            if (SpellUtils.instance.modifierIsPresent(SpellModifiers.DAMAGE, stack, 0)
                && SpellUtils.instance.componentIsPresent(stack, AstralDistortion.class, 0)) {
    ...
    public boolean applyEffectBlock(ItemStack stack, World world, int blockx, int blocky, int blockz, int blockFace,
        ...
        if (!world.isRemote) {
            if (ExtendedProperties.For(caster)
                ...
                if (caster instanceof EntityPlayer) {
                    ((EntityPlayer) caster).addChatMessage(
                        new ChatComponentText(StatCollector.translateToLocal("am2.tooltip.noMoreSummons")));
                }
            }
        }
    ...
    public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
        ...
        if (target instanceof EntityLivingBase && EntityUtilities.isSummon((EntityLivingBase) target)) return false;
