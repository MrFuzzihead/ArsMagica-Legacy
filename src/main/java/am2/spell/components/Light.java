...
public class Light implements ISpellComponent, IRitualInteraction {
    ...
    public boolean applyEffectBlock(ItemStack stack, World world, int blockx, int blocky, int blockz, int blockFace,
        ...
        if (!world.isRemote && (world.isAirBlock(blockx, blocky, blockz) || blockto == Blocks.snow
            || blockto == Blocks.water
            || blockto == Blocks.flowing_water
            || blockto instanceof BlockFlower)) {
            if (caster instanceof EntityPlayer) {
                BlockEvent.PlaceEvent place = new BlockEvent.PlaceEvent(
                    new BlockSnapshot(world, blockx, blocky, blockz, Blocks.water, 0),
                    block,
                    (EntityPlayer) caster);
    ...
    public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
        if (target instanceof EntityLivingBase) {
            ...
            if (!world.isRemote) ((EntityLivingBase) target).addPotionEffect(
                new BuffEffectIllumination(
                    duration,
    ...
    public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target,
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            if (particle != null) {
                ...
                particle
                    .addVelocity(rand.nextDouble() * 0.2 - 0.1, rand.nextDouble() * 0.2, rand.nextDouble() * 0.2 - 0.1);
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemRune.META_WHITE),
            BlocksCommonProxy.cerublossom, Blocks.torch, BlocksCommonProxy.vinteumTorch };
    }
    ...
    public ItemStack[] getReagents() {
        return new ItemStack[] { new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_MOONSTONE),
