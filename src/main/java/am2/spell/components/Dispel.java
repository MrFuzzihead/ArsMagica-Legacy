...
public class Dispel implements ISpellComponent {
    ...
    public boolean applyEffectBlock(ItemStack stack, World world, int blockx, int blocky, int blockz, int blockFace,
        ...
        ArrayList<EntityItem> items = (ArrayList<EntityItem>) world.getEntitiesWithinAABB(
            EntityItem.class,
            AxisAlignedBB.getBoundingBox(blockx - 2, blocky - 2, blockz - 2, blockx + 2, blocky + 2, blockz + 2));
        if (items.size() > 0) {
    ...
    public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
        ...
        if (effectsToRemove.size() == 0 && ExtendedProperties.For((EntityLivingBase) target)
            .getNumSummons() == 0) {
            return false;
        }
    ...
    private void removeEnchants(EntityItem target) {
        if (target.getEntityItem().stackTagCompound != null) {
            if (target.getEntityItem().stackTagCompound.getTagList("ench", 10)
                .tagCount() > 0) {
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemsCommonProxy.rune.META_PURPLE),
            ...
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_BLUETOPAZ), Items.milk_bucket };
    }
