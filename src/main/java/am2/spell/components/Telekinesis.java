...
public class Telekinesis implements ISpellComponent {
    ...
    private boolean doTK_Extrapolated(ItemStack stack, World world, double impactX, double impactY, double impactZ,
        EntityLivingBase caster) {
        if (caster instanceof EntityPlayer) {
            double range = ExtendedProperties.For(caster).TK_Distance;
            MovingObjectPosition mop = ItemsCommonProxy.spell
                .getMovingObjectPosition(caster, world, range, false, false);
            if (mop == null) {
                ...
                impactY = caster.posY + caster.getEyeHeight()
                    + (-Math.sin(Math.toRadians(caster.rotationPitch)) * range);
            }
        }
        ...
        for (Entity e : entities) {
            ...
            AMVector3 movement = MathUtilities
                .GetMovementVectorBetweenPoints(new AMVector3(e), new AMVector3(impactX, impactY, impactZ));
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemsCommonProxy.rune.META_PURPLE),
            Blocks.sticky_piston, Blocks.chest };
    }
