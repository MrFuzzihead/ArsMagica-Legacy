...
public class Bless implements ISpellComponent {
    ...
    public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
        ...
        while (iter.hasNext()) {
            ...
            if (Potion.potionTypes[potionID].isBadEffect) { // method is clientside only; we need the field
                ...
                // ((BuffEffect) pe).stopEffect((EntityLivingBase) target);
                // }
                // effectsToMagnify.put(potionID, pe.getDuration() + ":" + pe.getAmplifier() + ":" + (pe instanceof
                // BuffEffect));
            }
        }
        ...
        if (!world.isRemote) {
            ...
            // magnifyPotions(world, (EntityLivingBase) target, magnitudeLeft, targetAmplifier, targetDuration,
            // potionID, Integer.parseInt(effectsToMagnify.get(potionID).split(":")[0]),
            // Integer.parseInt(effectsToMagnify.get(potionID).split(":")[1]),
            // Boolean.parseBoolean(effectsToMagnify.get(potionID).split(":")[2]));
            // }
        }
        return true;
    }

    public static int magnifyPotions(World world, EntityLivingBase target, int magnitudeLeft, int targetAmplifier,
        int targetDuration, Integer potionID, int lastDuration, int lastAmplifier, boolean lastBuffEffect) {
                ...
                if (!world.isRemote) {
                    ...
                    if (lastBuffEffect) {
                        target.addPotionEffect(
                            buffEffectFromPotionID(
                                potionID,
                                Math.max(targetDuration, lastDuration),
                                Math.max(targetAmplifier, lastAmplifier)));
                    } else {
                        target.addPotionEffect(
                            new PotionEffect(
                                potionID,
    ...
    public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target,
        ...
        for (int i = 0; i < 25; ++i) {
            ...
            if (particle != null) {
                ...
                if (rand.nextBoolean()) particle.setRGBColorF(0.1f, 0.9f, 0.1f);
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.rune, 1, ItemRune.META_PINK),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_ARCANEASH),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemOre.META_BLUETOPAZ), Items.golden_apple };
    }
