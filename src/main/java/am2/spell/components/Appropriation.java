...
public class Appropriation implements ISpellComponent {
    ...
    public Object[] getRecipeItems() {
        return new Object[] { Items.ender_pearl,
            new ItemStack(BlocksCommonProxy.AMOres, 1, BlocksCommonProxy.AMOres.META_CHIMERITE_BLOCK), Blocks.chest };
    }
            ...
            if (originalSpellStack.stackTagCompound.hasKey(storageKey)) {
                ...
                if (world.isAirBlock(blockx, blocky, blockz) || !world.getBlock(blockx, blocky, blockz)
                    ...
                    if (stack.getTagCompound() != null) {
                        nbt = (NBTTagCompound) stack.getTagCompound()
                            .copy();
                    }
                    ...
                    if (stack.getTagCompound() != null) {
                        newNBT = (NBTTagCompound) stack.getTagCompound()
                            .copy();
                    }
                    ...
                    if (blockSnapshots.size() > 1) {
                        placeEvent = ForgeEventFactory
                            .onPlayerMultiBlockPlace(casterPlayer, blockSnapshots, ForgeDirection.UNKNOWN);
                    } else if (blockSnapshots.size() == 1) {
                        placeEvent = ForgeEventFactory
                            .onPlayerBlockPlace(casterPlayer, blockSnapshots.get(0), ForgeDirection.UNKNOWN);
                    }
                    ...
                    if (placeEvent != null && (placeEvent.isCanceled())) {
                        ...
                        for (net.minecraftforge.common.util.BlockSnapshot blocksnapshot : blockSnapshots) {
                            ...
                            if (newBlock != null && !(newBlock.hasTileEntity(metadata))) { // Containers get placed
                                                                                           // automatically
                    ...
                    // restore((EntityPlayer)caster, world, originalSpellStack, blockx, blocky, blockz, impactX,
                    // impactY, impactZ);
                }
            } else {
                ...
                // MultiblockStructureDefinition.BlockDec blockDec = new MultiblockStructureDefinition.BlockDec(block,
                // meta);
                ...
                if (!world.isRemote) {
                    BreakEvent event = ForgeHooks.onBlockBreakEvent(
                        world,
                        ((EntityPlayerMP) caster).theItemInWorldManager.getGameType(),
    ...
    public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
        if (target instanceof EntityPlayer || target instanceof IBossDisplayData) return false;

        if (!(target instanceof EntityLivingBase)) return false;

        for (Class clazz : AMCore.config.getAppropriationMobBlacklist()) if (target.getClass() == clazz) return false;
    ...
    private ItemStack getOriginalSpellStack(EntityPlayer caster) {
        ...
        if (originalSpellStack == null) return null;
            ...
            originalSpellStack = ((ItemSpellBook) originalSpellStack.getItem()).GetActiveItemStack(originalSpellStack); // it's
                                                                                                                        // a
                                                                                                                        // spell
                                                                                                                        // book
                                                                                                                        // -
                                                                                                                        ...
                                                                                                                        // active
                                                                                                                        // scroll
            // sanity check needed here because from cast to apply the spell could have changed - just ensure
            // appropriation is a part of this spell somewhere so that any stored item can be retrieved
    ...
    private void restore(EntityPlayer player, World world, ItemStack stack, int x, int y, int z, double hitX,
        ...
        if (stack.stackTagCompound.hasKey(storageKey)) {
            ...
            if (storageCompound != null) {
                ...
                if (type.equals("ent")) {
                    ...
                    try {
                        Entity ent = (Entity) Class.forName(clazz)
                    ...
                    if (block != null) {
                        ...
                        if (!player.worldObj.isRemote) player.addChatComponentMessage(
                            new ChatComponentText(StatCollector.translateToLocal("am2.tooltip.approError")));
        ...
        for (int i = 0; i < 5 + 5 * AMCore.config.getGFXLevel(); ++i) {
            ...
            if (particle != null) {
                ...
                particle.AddParticleController(
                    new ParticleOrbitPoint(particle, x, y, z, 1, false)
                        .SetTargetDistance(world.rand.nextDouble() + 0.1f)
                        .SetOrbitSpeed(0.2f));
            }
        }
    }
