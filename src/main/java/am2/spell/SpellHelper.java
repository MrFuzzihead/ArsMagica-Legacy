...
public class SpellHelper {
    ...
    public SpellCastResult applyStageToGround(ItemStack stack, EntityLivingBase caster, World world, int blockX,
        int blockY, int blockZ, int blockFace, double impactX, double impactY, double impactZ, int stage,
        boolean consumeMBR) {
        ...
        for (ISpellComponent component : components) {
            ...
            if (SkillTreeManager.instance.isSkillDisabled(component)) continue;
            ...
            if (canApplyToBlock(stack, caster, world, blockX, blockY, blockZ)) {
                if (component.applyEffectBlock(
                        ...
                        if (SpellUtils.instance.modifierIsPresent(SpellModifiers.COLOR, stack, 0)) {
                            ...
                            for (ISpellModifier mod : mods) {
                                if (mod instanceof Colour) {
                                    byte[] meta = SpellUtils.instance
                                        .getModifierMetadataFromStack(stack, mod, 0, ordinalCount++);
                                    color = (int) mod.getModifier(SpellModifiers.COLOR, null, null, null, meta);
                                }
                            }
                        }
                        component.spawnParticles(
                            world,
                            blockX + 0.5,
                            blockY + 0.5,
                            blockZ + 0.5,
                    ...
                    if (consumeMBR) SpellUtils.instance.doAffinityShift(caster, component, stageShape, mana);
                }
            }
        }
        ...
        int persistenceModifiers = SpellUtils.instance
            .getModifiedInt_Add(0, stack, caster, caster, world, 0, SpellModifiers.LINGERING);
        if (persistenceModifiers > 0) {
            int timeToNextCast = SpellUtils.instance
                .getModifiedInt_Mul(15, stack, caster, caster, world, 0, SpellModifiers.DURATION);
            lingeringSpellList.add(
                new LingeringSpell(
    ...
    public SpellCastResult applyStageToEntity(ItemStack stack, EntityLivingBase caster, World world, Entity target,
        int stage, boolean shiftAffinityAndXP) {
        ISpellShape stageShape = SpellUtils.instance.getShapeForStage(stack, 0);
        if (stageShape == null) return SpellCastResult.MALFORMED_SPELL_STACK;

        if ((!AMCore.config.getAllowCreativeTargets()) && target instanceof EntityPlayerMP
            && ((EntityPlayerMP) target).capabilities.isCreativeMode) {
        ...
        for (ISpellComponent component : components) {
            ...
            if (canApplyToEntity(stack, caster, world, target)) {
                if (component.applyEffectEntity(stack, world, caster, target)) {
                    ...
                    if (shiftAffinityAndXP) SpellUtils.instance.doAffinityShift(caster, component, stageShape, mana);
                }
            }
        }
        ...
        if (appliedOneComponent) {
            int persistenceModifiers = SpellUtils.instance
                .getModifiedInt_Add(0, stack, caster, target, world, 0, SpellModifiers.LINGERING);
            if (persistenceModifiers > 0) {
                int timeToNextCast = SpellUtils.instance
                    .getModifiedInt_Mul(15, stack, caster, target, world, 0, SpellModifiers.DURATION);
                lingeringSpellList
                    .add(new LingeringSpell(persistenceModifiers, stack, world, caster, target, timeToNextCast));
            }
            return SpellCastResult.SUCCESS;
        } else {
            return SpellCastResult.EFFECT_FAILED;
    ...
    private boolean canApplyToEntity(ItemStack stack, EntityLivingBase caster, World world, Entity target) {
        if (SpellUtils.instance.getModifiedInt_Add(SpellModifiers.KEYSTONE_REC, stack, caster, target, world, 0) != 0
        ...
        if (SpellUtils.instance.getModifiedInt_Add(SpellModifiers.TARGET_PLAYERS, stack, caster, target, world, 0) != 0
            && !(target instanceof EntityPlayer)) {
            return false;
        } else
            if (SpellUtils.instance.getModifiedInt_Add(SpellModifiers.TARGET_MONSTERS, stack, caster, target, world, 0)
                != 0 && !(target instanceof EntityMob)) {
                    return false;
                } else if (SpellUtils.instance
                    .getModifiedInt_Add(SpellModifiers.TARGET_CREATURES, stack, caster, target, world, 0) != 0
                    && !(target instanceof EntityAgeable)) {
                        return false;
                    }
        if (SpellUtils.instance.getModifiedInt_Add(SpellModifiers.TARGET_BLOCKS, stack, caster, target, world, 0)
            != 0) {
            return false;
        }
        return true;
    ...
    private boolean canApplyToBlock(ItemStack stack, EntityLivingBase caster, World world, int x, int y, int z) {
        if (SpellUtils.instance.getModifiedInt_Add(SpellModifiers.KEYSTONE_REC, stack, caster, caster, world, 0) != 0
            && caster instanceof EntityPlayer) {
            TileEntity te = world.getTileEntity(x, y, z);
            if (te != null) {
                if (te instanceof IKeystoneLockable) {
                    if (KeystoneUtilities.instance
                        .canPlayerAccess((IKeystoneLockable) te, (EntityPlayer) caster, KeystoneAccessType.NONE)) {
                    ...
                    if (casterKey != -1 && (casterKey == blockKey)) return false; // if matching keystone in inventory
                                                                                  // of TE, don't want to break
                }
            }
        }
        if (SpellUtils.instance.getModifiedInt_Add(SpellModifiers.TARGET_BLOCKS, stack, caster, caster, world, 0)
            == 0) {
            if (SpellUtils.instance.getModifiedInt_Add(SpellModifiers.TARGET_PLAYERS, stack, caster, caster, world, 0)
                != 0) {
                return false;
            } else if (SpellUtils.instance
                .getModifiedInt_Add(SpellModifiers.TARGET_MONSTERS, stack, caster, caster, world, 0) != 0) {
                    return false;
                } else if (SpellUtils.instance
                    .getModifiedInt_Add(SpellModifiers.TARGET_CREATURES, stack, caster, caster, world, 0) != 0) {
                        return false;
                    }
        }
        return true;
    ...
    private SpellCastingEvent.Pre preSpellCast(ItemStack stack, EntityLivingBase caster, boolean isChanneled) {
        ...
        SpellCastingEvent.Pre event = new SpellCastingEvent().new Pre(stack, (ItemSpellBase) stack.getItem(), caster,
            manaCost, burnout, isChanneled);
        ...
        if (!SpellUtils.instance.casterHasMana(caster, manaCost)) event.castResult = SpellCastResult.NOT_ENOUGH_MANA;

        return event;
    }

    public SpellCastResult applyStackStage(ItemStack stack, EntityLivingBase caster, EntityLivingBase target, double x,
        double y, double z, int side, World world, boolean consumeMBR, boolean giveXP, int ticksUsed) {

        if (caster.isPotionActive(BuffList.silence.id)) return SpellCastResult.SILENCED;
        ...
        if (SkillTreeManager.instance.isSkillDisabled(shape)) return SpellCastResult.EFFECT_FAILED;
        ...
        if (consumeMBR) {
            ...
            if (checkEvent.castResult != SpellCastResult.SUCCESS) {
                ...
                SpellCastingEvent.Post event = new SpellCastingEvent().new Post(parsedStack,
                    (ItemSpellBase) parsedStack.getItem(), caster, checkEvent.manaCost, checkEvent.burnout, false,
                    checkEvent.castResult);
        ...
        if (shape != null) {
            ...
            if (cloaking > 0) {
                double radius = SpellUtils.instance
                    .getModifiedDouble_Add(10, stack, caster, target, world, 0, SpellModifiers.RADIUS);
                ...
                if (!world.isRemote) AMNetHandler.INSTANCE.sendPacketToAllClientsNear(
                    world.provider.dimensionId,
                    x,
                    y,
                    z,
                    radius,
                    AMPacketIDs.CLOAKING,
                    writer1.generate());
            }
        ...
        if (result == SpellCastResult.SUCCESS) {
            ...
            if (world.isRemote) {
                String sfx = shape
                    .getSoundForAffinity(SpellUtils.instance.mainAffinityFor(parsedStack), parsedStack, null);
                if (sfx != null) {
        ...
        SpellCastingEvent.Post event = new SpellCastingEvent().new Post(parsedStack,
            (ItemSpellBase) parsedStack.getItem(), caster, manaCost, burnout, false, result);
        MinecraftForge.EVENT_BUS.post(event);

        return result;
    }

    public SpellCastResult applyStackStageOnUsing(ItemStack stack, EntityLivingBase caster, EntityLivingBase target,
        double x, double y, double z, World world, boolean consumeMBR, boolean giveXP, int ticks) {
    ...
    public boolean attackTargetSpecial(ItemStack spellStack, Entity target, DamageSource damagesource,
        float magnitude) {

        if (target.worldObj.isRemote) return true;
        ...
        if (damagesource.getSourceOfDamage() != null) {
            if (damagesource.getSourceOfDamage() instanceof EntityLivingBase) {
                ...
                if ((source instanceof EntityLightMage || source instanceof EntityDarkMage)
                    && target.getClass() == EntityCreeper.class) {
                    return false;
                    ...
                    return false;
                } else if (source instanceof EntityPlayer && target instanceof EntityPlayer
                ...
                if (source.isPotionActive(BuffList.fury)) magnitude += 4;
            }
        ...
        if (target instanceof EntityDragon) {
            success = ((EntityDragon) target)
                .attackEntityFromPart(((EntityDragon) target).dragonPartHead, damagesource, magnitude);
        } else {
        ...
        if (dmgSrcPlayer != null) {
            if (spellStack != null && target instanceof EntityLivingBase) {
                if (!target.worldObj.isRemote && ((EntityLivingBase) target).getHealth() <= 0
                    && SpellUtils.instance.modifierIsPresent(SpellModifiers.DISMEMBERING_LEVEL, spellStack, 0)) {
                    double chance = SpellUtils.instance.getModifiedDouble_Add(
                        0,
                        spellStack,
                        dmgSrcPlayer,
                        target,
                        dmgSrcPlayer.worldObj,
                        0,
                        SpellModifiers.DISMEMBERING_LEVEL);
    ...
    public float modifyDamage(EntityLivingBase caster, float damage) {
        float factor = (float) (ExtendedProperties.For(caster)
            .getMagicLevel() < 20 ? 0.5
                + (0.5 * ExtendedProperties.For(caster)
                    .getMagicLevel() / 19)
                : 1.0 + (1.0 * (ExtendedProperties.For(caster)
                    .getMagicLevel() - 20) / 79));
    ...
    public static class LingeringSpell {
        ...
        private LingeringSpell(int amountOfModifiers, ItemStack stack, World world, EntityLivingBase caster,
            int timeToNextCast) {
        ...
        public LingeringSpell(int amountOfModifiers, ItemStack stack, World world, EntityLivingBase caster,
            Entity target, int timeToNextCast) {
        ...
        public LingeringSpell(int amountOfModifiers, ItemStack stack, World world, EntityLivingBase caster,
            Entity target, int timeToNextCast, EntitySpellEffect zoneEntity) {
        ...
        // lingers on a block
        public LingeringSpell(int amountOfModifiers, ItemStack stack, World world, EntityLivingBase caster, int blockX,
            int blockY, int blockZ, int blockFace, double impactX, double impactY, double impactZ, int timeToNextCast) {
            this(amountOfModifiers, stack, world, caster, timeToNextCast);
            this.blockX = blockX;
            this.blockY = blockY;
            this.blockZ = blockZ;
        ...
        public boolean doUpdate() {
            if (this.zoneSpell) {
                if (this.zoneEntity != null) {
                    if (this.target != null)
                        this.zoneEntity.setPosition(this.target.posX, this.target.posY, this.target.posZ);
