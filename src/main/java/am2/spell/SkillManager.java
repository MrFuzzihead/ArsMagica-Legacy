...
public class SkillManager implements ISpellPartManager {
    ...
    public ArrayList<Integer> getAllTalents() {
        ArrayList<Integer> modifiers = new ArrayList<Integer>();
        for (Integer i : registeredParts.keySet()) {
            if (!(registeredParts.get(i) instanceof ISpellShape) && !(registeredParts.get(i) instanceof ISpellComponent)
                && !(registeredParts.get(i) instanceof ISpellModifier)) {
                modifiers.add(i);
            }
        }
        return modifiers;
    ...
    public int registerSkillTreeEntry(ISkillTreeEntry part, String name) {
        ...
        if (part instanceof ISpellComponent) id += COMPONENT_OFFSET;
        else if (part instanceof ISpellModifier) id += MODIFIER_OFFSET;
        else if (!(part instanceof ISpellShape)) id += TALENT_OFFSET;

        if (registeredParts.containsKey(id)) {
            String existing = reversedPartNames.get(id);
            LogHelper.info(
                "Attempted to register duplicate spell part name, %s (which would overwrite %s).  The part was NOT registered.",
                name,
                existing);
            return -1;
        }
        ...
        if (part instanceof ISpellPart) SpellRecipeManager.instance.RegisterRecipe((ISpellPart) part);
    ...
    public String getSkillName(ISkillTreeEntry component) {
        ...
        if (component instanceof ISpellComponent) id += COMPONENT_OFFSET;
        else if (component instanceof ISpellModifier) id += MODIFIER_OFFSET;
        else if (!(component instanceof ISpellShape)) id += TALENT_OFFSET;
    ...
    public int getSpellPartID(Class clazz) {
        for (Integer id : registeredParts.keySet()) {
            ...
            if (clazz.isAssignableFrom(entry.getClass())) return id;
        }
