...
public class Lunar implements ISpellModifier {
    ...
    public Object[] getRecipeItems() {
        return new Object[] { new ItemStack(ItemsCommonProxy.essence, 1, ItemsCommonProxy.essence.META_NATURE),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_MOONSTONE),
            new ItemStack(ItemsCommonProxy.itemOre, 1, ItemsCommonProxy.itemOre.META_IMBUEDMOONFLOWER), Items.clock };
    }
    ...
    public float getManaCostMultiplier(ItemStack spellStack, int stage, int quantity, EntityLivingBase caster) {
        ...
        if (caster.dimension == 1) multiplier = 2.0f;
            ...
            multiplier = (float) Math.round((1.5 + Math.exp(0.13 * (Math.abs((time - 18000) / 1000)))) * 100) / 100;
        }
