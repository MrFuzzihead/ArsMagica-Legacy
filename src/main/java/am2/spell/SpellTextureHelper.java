...
public class SpellTextureHelper {
    ...
    public void loadAllIcons(IIconRegister register) {
        ...
        try {
            ...
            if (icons == null) icons = new IIcon[0];
            e.printStackTrace();
        }
    ...
    public static List<String> getResourceListing() throws IOException, URISyntaxException {
        CodeSource src = AMCore.class.getProtectionDomain()
            .getCodeSource();
        ...
        if (src != null) {
            ...
            if (jar.getProtocol()
                ...
                while (entries.hasMoreElements()) {
                    ...
                    if (entry.getName()
                        .startsWith("assets/arsmagica2/textures/items/spells/icons/")) {
                        String name = entry.getName()
                            .replace("assets/arsmagica2/textures/items/spells/icons/", "");
                        if (name.isEmpty()) continue;
                        toReturn.add("spells/icons/" + name.replace(".png", ""));
                    }
                }
                jarFile.close();
            } else if (jar.getProtocol()
                .equals("file")) {
                    String path = (Util.getOSType() == Util.EnumOS.LINUX ? "/" : "") + jar.toURI()
                        .toString()
    ...
    public IIcon getIcon(int index) {
        if (icons.length == 0) return Items.diamond_sword.getIconFromDamage(0);
