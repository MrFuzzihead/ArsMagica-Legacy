...
public class SpellValidator {
    ...
    public ValidationResult spellDefIsValid(ArrayList<ArrayList<ISpellPart>> shapeGroups,
        ArrayList<ArrayList<ISpellPart>> segmented) {
        boolean noParts = true;
        for (ArrayList<ISpellPart> shapeGroup : shapeGroups) if (!shapeGroup.isEmpty()) {
            noParts = false;
            break;
        }
        for (ArrayList<ISpellPart> iSpellParts : segmented) if (!iSpellParts.isEmpty()) {
            noParts = false;
            break;
        }
        ...
        for (int i = 0; i < segmented.size(); ++i) {
            ...
            if (result == StageValidations.NOT_VALID) {
                return new ValidationResult(
                    segmented.get(i)
                        .get(0),
                    StatCollector.translateToLocal("am2.spell.validate.compMiss"));
                ...
                return new ValidationResult(
                    ...
                    String.format(
                        "%s %s",
                        SkillManager.instance.getDisplayName(
                            segmented.get(i)
                                .get(0)),
                        StatCollector.translateToLocal("am2.spell.validate.principum")));
                ...
                return new ValidationResult(
                    ...
                    String.format(
                        ...
                        SkillManager.instance.getDisplayName(
                            segmented.get(i)
                                .get(0)),
                        StatCollector.translateToLocal("am2.spell.validate.terminus")));
            }
        }
    ...
    private StageValidations validateStage(ArrayList<ISpellPart> stageDefinition, boolean isFinalStage) {
        ...
        for (ISpellPart part : stageDefinition) {
            ...
            if (part instanceof ISpellShape) {
                one_shape = true;
                if (((ISpellShape) part).isTerminusShape()) terminus = true;
                if (((ISpellShape) part).isPrincipumShape()) principum = true;
                continue;
            }
        ...
        if (principum) return StageValidations.PRINCIPUM;
        if (!one_component || !one_shape) return StageValidations.NOT_VALID;
        if (terminus) return StageValidations.TERMINUS;
    ...
    public static ArrayList<ArrayList<ISpellPart>> splitToStages(ArrayList<ISpellPart> currentRecipe) {
        ...
        for (ISpellPart part : currentRecipe) {
            if (part instanceof ISpellShape) idx++;
