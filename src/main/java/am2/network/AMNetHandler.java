...
public class AMNetHandler {
    ...
    public void sendVelocityAddPacket(World world, EntityLivingBase target, double velX, double velY, double velZ) {
        ...
        byte[] data = new AMDataWriter().add(target.getEntityId())
            .add(velX)
            .add(velY)
            .add(velZ)
            .generate();
        ...
        sendPacketToAllClientsNear(
            ...
            target.posZ,
            50,
            AMPacketIDs.PLAYER_VELOCITY_ADD,
            data);
    }
    ...
    public void syncWorldName(EntityPlayerMP player, String name) {
        sendPacketToClientPlayer(
            (EntityPlayerMP) player,
            AMPacketIDs.SYNC_WORLD_NAME,
            new AMDataWriter().add(name)
                .generate());
    }
    ...
    public void sendShapeGroupChangePacket(int newCastingMode, int entityid) {
        byte[] packetData = new AMDataWriter().add(newCastingMode)
            .add(entityid)
            .generate();
    ...
    public <T extends EntityLivingBase & IArsMagicaBoss> void sendActionUpdateToAllAround(T boss) {
        if (boss.worldObj != null && !boss.worldObj.isRemote) {
            ...
            writer.add(
                boss.getCurrentAction()
                    .ordinal());

            sendPacketToAllClientsNear(
                boss.worldObj.provider.dimensionId,
                boss.posX,
                boss.posY,
                boss.posZ,
                64,
                AMPacketIDs.ENTITY_ACTION_UPDATE,
                writer.generate());
        }
    }

    public void sendStarImpactToClients(double x, double y, double z, World world, ItemStack spellStack) {
        AMDataWriter writer = new AMDataWriter().add(x)
            .add(y)
            .add(z);
        if (spellStack != null) writer.add(true)
            .add(spellStack);
        else writer.add(false);
    ...
    public void sendSpellApplyEffectToAllAround(EntityLivingBase caster, Entity target, double x, double y, double z,
        ...
        sendPacketToAllClientsNear(
            ...
            z,
            64,
            AMPacketIDs.SPELL_APPLY_EFFECT,
            writer.generate());
    }

    public void sendHecateDeathToAllAround(EntityHecate hecate) {
        AMDataWriter writer = new AMDataWriter();
        writer.add(hecate.posX)
            .add(hecate.posY)
            .add(hecate.posZ);

        sendPacketToAllClientsNear(
            hecate.worldObj.provider.dimensionId,
            hecate.posX,
            hecate.posY,
            hecate.posZ,
            32,
            AMPacketIDs.HECATE_DEATH,
            writer.generate());
    }
    ...
    public void sendCompendiumUnlockPacket(EntityPlayerMP player, String id, boolean isCategory) {
        ...
        if (player.worldObj != null && player.isPotionActive(BuffList.psychedelic)) {
            if (player.worldObj.provider.dimensionId == 1 && player.getActivePotionEffect(BuffList.psychedelic)
                .getAmplifier() == 1) {
                if (player.ticksExisted > 5000 && (player.getRNG()
                    .nextInt(10) == 0))
        ...
        if (!AMCore.config.stagedCompendium()) return;
        AMDataWriter writer = new AMDataWriter();
        writer.add(id);
    ...
    public void sendCalefactorCookUpdate(TileEntityCalefactor calefactor, byte[] data) {
        ...
        writer.add(data);
        sendPacketToAllClientsNear(
            calefactor.getWorldObj().provider.dimensionId,
            calefactor.xCoord,
            calefactor.yCoord,
            calefactor.zCoord,
            32,
            AMPacketIDs.CALEFACTOR_DATA,
            writer.generate());
    }
    ...
    public void sendObeliskUpdate(TileEntityObelisk obelisk, byte[] data) {
        ...
        writer.add(data);
        sendPacketToAllClientsNear(
            obelisk.getWorldObj().provider.dimensionId,
            obelisk.xCoord,
            obelisk.yCoord,
            obelisk.zCoord,
            32,
            AMPacketIDs.OBELISK_DATA,
            writer.generate());
    }
