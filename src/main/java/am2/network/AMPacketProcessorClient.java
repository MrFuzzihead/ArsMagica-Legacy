...
public class AMPacketProcessorClient extends AMPacketProcessorServer {
    ...
    private void handleCompendiumSync(byte[] remaining) {
        ExtendedProperties.For(Minecraft.getMinecraft().thePlayer)
            .onSyncCompendiumDataPacket(remaining);
    }
    ...
    private void handleSyncWorldData(byte[] remaining) {
        ...
        for (int j = 0; j < data.getInteger("datasize"); j++) {
            CustomWorldData
                .setWorldVarNoSync(world, data.getString("dataentryname" + j), data.getString("dataentry" + j));
        }
    }
    ...
    private void handleCasterUpdate(byte[] remaining) {
        ...
        PowerNodeRegistry.For(Minecraft.getMinecraft().theWorld)
            .setPower(((TileEntityBlockCaster) te), PowerTypes.LIGHT, rdr.getFloat());
        PowerNodeRegistry.For(Minecraft.getMinecraft().theWorld)
            .setPower(((TileEntityBlockCaster) te), PowerTypes.NEUTRAL, rdr.getFloat());
        PowerNodeRegistry.For(Minecraft.getMinecraft().theWorld)
            .setPower(((TileEntityBlockCaster) te), PowerTypes.DARK, rdr.getFloat());
    }
    ...
    private void handleLecternData(byte[] data) {
        ...
        if (rdr.getBoolean()) ((TileEntityLectern) te).setStack(rdr.getItemStack());
        else((TileEntityLectern) te).setStack(null);
    }
    ...
    private void handleRcvPowerPaths(byte[] data) {
        ...
        if (bite == 0) {
            PowerNodeEntry pnd = PowerNodeRegistry.For(Minecraft.getMinecraft().theWorld)
                .parseFromNBT(compound);
            ...
            if (te instanceof IPowerNode) PowerNodeRegistry.For(Minecraft.getMinecraft().theWorld)
    ...
    private void handleSpellApplyEffect(byte[] data) {
        ...
        if (!(caster instanceof EntityLivingBase) || !(target instanceof EntityLivingBase)) return;
    ...
    private void handleStarFall(byte[] data) {
        ...
        if (rdr.getBoolean()) spellStack = rdr.getItemStack();
                ...
                for (ISpellModifier mod : mods) {
                    if (mod instanceof Colour) {
                        byte[] meta = SpellUtils.instance
                            .getModifierMetadataFromStack(spellStack, mod, 0, ordinalCount++);
        ...
        for (int i = 0; i < 360; i += AMCore.config.FullGFX() ? 5 : AMCore.config.LowGFX() ? 10 : 20) {
            AMParticle effect = (AMParticle) AMCore.proxy.particleManager
                .spawn(Minecraft.getMinecraft().theWorld, "sparkle2", x, y + 1.5, z);
            if (effect != null) {
                ...
                effect.AddParticleController(
                    new ParticleFadeOut(effect, 1, false).setFadeSpeed(0.05f)
    ...
    private void handleCompendiumUnlock(byte[] data) {
        ...
        if (category) ArcaneCompendium.instance.unlockCategory(s);
        else ArcaneCompendium.instance.unlockEntry(s);
    }
    ...
    private void handleSetMagicicansWorkbenchRecipe(EntityPlayer player) {
        if (player.openContainer instanceof ContainerMagiciansWorkbench) {
            ...
            player.openContainer
                .onCraftMatrixChanged(((ContainerMagiciansWorkbench) player.openContainer).getWorkbench());
        }
    }
        ...
        try {
            ...
            player.addChatMessage(
                new ChatComponentText(
                    "An error occurred while attempting to write the NBT, check the console for more information."));
            return;
        }
    ...
    private void openUICustomization() {
        Minecraft.getMinecraft()
            .displayGuiScreen(new GuiHudCustomization());
    }
    ...
    private void handleSyncSpellKnowledge(byte[] data) {
        ...
        if (!SkillData.For((EntityPlayer) ent)
    ...
    private void handleKeystoneGuiOpen(byte[] data) {
        ...
        FMLNetworkHandler.openGui(
            player,
            AMCore.instance,
            ArsMagicaGuiIdList.GUI_KEYSTONE_CHEST,
            Minecraft.getMinecraft().theWorld,
            x,
            y,
            z);
    ...
    private void handleSyncBetaParticles(byte[] data) {
        EntityPlayer localPlayer = Minecraft.getMinecraft().thePlayer;
        if (!PlayerTracker.hasAA(localPlayer)) return;
    ...
    private void handleSpellCast(byte[] data) {
        ...
        int casterID = rdr.getInt();
        int targetID = -1;
        if (rdr.getBoolean()) targetID = rdr.getInt();
        double x = rdr.getDouble();
        double y = rdr.getDouble();
        double z = rdr.getDouble();
        int side = rdr.getInt();
