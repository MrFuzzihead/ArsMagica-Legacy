...
//		this.drawTexturedModalRect(0, 0, 0, 0, Minecraft.getMinecraft().displayWidth / 2, Minecraft.getMinecraft().displayHeight / 2);
	...
	private void RenderManaBar(int i, int j, FontRenderer fontRenderer){
		...
		if (AMCore.config.showHudBars()){
			...
			DrawPartialIconAtXY(AMGuiIcons.fatigueLevel, progressScaled, 1, fatigue_hud.iX, fatigue_hud.iY + 3f, (float)barWidth, 40, false);
		...
		if (AMCore.config.getShowNumerics()){
			...
			if (curItem != null && (curItem.getItem() == ItemsCommonProxy.spell || curItem.getItem() == ItemsCommonProxy.spellBook || curItem.getItem() == ItemsCommonProxy.arcaneSpellbook))
				spellStack = curItem.getItem() == ItemsCommonProxy.spell ? curItem : ((ItemSpellBook)curItem.getItem()).GetActiveItemStack(curItem);
			...
			if(spellStack != null){
				SpellUtils.SpellRequirements spellRequirements = SpellUtils.instance.getSpellRequirements(spellStack, player);
		...
		for (PotionEffect pe : getPotionEffectsByTimeRemaining()){
			...
			if (potion.isBadEffect()){
				if (potion.hasStatusIcon()){
					...
					this.drawTexturedModalRect_Classic(negative_buff_x, negative_buff_y, l % 8 * 18, 198 + (float)l / 8 * 18, 10, 10, 18, 18);
					GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
					negative_buff_x -= 12;
				...
				if (potion.hasStatusIcon()){
					...
					this.drawTexturedModalRect_Classic(positive_buff_x, positive_buff_y, l % 8 * 18, 198 + (float)l / 8 * 18, 10, 10, 18, 18);
					GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
					positive_buff_x += 12;
