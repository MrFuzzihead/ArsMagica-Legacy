...
public class AMIngameGUI {
    ...
    private static final ResourceLocation inv_top = new ResourceLocation(
        "arsmagica2",
        ResourceManager.GetGuiTexturePath("Inventory_Top.png"));
    private static final ResourceLocation mc_gui = new ResourceLocation("textures/gui/icons.png");
    private static final ResourceLocation spellbook_ui = new ResourceLocation(
        "arsmagica2",
        ResourceManager.GetGuiTexturePath("spellbook_ui.png"));
    ...
    public static final ResourceLocation[] dmtfog = { new ResourceLocation("arsmagica2:textures/guis/dmtfog1.png"),
        new ResourceLocation("arsmagica2:textures/guis/dmtfog2.png"),
        new ResourceLocation("arsmagica2:textures/guis/dmtfog3.png"),
        new ResourceLocation("arsmagica2:textures/guis/dmtfog4.png"),
        new ResourceLocation("arsmagica2:textures/guis/dmtfog5.png") };
    public static final ResourceLocation[] ayafractal = {
        new ResourceLocation("arsmagica2:textures/guis/ayafractal1.png"),
        new ResourceLocation("arsmagica2:textures/guis/ayafractal2.png"),
        new ResourceLocation("arsmagica2:textures/guis/ayafractal3.png"),
        new ResourceLocation("arsmagica2:textures/guis/ayafractal4.png"),
        new ResourceLocation("arsmagica2:textures/guis/ayafractal5.png"),
        new ResourceLocation("arsmagica2:textures/guis/ayafractal6.png"),
        new ResourceLocation("arsmagica2:textures/guis/ayafractal7.png") };
    ...
    public void renderGameOverlay() {
        ...
        boolean drawAMHud = !AMCore.config.showHudMinimally()
            || (ci != null && (ci.getItem() == ItemsCommonProxy.spellBook || ci.getItem() == ItemsCommonProxy.spell
    ...
    public void renderPsychedelicOverlay() {
        ...
        if (this.mc.thePlayer.getActivePotionEffect(BuffList.psychedelic)
            .getAmplifier() == 1) {
            GL11.glColor4d(
                1.0,
                1.0,
                1.0,
                (((float) psychoTick / 400) % 1) < 0.5f ? (((float) psychoTick / 400) % 1)
                    : 1 - (((float) psychoTick / 400) % 1));
            Minecraft.getMinecraft().renderEngine.bindTexture(ayafractal[(int) (double) (psychoTick / 400)]);
        } else {
            GL11.glColor4d(
                1.0,
                1.0,
                1.0,
                (((float) psychoTick / 560) % 1) < 0.5f ? (((float) psychoTick / 560) % 1)
                    : 1 - (((float) psychoTick / 560) % 1));
            Minecraft.getMinecraft().renderEngine.bindTexture(dmtfog[(int) (double) (psychoTick / 560)]);
        }
        ...
        // GL11.glColor4d(1.0, 1.0, 1.0, (((float)psychoTick / 100) % 1) < 0.5f ? (((float)psychoTick / 100) % 1) :
        // 1-(((float)psychoTick / 100) % 1));
        // Minecraft.getMinecraft().renderEngine.bindTexture(ayafractal[(int) Math.ceil((psychoTick+100 > 699 ?
        // psychoTick+100-699 : psychoTick+100) / 100)]);
        // } else {
        // GL11.glColor4d(1.0, 1.0, 1.0, (((float)psychoTick / 140) % 1) < 0.5f ? (((float)psychoTick / 140) % 1) :
        // 1-(((float)psychoTick / 140) % 1));
        // Minecraft.getMinecraft().renderEngine.bindTexture(dmtfog[(int) Math.ceil((psychoTick+140 > 699 ?
        // psychoTick+140-699 : psychoTick+140) / 140)]);
        // }
        // this.drawTexturedModalRect(0, 0, 0, 0, Minecraft.getMinecraft().displayWidth / 2,
        // Minecraft.getMinecraft().displayHeight / 2);

        /*
         * this.drawTexturedModalRect_Classic(0, 0, 0, 0, 1920, 1080, 640, 360); // alternative drawing thing to try w/o
         * scaling
         * GL11.glColor4d(1.0, 1.0, 1.0, 0.5);
         * if (this.mc.thePlayer.getActivePotionEffect(BuffList.psychedelic).getAmplifier() == 1) {
    ...
    private void RenderArsMagicaGUIItems(int i, int j, FontRenderer fontRenderer) {
        if (ExtendedProperties.For(mc.thePlayer)
            .getMagicLevel() > 0 || mc.thePlayer.capabilities.isCreativeMode) {
    ...
    private void RenderSpellBookUI(int i, int j, FontRenderer fontrenderer, ItemStack bookStack) {
        ...
        mc.renderEngine.bindTexture(spellbook_ui);
        this.zLevel = 1000;
        drawTexturedModalRect_Classic(x, (float) spellUI_y, 148, 0, activeSpellSize, activeSpellSize, 20, 20);
        this.zLevel = 0;
    ...
    private void RenderManaBar(int i, int j, FontRenderer fontRenderer) {
        ...
        if (AMCore.config.showHudBars()) {
            ...
            if (curItem != null
                && (curItem.getItem() == ItemsCommonProxy.spell || curItem.getItem() == ItemsCommonProxy.spellBook
                    || curItem.getItem() == ItemsCommonProxy.arcaneSpellbook)) {
                ItemStack spellStack = curItem.getItem() == ItemsCommonProxy.spell ? curItem
                ...
                if (spellStack != null) {
                    ...
                    for (int p : parts) {
                        IIcon icon = SpellIconManager.instance
                            .getIcon(SkillManager.instance.getSkillName(SkillManager.instance.getSkill(p)));
                        if (icon != null) {
                            DrawIconAtXY(icon, sx, sy);
                            sx += 3;
                            sy += 3;
                        }
                    }
            ...
            DrawIconAtXY(AMGuiIcons.fatigueIcon, fatigue_hud.iX + barWidth, fatigue_hud.iY);
            ...
            String magicLevel = String.valueOf(
                ExtendedProperties.For(mc.thePlayer)
                    .getMagicLevel());
        ...
        if (AMCore.config.getShowNumerics()) {
            ...
            if (curItem != null
                && (curItem.getItem() == ItemsCommonProxy.spell || curItem.getItem() == ItemsCommonProxy.spellBook
                    || curItem.getItem() == ItemsCommonProxy.arcaneSpellbook))
                spellStack = curItem.getItem() == ItemsCommonProxy.spell ? curItem
                    : ((ItemSpellBook) curItem.getItem()).GetActiveItemStack(curItem);

            String manaStr = StatCollector.translateToLocal("am2.gui.mana") + ": "
                + (int) (mana + bonusMana)
                + "/"
                + (int) maxMana;
            String burnoutStr = StatCollector.translateToLocal("am2.gui.burnout") + ": "
            ...
            if (spellStack != null) {
                SpellUtils.SpellRequirements spellRequirements = SpellUtils.instance
                    .getSpellRequirements(spellStack, player);
                float manacost = spellRequirements.manaCost;
                ManaCostEvent mce = new ManaCostEvent(spellStack, player, manacost, 0);
                MinecraftForge.EVENT_BUS.post(mce);
                manacost = mce.manaCost;
                String manacostStr = I18n.format("(%s)", (int) manacost);
                fontRenderer.drawString(manacostStr, manaNumericPos.iX + 100, manaNumericPos.iY, 0x1fb5d5);
            }
        ...
        if (props.redGlint > 0) {
            ...
            GL11.glEnable(GL11.GL_BLEND);
            GL11.glDepthMask(false);
            GL11.glColor4d(1.0, 1.0, 1.0, (double) props.redGlint / 300);
    ...
    private void RenderArmorStatus(int i, int j, Minecraft mc, FontRenderer fontRenderer) {
        ...
        for (int slot = 0; slot < 4; ++slot) {
            if (ArmorHelper.PlayerHasArmorInSlot(mc.thePlayer, 3 - slot)) {
                ...
                AMVector2 position = getArmorSlotPosition(
                    slot,
    ...
    public void RenderAffinity(int i, int j) {
        ...
        for (Affinity affinity : ad.getHighestAffinities()) {
            ...
            GL11.glColor3f(1.0f, 1.0f, 1.0f);
            AMGuiHelper
                .DrawIconAtXY(affinity.representItem.getIconFromDamage(affinity.representMeta), x, y, j, 12, 12, true);

            if (AMCore.config.getShowNumerics()) {
                String display = String.format(
                    "%.2f%%",
                    AffinityData.For(mc.thePlayer)
                        .getAffinityDepth(affinity) * 100);
                if (x < i / 2) Minecraft.getMinecraft().fontRenderer.drawString(display, x + 14, y + 2, affinity.color);
                else Minecraft.getMinecraft().fontRenderer.drawString(
                    display,
    ...
    public void RenderContingency(int i, int j) {
        ...
        for (int z = 0; z < 5; z++) {
            IIcon icon = null;
            ItemStack type = ExtendedProperties.For(Minecraft.getMinecraft().thePlayer)
                .getContingencyEffect(z);
            switch (z) {
                case 0:
    ...
    public void RenderMagicXP(int i, int j) {
        ...
        if (props.getMagicLevel() > 0) {
            ...
            if (AMCore.config.getShowNumerics()
                ...
                String xpStr = StatCollector.translateToLocal("am2.gui.xp") + ": "
    ...
    private ArrayList<PotionEffect> getPotionEffectsByTimeRemaining() {
        Iterator i = mc.thePlayer.getActivePotionEffects()
            .iterator();
        ArrayList<PotionEffect> potions = new ArrayList<PotionEffect>();

        while (i.hasNext()) potions.add((PotionEffect) i.next());

        potions.sort(durationComparator);
        return potions;
    }
     ...
     */
    public void drawTexturedModalRect_Classic(float dst_x, float dst_y, float src_x, float src_y, float dst_width,
        float dst_height, float src_width, float src_height) {
        float var7 = 0.00390625F;
        float var8 = 0.00390625F;
    ...
    private void DrawIconAtXY(IIcon icon, float x, float y) {
        DrawIconAtXY(icon, "items", x, y, 16, 16, false);
    }
    ...
    private void DrawPartialIconAtXY(IIcon IIcon, float pct_x, float pct_y, float x, float y, float w, float h,
    ...
    static class PotionEffectDurationComparator implements Comparator<PotionEffect> {
