...
public class GuiCompendiumIndex extends GuiScreen implements GuiYesNoCallback {
    ...
    private static final ResourceLocation background = new ResourceLocation(
        "arsmagica2",
        "textures/guis/ArcaneCompendiumIndexGui.png");

    public GuiCompendiumIndex() {
        categories = ArcaneCompendium.instance.getCategories();
        activeCategory = categories.iterator()
            .next()
            .getCategoryLabel();
        activeCategoryID = categories.iterator()
            .next()
            .getCategoryName();
    ...
    private void storeBreadcrumb() {
        AMGuiHelper.instance.pushCompendiumBreadcrumb(
            "",
            this.page,
            CompendiumBreadcrumb.TYPE_INDEX,
            activeCategory,
            activeCategoryID,
            currentParentEntryName,
            pagingData);
    }
    ...
    protected void actionPerformed(GuiButton buttonClicked) {
        ...
        if (buttonClicked.id == backToIndex.id) {
            ...
            Minecraft.getMinecraft()
                .displayGuiScreen(new GuiCompendiumIndex(crumb));
            return;
        }
        ...
        if (buttonClicked instanceof GuiButtonCompendiumTab) {
            ...
            if (((GuiButtonCompendiumLink) buttonClicked).hasSubItems()) {
                ...
                CompendiumEntry entry = ArcaneCompendium.instance
                    .getEntry(((GuiButtonCompendiumLink) buttonClicked).getEntryID());
                if (entry != null) {
                    storeBreadcrumb();
                    GuiArcaneCompendium gui = entry
                        .getCompendiumGui(((GuiButtonCompendiumLink) buttonClicked).getEntryID());
                    if (gui != null) {
                        Minecraft.getMinecraft()
    ...
    private void switchCategoryAndPage() {
        if (currentParentEntry != null) {
            lines = GuiArcaneCompendium
                .splitStringToLines(fontRendererObj, currentParentEntry.getDescription(), lineWidth, maxLines);
        ...
        for (Object button : this.buttonList) {
            ...
            if (button instanceof GuiButtonCompendiumLink) {
                ((GuiButtonCompendiumLink) button).visible = ((GuiButtonCompendiumLink) button).getCategory()
                    ...
                    && (((GuiButtonCompendiumLink) button).getPageNum() == this.page
                        || ((GuiButtonCompendiumLink) button).getDisplayOnAllPages());
            }
        }
    }
    ...
    private void setCategoryHasNewItems(String category, boolean hasNew) {
        for (Object button : this.buttonList) {
            if (button instanceof GuiButtonCompendiumTab
                && ((GuiButtonCompendiumTab) button).categoryID.equals(category)) {
    ...
    private int initTabs(int idCount) {
        ...
        for (CompendiumEntryType category : this.categories) {
            ...
            ArrayList<CompendiumEntry> itemsInCategory = ArcaneCompendium.instance
                .getEntriesForCategory(category.getCategoryName());
            for (CompendiumEntry entry : itemsInCategory) if (!entry.isLocked()) unlockedSubItems++;

            if (unlockedSubItems == 0) continue;

            GuiButtonCompendiumTab tab = new GuiButtonCompendiumTab(
                idCount++,
                tabX,
                tabY,
                category.getCategoryLabel(),
                category.getCategoryName(),
                category.getRepresentItem());
            if (tabWidth < tab.getWidth()) tabWidth = tab.getWidth();
            tabY += 18;
            tabX--;
    ...
    private int initButtonsForCategory(String category, int idCount, boolean display) {
        ...
        for (CompendiumEntry entry : itemsInCategory) {
            ...
            GuiButtonCompendiumLink link = new GuiButtonCompendiumLink(
                idCount++,
                buttonX,
                buttonY,
                fontRendererObj,
    ...
    private int initSubItemButtonsForEntry(CompendiumEntry entry, int idCount, String category) {
        ...
        boolean entryHasDescription = !entry.getDescription()
            .isEmpty();
        ...
        for (CompendiumEntry subItem : entry.getSubItems()) {
            ...
            GuiButtonCompendiumLink link = new GuiButtonCompendiumLink(
                ...
                subItem.hasSubItems(),
                numPages);
            if (entryHasDescription && entry.getSubItems().length < maxLines) link.setShowOnAllPages();
            link.visible = false;
            buttonY += 12;
            if (buttonY > i1 + (ySize) - 25) {
                if (buttonX > l + 40) {
    ...
    private boolean checkSCMLimit(CompendiumEntry entry) {
        if (entry instanceof CompendiumEntrySpellComponent || entry instanceof CompendiumEntrySpellModifier
            || entry instanceof CompendiumEntrySpellShape) {
            ...
            return sk.isEntryKnown(ste)
                || sk.getLearnState(ste, Minecraft.getMinecraft().thePlayer) == LearnStates.CAN_LEARN;
        }
        return true;
    }
