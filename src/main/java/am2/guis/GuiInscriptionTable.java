...
public class GuiInscriptionTable extends GuiContainer {
    ...
    private static final ResourceLocation background = new ResourceLocation(
        "arsmagica2",
        ResourceManager.GetGuiTexturePath("inscriptionTableGui.png"));
    ...
    public GuiInscriptionTable(InventoryPlayer playerInventory, TileEntityInscriptionTable table) {
        ...
        knownShapes = SkillData.For(this.usingPlayer)
            .getKnownShapes();
        knownComponents = SkillData.For(this.usingPlayer)
            .getKnownComponents();
        knownModifiers = SkillData.For(this.usingPlayer)
            .getKnownModifiers();
    }
    ...
    public void initGui() {
        ...
        searchBar = new GuiTextField(
            ...
            searchFieldPosition.iX,
            searchFieldPosition.iY,
            searchFieldDimensions.iX,
            searchFieldDimensions.iY);

        nameFieldPosition = new AMVector2(39, 93);
        nameFieldDimensions = new AMVector2(141, 12);
        nameBar = new GuiTextField(
            Minecraft.getMinecraft().fontRenderer,
            nameFieldPosition.iX,
            nameFieldPosition.iY,
            nameFieldDimensions.iX,
            nameFieldDimensions.iY);

        int l = (width - xSize) / 2;
        int i1 = (height - ySize) / 2;

        createSpellButton = new GuiButtonVariableDims(
            0,
            l - 65,
            i1,
            StatCollector.translateToLocal("am2.gui.makeSpell"));
        createSpellButton.setDimensions(60, 20);

        resetSpellButton = new GuiButtonVariableDims(
            1,
            l + 120,
            i1 + 72,
            StatCollector.translateToLocal("am2.gui.resetSpell"));
    ...
    protected void mouseClicked(int par1, int par2, int par3) {
        ...
        if (hoveredItem != null && hoveredIcon != null) {
            if (spellPartIsValidAddition(hoveredItem) && !lowerHover) {
                ...
                if (lowerHoverShapeGroup == -1
                    && ((ContainerInscriptionTable) this.inventorySlots).currentRecipeContains(hoveredItem)) {
                    if (hoveredItem instanceof ISpellShape) {
                        int index = lowerHoverIndex;
                        int startIndex = index;
                        int count = 0;
                        index++;
                        while (index < ((ContainerInscriptionTable) this.inventorySlots).getCurrentRecipeSize()
                            && !(((ContainerInscriptionTable) this.inventorySlots)
                                .getRecipeItemAt(index) instanceof ISpellShape)) {
                            count++;
                            index++;
                        }
                    ...
                    if (hoveredItem instanceof ISpellShape) {
                        ...
                        index++;
                        while (index
                            < ((ContainerInscriptionTable) this.inventorySlots).getShapeGroupSize(lowerHoverShapeGroup)
                            && !(((ContainerInscriptionTable) this.inventorySlots)
                                .getShapeGroupPartAt(lowerHoverShapeGroup, index) instanceof ISpellShape)) {
                            count++;
                            index++;
                        }
                        ((ContainerInscriptionTable) this.inventorySlots)
                            .removeMultipleRecipePartsFromGroup(lowerHoverShapeGroup, startIndex, count);
                    } else {
                        ((ContainerInscriptionTable) this.inventorySlots)
                            .removeSingleRecipePartFromGroup(lowerHoverShapeGroup, lowerHoverIndex);
                    }
                    result = ((ContainerInscriptionTable) this.inventorySlots).validateCurrentDefinition();
                }
            }
            ...
            if (par1 >= searchFieldPosition.iX && par1 <= searchFieldPosition.iX + searchFieldDimensions.iX) {
                if (par2 >= searchFieldPosition.iY && par2 <= searchFieldPosition.iY + searchFieldDimensions.iY) {
                    if (par3 == 1 || searchBar.getText()
                        .equals(defaultSearchLabel)) {
                        searchBar.setText("");
                    }
                    if (nameBar.getText()
                        .equals("")) {
            ...
            if (par1 >= nameFieldPosition.iX && par1 <= nameFieldPosition.iX + nameFieldDimensions.iX) {
                if (par2 >= nameFieldPosition.iY && par2 <= nameFieldPosition.iY + nameFieldDimensions.iY) {
                    if (par3 == 1 || nameBar.getText()
                        .equals(defaultNameLabel)) {
                        nameBar.setText("");
                ...
                for (int i = 0; i < sg; ++i) {
                    ...
                    if (x >= SGX && x <= SGX + shapeGroupWidth) {
                        if (y >= SGY && y <= SGY + shapeGroupWidth) {
                            ((ContainerInscriptionTable) this.inventorySlots)
                                .addRecipePartToGroup(i, (ISpellPart) hoveredItem);
                            result = ((ContainerInscriptionTable) this.inventorySlots).validateCurrentDefinition();
                        }
                    }
                }
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
        ...
        for (int sg = 0; sg < TileEntityInscriptionTable.MAX_STAGE_GROUPS; ++sg) {
            ...
            drawTexturedModalRect(
                offsetX + (sg * (shapeGroupWidth + shapeGroupPadding)),
                i1 + shapeGroupY,
                176,
                165,
                37,
        ...
        if (result.valid) {
            if (((ContainerInscriptionTable) this.inventorySlots).slotHasStack(0)) {
                if (((ContainerInscriptionTable) this.inventorySlots).slotIsBook(0)) {
                    Minecraft.getMinecraft().fontRenderer
                        .drawSplitString(StatCollector.translateToLocal("am2.gui.bookOut"), 225, 5, 100, 0xFF7700);
                } else {
            ...
            if (((ContainerInscriptionTable) this.inventorySlots).slotHasStack(0)
                && !((ContainerInscriptionTable) this.inventorySlots).slotIsBook(0)) {
                resetSpellButton.visible = true;
            } else {
                resetSpellButton.visible = false;
            }
    ...
    private void drawBookIcon() {
        ...
        if (AMGuiHelper.instance.getFastTicker() < 20) GL11.glColor4f(1.0f, 1.0f, 1.0f, 0.4f);
        else GL11.glColor4f(1.0f, 1.0f, 1.0f, 0.7f);
    ...
    private boolean drawCurrentRecipe(ArrayList<String> labelText, int l, int i1) {
        ...
        for (int i = 0; i < ((ContainerInscriptionTable) this.inventorySlots).getCurrentRecipeSize(); ++i) {
            ...
            // missingShape, missingComponent, missingModifier check. Temporary solution until someone figures out a
            // good API.
    ...
    private boolean drawIconSet(Integer[] ids, ArrayList<String> labelText) {
        ...
        for (Integer i : ids) {
            ...
            // skip if part is null (how?), !if it's missingShape, missingComponent, missingModifier!, or disabled in
            // config
            if (part == null || SkillManager.instance.getSkillName(part) == null
                || SkillTreeManager.instance.isSkillDisabled(part)) {
                continue;
            }
            String name = SkillManager.instance.getDisplayName(part);
            ...
            String filterText = searchBar.getText()
                .toLowerCase();
            if (filterText != "" && !filterText.equals(defaultSearchLabel.toLowerCase())
                && !name.toLowerCase()
                    .contains(filterText)) {
                continue;
            }
    ...
    private boolean spellPartIsValidAddition(ISkillTreeEntry part) {
        ...
        if (!hasShape && !(part instanceof ISpellShape)) return false;
        if (part instanceof ISpellShape
            && ((ContainerInscriptionTable) this.inventorySlots).currentRecipeContains(part)) return false;
        if (part instanceof ISpellComponent) {
            int index = ((ContainerInscriptionTable) this.inventorySlots).getCurrentRecipeSize() - 1;
            while (index >= 0
                && !(((ContainerInscriptionTable) this.inventorySlots).getRecipeItemAt(index) instanceof ISpellShape)) {
    ...
    private boolean drawIcon(ISkillTreeEntry part, boolean allowDarken) {
        ...
        if (shapeIcon == null) return false;
