...
public class GuiArmorImbuer extends GuiContainer {
    ...
    private static final ResourceLocation foreground = new ResourceLocation(
        "arsmagica2",
        ResourceManager.GetGuiTexturePath("ArmorUpgradeGUI.png"));
    private static final ResourceLocation background = new ResourceLocation(
        "arsmagica2",
        ResourceManager.GetGuiTexturePath("ArmorUpgradeGUIIcons.png"));
    private static final ResourceLocation background_bw = new ResourceLocation(
        "arsmagica2",
        ResourceManager.GetGuiTexturePath("ArmorUpgradeGUIIconsBW.png"));
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
            ...
            for (ImbuementTiers tier : ImbuementTiers.values()) {
                ...
                for (IArmorImbuement infusion : existingInfusions) {
                    ...
                    if (infusion.getTier() == tier) {
                        ...
                        if (tier.ordinal() >= highestSelectedTier) highestSelectedTier = tier.ordinal() + 1;
                        break;
                    }
                }
                for (IArmorImbuement infusion : infusions) {
                    if ((tierInfusion == null && infusion.getTier()
                        .ordinal() <= highestSelectedTier)) {
        ...
        if (!hoverLines.isEmpty()) AMGuiHelper.drawHoveringText(hoverLines, i, j, fontRendererObj, width, height);
    }
    ...
    private void drawInfusionIconAt(int x, int y, int index) {
        drawTexturedModalRect(
            ...
            y,
            (index % 5) * spriteWidth,
            (int) (double) (index / 5) * spriteHeight,
            spriteWidth,
            spriteHeight);
    }
