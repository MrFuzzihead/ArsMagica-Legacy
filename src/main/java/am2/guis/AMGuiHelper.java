...
public class AMGuiHelper {
    ...
    public void flashManaBar() {
        if (flashTimers[4] <= 1) flashTimers[4] = flashDuration;
    }
    ...
    public void tick() {
        ...
        for (int i = 0; i < this.flashTimers.length; ++i) {
            if (this.flashTimers[i] > 0) this.flashTimers[i]--;
        }
        ...
        if (magicXPBarShowTimer > 0) {
            magicXPBarShowTimer--;
            if (magicXPBarShowTimer < 20) magicXPBarAlpha -= 0.05f;
        }
    ...
    public CompendiumBreadcrumb popCompendiumBreadcrumb() {
        if (!compendiumBreadcrumbs.isEmpty()) return compendiumBreadcrumbs.pollLast();
        return null;
    }
        ...
        if (breadcrumb != null) {
            if (breadcrumb.entryType == CompendiumBreadcrumb.TYPE_ENTRY) {
                Minecraft.getMinecraft()
                    .displayGuiScreen(new GuiArcaneCompendium(breadcrumb));
            } else {
                Minecraft.getMinecraft()
                    .displayGuiScreen(new GuiCompendiumIndex(breadcrumb));
            }
        } else {
            Minecraft.getMinecraft()
    ...
    public static void DrawIconAtXY(IIcon IIcon, float x, float y, float zLevel, int w, int h,
    ...
    public static void DrawItemAtXY(ItemStack stack, float x, float y, float zLevel, float scale) {
        ...
        if (scale != 1.0f) {
            ...
            success = ForgeHooksClient.renderInventoryItem(
                renderBlocks,
            ...
            success = ForgeHooksClient
                .renderInventoryItem(renderBlocks, Minecraft.getMinecraft().renderEngine, stack, true, zLevel, x, y);
    ...
    public static void flipView(float f) {
        float flip = ExtendedProperties.For(Minecraft.getMinecraft().thePlayer)
            .getFlipRotation();
        float lastFlip = ExtendedProperties.For(Minecraft.getMinecraft().thePlayer)
            .getPrevFlipRotation();
    ...
    public static void shiftView(float f) {
        ...
        float flip = ExtendedProperties.For(entity)
            .getFlipRotation();
        float prevFlip = ExtendedProperties.For(entity)
            .getPrevFlipRotation();
        if (flip > 0) {
    ...
    public static boolean overrideMouseInput(EntityRenderer renderer, float f, boolean b) {
        ...
        if (!mc.inGameHasFocus || mc.thePlayer == null || mc.theWorld == null) return true;
        ...
        if (mc.gameSettings.smoothCamera) {
            ...
            ReflectionHelper.setPrivateValue(
                ...
                renderer,
                (Float) ReflectionHelper.getPrivateValue(EntityRenderer.class, renderer, scy) - f3,
                scy);
            // renderer.smoothCamPitch += f4;
            ReflectionHelper.setPrivateValue(
                EntityRenderer.class,
                renderer,
                (Float) ReflectionHelper.getPrivateValue(EntityRenderer.class, renderer, scp) - f4,
                scp);
