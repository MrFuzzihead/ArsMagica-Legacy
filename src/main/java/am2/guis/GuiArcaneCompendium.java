...
public class GuiArcaneCompendium extends GuiScreen {
    ...
    private static final ResourceLocation background = new ResourceLocation(
        "arsmagica2",
        "textures/guis/ArcaneCompendiumGui.png");
    private static final ResourceLocation extras = new ResourceLocation(
        "arsmagica2",
        "textures/guis/ArcaneCompendiumGuiExtras.png");
    ...
    public GuiArcaneCompendium(MultiblockStructureDefinition multi, TileEntity controllingTileEntity) {
        this(
            multi.getID()
                .equals("fakeAltar") ? "craftingAltar" : multi.getID());
    ...
    private void setupRitualPage() {
        ...
        temp[temp.length - 1] = new ItemStack(
            ItemsCommonProxy.spell_component,
            1,
            SkillManager.instance.getShiftedPartID((ISkillTreeEntry) ritualController));
        relatedEntries = temp;
    }
    ...
    private void getAndAnalyzeRecipe() {
        ...
        if (renderStack.getItem() == ItemsCommonProxy.essence
            || renderStack.getItem() == ItemsCommonProxy.deficitCrystal) {
            RecipeArsMagica essenceRecipe = RecipesEssenceRefiner.essenceRefinement()
                .recipeFor(renderStack);
            ...
            if (part instanceof ISpellPart) {
                ...
                SpellRecipeItemsEvent event = new SpellRecipeItemsEvent(
                    SkillManager.instance.getSkillName(part),
                    SkillManager.instance.getShiftedPartID(part),
                    recipeItems);
                        ...
                        if (o instanceof ItemStack) {
                            ...
                            if (((String) o).startsWith("P:")) {
                                ...
                                try {
                                    ...
                                    recipe.add(
                                        new ItemStack(ItemsCommonProxy.essence, amount, ItemEssence.META_MAX + type));
                                } catch (Throwable t) {
                                    continue;
                                }
                            } else {
            ...
            if (recipe != null) {
                ...
                if (recipe instanceof ShapedRecipes) {
                    ...
                    recipeWidth = ReflectionHelper
                        .getPrivateValue(ShapedOreRecipe.class, ((ShapedOreRecipe) recipe), "width");

                    recipeHeight = ReflectionHelper
                        .getPrivateValue(ShapedOreRecipe.class, ((ShapedOreRecipe) recipe), "height");
                    ...
                    craftingComponents = ((ShapelessOreRecipe) recipe).getInput()
                        .toArray();
                } else {
                    craftingComponents = null;
                }
    ...
    public GuiArcaneCompendium(String entryName) {
        ...
        if (entry != null) {
            lines = splitStringToLines(
                Minecraft.getMinecraft().fontRenderer,
                entry.getDescription(entryName),
                lineWidth,
                maxLines);
        ...
        for (int i = 0; i < childEntries.length; ++i) {
            if (childEntries[i].getID()
                .indexOf('@') == -1) {
                name = childEntries[i].getID();
            } else {
                String[] split = childEntries[i].getID()
                    .split("@");
                name = split[0];
                try {
        ...
        if (modifiedBy == null) modifiedBy = new SpellModifiers[0];
                    ...
                    for (SpellModifiers mod : modifiedBy) {
                        if (entry instanceof CompendiumEntrySpellModifier) {
                            if (modifier.getAspectsModified()
                                .size() == 1 && modifier.getAspectsModified()
                                    .contains(mod)) {
    ...
    public void initGui() {
        ...
        backToIndex = new GuiButtonCompendiumTab(
            2,
            l - 10,
            i1 + 20,
            StatCollector.translateToLocal("am2.gui.back"),
            "back",
            null);
        ...
        pauseCycling = new GuiButtonVariableDims(
            5,
            l + 285,
            i1 + 190,
            AMGuiHelper.instance.runCompendiumTicker ? StatCollector.translateToLocal("am2.gui.pause")
                : StatCollector.translateToLocal("am2.gui.cycle")).setDimensions(40, 20);
    ...
    protected void mouseClicked(int par1, int par2, int par3) {
        if (stackTip != null) {
            ...
            if (stackTip.getItem() instanceof ItemBlock) {
                ...
                String name = SkillManager.instance
                    .getSkillName(SkillManager.instance.getSkill(stackTip.getItemDamage()));
            ...
            if (newGuiToDisplay != null) {
                ...
                Minecraft.getMinecraft()
                    .displayGuiScreen(newGuiToDisplay);
            }
            return;
    ...
    private void storeBreadcrumb() {
        if (this.ritualController != null) {
            AMGuiHelper.instance.pushCompendiumBreadcrumb(
                this.entryName,
                this.page,
                CompendiumBreadcrumb.TYPE_ENTRY,
                this.entryMultiblock,
                this.blockAccess.getTileEntity(0, 0, 0),
                this.ritualController);
            ...
            AMGuiHelper.instance
                .pushCompendiumBreadcrumb(this.entryName, this.page, CompendiumBreadcrumb.TYPE_ENTRY, this.entryEntity);
    ...
    protected void actionPerformed(GuiButton par1GuiButton) {
        switch (par1GuiButton.id) {
            ...
            case 2:
                ...
                if (prevEntry != null) {
                    if (prevEntry.entryType == CompendiumBreadcrumb.TYPE_ENTRY) Minecraft.getMinecraft()
                        .displayGuiScreen(new GuiArcaneCompendium(prevEntry));
                    else Minecraft.getMinecraft()
                        .displayGuiScreen(new GuiCompendiumIndex(prevEntry));
                } else {
                    Minecraft.getMinecraft()
                        .displayGuiScreen(new GuiCompendiumIndex());
                }
                break;
            case 3:
                curLayer--;
            ...
            case 5:
                ...
                pauseCycling.displayString = AMGuiHelper.instance.runCompendiumTicker
                    ? StatCollector.translateToLocal("am2.gui.pause")
                    : StatCollector.translateToLocal("am2.gui.cycle");
                break;
        }
    }
    ...
    private void drawLeftPage(int l, int i1) {
        ...
        if (entry != null) {
            if (page == 0) fontRendererObj.drawString(entry.getName(entryName), x_start_title, y_start_title, 0x000000);
            AMGuiHelper
                .drawCompendiumText(lines.get(page), x_start_line, y_start_line, lineWidth, 0x000000, fontRendererObj);
        }
    }
    ...
    private void drawRightPage(int l, int i1, int mousex, int mousey) {
        ...
        if (!modifiers.isEmpty()) drawModifiers(cx, entryMultiblock == null ? cy : cy + 20, mousex, mousey);
    }
    ...
    private void drawModifiers(int cx, int cy, int mouseX, int mouseY) {
        ...
        if (this.entryMultiblock != null) label = StatCollector.translateToLocal("am2.gui.ritual");
        else if (entry instanceof CompendiumEntrySpellModifier)
            label = StatCollector.translateToLocal("am2.gui.modifies");
        else label = StatCollector.translateToLocal("am2.gui.modifiedBy");
    ...
    private void drawRightPage_Entity(int cx, int cy) {
        ...
        fontRendererObj
            .drawString(renderString, cx - fontRendererObj.getStringWidth(renderString) / 2, i1 + 20, 0x000000);
    ...
    private void drawRightPageExtras_Block_Item(int cx, int cy) {
        ...
        if (craftingComponents == null) {
            ...
            GL11.glEnable(GL11.GL_BLEND);
            GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
            if (this.renderStack.getItem() == ItemsCommonProxy.essence
                || this.renderStack.getItem() == ItemsCommonProxy.deficitCrystal) {
    ...
    private void renderItemToolTip(ItemStack stack, int x, int y) {
        try {
            ...
            if (stack.getItem() instanceof ItemBlock) {
                ...
                if (entry == null) entry = ArcaneCompendium.instance.getEntry(name);
            } else {
    ...
    private void drawRightPage_Multiblock(int cx, int cy, int mousex, int mousey) {
        ...
        String label = String.format(
            "%s: %s",
            StatCollector.translateToLocal("am2.gui.layer"),
            curLayer == -1 ? StatCollector.translateToLocal("am2.gui.all") : "" + curLayer);

        fontRendererObj.drawString(label, cx - fontRendererObj.getStringWidth(label) / 2, cy - 90, 0x000000);

        GL11.glPushMatrix();
        GL11.glPushAttrib(
            GL11.GL_DEPTH_BUFFER_BIT | GL11.GL_COLOR_BUFFER_BIT | GL11.GL_TEXTURE_BIT | GL11.GL_LIGHTING_BIT);
    ...
    private BlockCoord getPickedBlock(int cx, int cy, int mousex, int mousey) {
        ...
        cy -= (int) (step_y * entryMultiblock.getMinLayer() / 2);
        cy -= (int) (step_y * entryMultiblock.getMaxLayer() / 2);
        ...
        for (int i = start; i <= end; ++i) {
            ...
            float px = cx - (step_x * ((float) entryMultiblock.getWidth() / 2));
            float py = cy - (step_z * ((float) entryMultiblock.getLength() / 2));
    ...
    private void drawMultiblockLayer(int cx, int cy, int layer, BlockCoord pickedBlock, int mousex, int mousey) {
        ...
        for (BlockCoord bc : layerBlocksSorted.keySet()) {
            ...
            BlockDec bd = layerBlocksSorted.get(bc)
                .get(
                    AMGuiHelper.instance.getSlowTicker() % layerBlocksSorted.get(bc)
                        .size());
            float x = px + ((bc.getX() - bc.getZ()) * step_x);
    ...
    private void RenderRecipe(int cx, int cy, int mousex, int mousey) {
        ...
        if (this.renderStack.getItem() == ItemsCommonProxy.essence
            ...
            for (int i = 0; i < craftingComponents.length; ++i) {
                ...
                float nextangle = (float) (Math
                    .toRadians((angleStep * ((i + 1) % craftingComponents.length)) + framecount % 360));
                float dist = 45;
        ...
        if (recipeHeight > 0) {
            ...
            fontRendererObj
                .drawString(label, cx - (int) (fontRendererObj.getStringWidth(label) / 2.5), cy - step * 3, 0x6600FF);
    ...
    private void renderCraftingComponent(int index, int sx, int sy, int mousex, int mousey) {
        ...
        if (craftingComponent instanceof ItemStack) {
            ...
            if (((List<?>) craftingComponent).isEmpty()) return;
        ...
        ArrayList<ItemStack> alternates = new ArrayList<>();
        if (stack != null) {
            ArrayList<ItemStack> oredict = OreDictionary.getOres(
                stack.getItem()
                    .getUnlocalizedName());
    ...
    private void renderBlock(Block block, int meta, float x, float y, int offsetX, int offsetY, int offsetZ,
        boolean picked) {
        ...
        if (picked) mc.renderEngine.bindTexture(red);
        else mc.renderEngine.bindTexture(terrain);
    ...
    public static ArrayList<String> splitStringToLines(FontRenderer fontRenderer, String string, int lineWidth,
        int maxLines) {
