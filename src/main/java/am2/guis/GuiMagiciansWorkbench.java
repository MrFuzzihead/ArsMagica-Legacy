...
public class GuiMagiciansWorkbench extends GuiContainer {
    ...
    private static final ResourceLocation background = new ResourceLocation(
        "arsmagica2",
        ResourceManager.GetGuiTexturePath("magicians_workbench.png"));
        ...
        if (par1GuiButton == clearGridOne) {
            for (int i = 0; i < 9; ++i) {
                ...
                this.mc.playerController
                    .windowClick(this.inventorySlots.windowId, slot.slotNumber, 0, 1, this.mc.thePlayer);
            }
    ...
    protected void mouseClicked(int par1, int par2, int par3) {
        ...
        if (hoverRecipeIndex > -1 && Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
            ((ContainerMagiciansWorkbench) this.inventorySlots).getWorkbench()
                .toggleRecipeLocked(hoverRecipeIndex);
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int mouseX, int mouseY) {
        ...
        else drawTexturedModalRect(l + 101, i1 + 37, 202, 162, 36, 36); // grid 2x2
        drawTexturedModalRect(l + 109, i1 + 87, 19, 225, 20, 20); // output

        GL11.glDisable(GL11.GL_BLEND);

        // tabs
        Iterator it = this.workbenchInventory.getRememberedRecipeItems()
            .iterator();
        int n = 0;
        recipeIndex = -1;
        hoverRecipeIndex = -1;
        for (n = 0; n < this.workbenchInventory.getRememberedRecipeItems()
            .size(); ++n) {
            int x = l + n * 20;
            int y = i1 + 136;
            drawTexturedModalRect(x, y, 0, 229, 19, 24);

            if (this.workbenchInventory.getRememberedRecipeItems()
                .get(n)
                .isLocked()) {
        ...
        while (it.hasNext()) {
            ...
            if (n == recipeIndex) {
                if (((ContainerMagiciansWorkbench) this.inventorySlots).isRecipeAlreadyInGrid(recipeIndex)) {
                    ...
                    ttList.add(
                        stack.output.getDisplayName() + " ("
                            + StatCollector.translateToLocal("am2.gui.componentsMissing")
                            + ")");
                    color = 0xFF0000;
                    recipeIndex = -1;
                }
                if (stack.isLocked()) ttList.add(StatCollector.translateToLocal("am2.tooltip.mwUnlock"));
                else ttList.add(StatCollector.translateToLocal("am2.tooltip.mwLock"));

                if (stack.is2x2) ttList.add("(2x2)");
                else ttList.add("(3x3)");
            }
            n++;
        }
