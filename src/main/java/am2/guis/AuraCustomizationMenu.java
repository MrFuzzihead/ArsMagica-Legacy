...
public class AuraCustomizationMenu extends GuiScreen {
    ...
    public void initGui() {
        ...
        btnParticleBehaviour = new GuiButtonVariableDims(
            11,
            50,
            60,
            ParticleController.AuraControllerOptions[AMCore.config.getAuraBehaviour()]);
        btnParticleColorMode = new GuiButtonVariableDims(
            12,
            50,
            80,
            AMCore.config.getAuraColorDefault() ? StatCollector.translateToLocal("am2.gui.default")
                : AMCore.config.getAuraColorRandom() ? StatCollector.translateToLocal("am2.gui.random")
                    : StatCollector.translateToLocal("am2.gui.custom"));

        btnParticleType.setDimensions(80, 20);
        btnParticleBehaviour.setDimensions(80, 20);
        btnParticleColorMode.setDimensions(80, 20);

        sliParticleScale = new GuiSlideControl(
            14,
            width - 110,
            40,
            100,
            StatCollector.translateToLocal("am2.gui.scale"),
            AMCore.config.getAuraScale() * 10,
            1f,
            200f);
        sliParticleAlpha = new GuiSlideControl(
            15,
            width - 110,
            60,
            100,
            StatCollector.translateToLocal("am2.gui.alpha"),
            AMCore.config.getAuraAlpha() * 100,
            1f,
            100f);
        sliParticleRed = new GuiSlideControl(
            16,
            width - 110,
            80,
            100,
            StatCollector.translateToLocal("am2.gui.red"),
            (AMCore.config.getAuraColor() >> 16) & 0xFF,
            0f,
            255f);
        sliParticleRed.setInteger(true);
        sliParticleGreen = new GuiSlideControl(
            17,
            width - 110,
            100,
            100,
            StatCollector.translateToLocal("am2.gui.green"),
            (AMCore.config.getAuraColor() >> 8) & 0xFF,
            0f,
            255f);
        sliParticleGreen.setInteger(true);
        sliParticleBlue = new GuiSlideControl(
            18,
            width - 110,
            120,
            100,
            StatCollector.translateToLocal("am2.gui.blue"),
            AMCore.config.getAuraColor() & 0xFF,
            0f,
            255f);
        sliParticleBlue.setInteger(true);

        sliParticleQuantity = new GuiSlideControl(
            20,
            width - 110,
            140,
            100,
            StatCollector.translateToLocal("am2.gui.qty"),
            AMCore.config.getAuraQuantity(),
            1,
            5);
        sliParticleQuantity.setInteger(true);

        sliParticleDelay = new GuiSlideControl(
            21,
            width - 110,
            160,
            100,
            StatCollector.translateToLocal("am2.gui.delay"),
            AMCore.config.getAuraDelay(),
            1,
            100);
        sliParticleDelay.setInteger(true);

        sliParticleSpeed = new GuiSlideControl(
            22,
            width - 110,
            180,
            100,
            StatCollector.translateToLocal("am2.gui.speed"),
            AMCore.config.getAuraSpeed(),
            0.05f,
            10.0f);
        ...
        this.buttonList.add(
            new GuiButton(200, this.width / 2 - 100, this.height - 28, StatCollector.translateToLocal("am2.gui.done")));
    ...
    protected void actionPerformed(GuiButton par1GuiButton) {
        ...
        switch (par1GuiButton.id) {
            ...
            case 13: // random color
                ...
                btnParticleColorMode.displayString = AMCore.config.getAuraColorDefault() ? "Default"
                    : AMCore.config.getAuraColorRandom() ? StatCollector.translateToLocal("am2.gui.random")
                        : StatCollector.translateToLocal("am2.gui.custom");
                break;
            case 14: // scale
            ...
            case 17: // green
            case 18: // blue
                int color = ((int) sliParticleRed.getShiftedValue() & 0xFF) << 16
                    | ((int) sliParticleGreen.getShiftedValue() & 0xFF) << 8
                    | (int) sliParticleBlue.getShiftedValue() & 0xFF;
        ...
        if (clickedBtn != null && button == 1) {
            if (clickedBtn.id == 10) {
                ...
                while (AMParticle.particleTypes[index].startsWith("lightning_bolt")
                    && !PlayerTracker.getAAL(Minecraft.getMinecraft().thePlayer)) {
                    index--;
                    if (index < 0) index = AMParticle.particleTypes.length - 1;
                }
    ...
    private GuiButton getControlByXY(int x, int y) {
        for (Object btn : this.buttonList) {
            if (btn instanceof GuiButton) {
                ...
                if (button.mousePressed(mc, x, y)) return button;
            }
        }
        return null;
    }
