...
public class GuiKeystone extends GuiContainer {
    ...
    private static final ResourceLocation background = new ResourceLocation(
        "arsmagica2",
        ResourceManager.GetGuiTexturePath("keystone_GUI.png"));
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
        ...
        hoveredCombo = -1;
        int numCombos = ItemsCommonProxy.keystone
    ...
    private void recalculateSlider() {
        int sliderMax = Math.max(
            0,
            ItemsCommonProxy.keystone.numCombinations(((ContainerKeystone) this.inventorySlots).getKeystoneStack())
                - 9);
        scrollBar.setMaximum(Math.max(sliderMax, 1));

        if (sliderMax == 0) scrollBar.enabled = false;
        else scrollBar.enabled = true;
    }
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button == addCombination) {
            if (combinationName.getText() == null || combinationName.getText()
                .trim()
                .equals("")) {
                ...
                ItemsCommonProxy.keystone.addCombination(
                    ((ContainerKeystone) this.inventorySlots).getKeystoneStack(),
            ...
            if (matchedCombo == null) {
                ...
                ItemsCommonProxy.keystone
                    .removeCombination(((ContainerKeystone) this.inventorySlots).getKeystoneStack(), matchedCombo.name);

                recalculateSlider();
            }
        } else {

            int numCombinatons = ItemsCommonProxy.keystone
                .numCombinations(((ContainerKeystone) this.inventorySlots).getKeystoneStack());
            ...
            if (numCombinatons == 1) {
                ...
                if (button == nextCombination) {
                    currentCombination++;
                    if (currentCombination >= numCombinatons) currentCombination = 0;
                    ...
                    currentCombination--;
                    if (currentCombination < 0) currentCombination = numCombinatons - 1;
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        for (int i = comboScrollOffset; i < numCombos; ++i) {
            KeystoneCombination combo = ItemsCommonProxy.keystone
