...
public class LightningBoltCommon {
    ...
    public LightningBoltCommon(World world, Entity detonator, Entity target, long seed, int speed) {
        this(
            world,
            new AMVector3(detonator),
            new AMVector3(target.posX, target.posY + target.getEyeHeight() - 0.699999988079071D, target.posZ),
            seed);
    ...
    public void fractal(int splits, float amount, float splitchance, float splitlength, float splitangle) {
        ...
        for (Iterator iterator = oldsegments.iterator(); iterator.hasNext();) {
            ...
            prev = segment.prev;
            AMVector3 subsegment = segment.diff.copy()
                .scale(1.0F / splits);
            ...
            for (int i = 1; i < splits; i++) {
                ...
                AMVector3 basepoint = startpoint.copy()
                    .add(
                        subsegment.copy()
                            .scale(i));
    ...
    private float rayTraceResistance(AMVector3 start, AMVector3 end, float prevresistance) {
        ...
        if (mop.typeOfHit == MovingObjectType.BLOCK) {
             ...
             * return prevresistance + (isr.getSpecialExplosionResistance(this.world, mop.blockX, mop.blockY,
             * mop.blockZ, start.x, start.y, start.z, this.wrapper) + 0.3F);
             * }
             */
    ...
    private void calculateCollisionAndDiffs() {
        ...
        if (((Integer) lastactivesegment.get(0)).intValue() + 1 < this.numsegments0) this.canhittarget = false;
    }
    ...
    public class SegmentSorter implements Comparator {
        ...
        public int compare(LightningBoltCommon.Segment o1, LightningBoltCommon.Segment o2) {
            ...
            if (comp == 0) {
                return Integer.valueOf(o1.segmentno)
                    .compareTo(Integer.valueOf(o2.segmentno));
            }
            return comp;
        }
    ...
    public class Segment {
        ...
        public void calcEndDiffs() {
            if (this.prev != null) {
                AMVector3 prevdiffnorm = this.prev.diff.copy()
                    .normalize();
                AMVector3 thisdiffnorm = this.diff.copy()
                    .normalize();
            ...
            if (this.next != null) {
                AMVector3 nextdiffnorm = this.next.diff.copy()
                    .normalize();
                AMVector3 thisdiffnorm = this.diff.copy()
                    .normalize();
        ...
        public Segment(LightningBoltCommon.BoltPoint start, LightningBoltCommon.BoltPoint end, float light,
        ...
        public Segment(AMVector3 start, AMVector3 end) {
            this(
                new LightningBoltCommon.BoltPoint(start, new AMVector3(0.0D, 0.0D, 0.0D)),
                new LightningBoltCommon.BoltPoint(end, new AMVector3(0.0D, 0.0D, 0.0D)),
                1.0F,
                0,
                0);
        }
