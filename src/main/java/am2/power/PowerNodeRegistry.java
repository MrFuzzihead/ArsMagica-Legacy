...
public class PowerNodeRegistry {
    ...
    static final int POWER_SEARCH_RADIUS_SQ = 100; // The power search radius squared. This is used for faster vector
                                                   // math.
    ...
    public static final PowerNodeRegistry For(World world) {
        if (world == null) return dummyRegistry;

        HashMap<Integer, PowerNodeRegistry> dimensionPowerManagers = world.isRemote ? clientDimensionPowerManagers
            : serverDimensionPowerManagers;
    ...
    PowerNodeEntry registerPowerNodeInternal(IPowerNode node) {
        ...
        if (powerNodes.containsKey(chunk)) {
            ...
            if (compound == null || !compound.hasKey("AM2PowerData")) {
                ...
                LogHelper.trace(
                    "Prior node list not found.  Created Power Node list for chunk %d, %d",
                    chunk.chunkXPos,
                    chunk.chunkZPos);
            } else {
                ...
                nodeList = powerNodes.get(chunk);
                // sanity check
                if (nodeList == null) nodeList = new HashMap<AMVector3, PowerNodeEntry>();
        ...
        if (nodeList.containsKey(nodeLoc)) return nodeList.get(nodeLoc);

        PowerNodeEntry pnd = new PowerNodeEntry();

        nodeList.put(nodeLoc, pnd);
        LogHelper.trace(
            "Successfully registered power node at {%d, %d, %d}",
            ((TileEntity) node).xCoord,
            ((TileEntity) node).yCoord,
            ((TileEntity) node).zCoord);
     ...
     * @return A localized message to return to the entity attempting to pair the nodes, either of success or why it
     *         failed.
     */
    public String tryPairNodes(IPowerNode powerSource, IPowerNode destination) {
        ...
        for (PowerTypes type : typesProvided) {
            ...
            PowerNodePathfinder pathfinder = new PowerNodePathfinder(
                ((TileEntity) powerSource).getWorldObj(),
                sourceLocation,
                destLocation,
                type);
        ...
        if (successes == typesProvided.size()) return StatCollector.translateToLocal("am2.tooltip.success");
    ...
    private void removePowerNode(ChunkCoordIntPair chunk, AMVector3 location) {
        ...
        if (powerNodes.containsKey(chunk)) {
            ...
            LogHelper.error(
                "Power Node removal requested in a non-tracked chunk (%d, %d)!",
                chunk.chunkXPos,
                chunk.chunkZPos);
        }
    }
    ...
    public float requestPower(IPowerNode destination, PowerTypes type, float amount) {
        ...
        float requested = data
            .requestPower(((TileEntity) destination).getWorldObj(), type, amount, destination.getCapacity());
    ...
    public float consumePower(IPowerNode consumer, PowerTypes type, float amount) {
        ...
        if (availablePower < amount) amount = availablePower;
    ...
    public float insertPower(IPowerNode destination, PowerTypes type, float amount) {
        ...
        if (curPower + amount > destination.getCapacity()) amount = destination.getCapacity() - curPower;
    ...
    public void setPower(IPowerNode destination, PowerTypes type, float amount) {
        ...
        if (amount > destination.getCapacity()) amount = destination.getCapacity();
    ...
    public boolean checkPower(IPowerNode node, float amount) {
        for (PowerTypes type : PowerTypes.all()) if (checkPower(node, type, amount)) return true;
        return false;
    }
    ...
    PowerNodeEntry getPowerNodeData(IPowerNode node) {
        ...
        if (pair != null && powerNodes.containsKey(pair)) {
            PowerNodeEntry pnd = powerNodes.get(pair)
                .get(new AMVector3((TileEntity) node));
            if (pnd != null) return pnd;
        }
    ...
    public IPowerNode[] getAllNearbyNodes(World world, AMVector3 location, PowerTypes power) {
        ...
        if (deadNodesRemoved > 0) LogHelper.trace("Removed %d dead power nodes", deadNodesRemoved);
    ...
    private ChunkCoordIntPair[] getSearchChunks(AMVector3 location) {
        ...
        for (int i = -1; i <= 1; ++i) {
            for (int j = -1; j <= 1; ++j) {
                // don't include the home chunk, this has already been added. We're only concerned with neighboring
                // chunks.
                if (i == 0 && j == 0) continue;
                // create the new CCIP at the offset location
                ChunkCoordIntPair newPair = new ChunkCoordIntPair(chunkX + i, chunkZ + j);
                // offset the x/z locations by POWER_SEARCH_RADIUS * i/j respectively and calculate the chunk that would
                // fall in.
                // If it matches the new chunk coordinates, we add that chunk to the list of chunks to search.
                if (((int) location.x + (POWER_SEARCH_RADIUS * i)) >> 4 == newPair.chunkXPos
                    && ((int) location.z + (POWER_SEARCH_RADIUS * j)) >> 4 == newPair.chunkZPos) {
    ...
    public void LoadChunkFromNBT(ChunkCoordIntPair chunk, NBTTagCompound compound) {
        if (!compound.hasKey("AM2PowerData")) return; // nothing was saved in this chunk for the power system
        ...
        for (int i = 0; i < powerNodeTagList.tagCount(); ++i) {
            ...
            AMVector3 nodeLocation = new AMVector3(
                nodeCompound.getInteger("xCoord"),
                nodeCompound.getInteger("yCoord"),
                nodeCompound.getInteger("zCoord"));
            PowerNodeEntry pnd = new PowerNodeEntry();
    ...
    private class ChunkCoordComparator implements Comparator<ChunkCoordIntPair> {
        ...
        public int compare(ChunkCoordIntPair a, ChunkCoordIntPair b) {
            if (a.chunkXPos == b.chunkXPos && a.chunkZPos == b.chunkZPos) return 0;

            if (a.chunkXPos > b.chunkXPos) return 1;
            else if (a.chunkZPos > b.chunkZPos) return 1;
            return -1;
        }
    ...
    private class IPowerNodeComparer implements Comparator<IPowerNode> {
        ...
        public int compare(IPowerNode o1, IPowerNode o2) {
            ...
            if (te1.xCoord == te2.xCoord && te1.zCoord == te2.zCoord && te1.yCoord == te2.yCoord) return 0;
