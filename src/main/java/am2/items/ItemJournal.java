...
public class ItemJournal extends ArsMagicaItem {
        ...
        if (!player.worldObj.isRemote) {
            if (getOwner(journal) == null) {
                ...
                player.addChatMessage(
                    new ChatComponentText(StatCollector.translateToLocal("am2.tooltip.notYourJournal")));
    ...
    private void addXPToJournal(ItemStack journal, int amount) {
        if (!journal.hasTagCompound()) journal.stackTagCompound = new NBTTagCompound();
