...
public class ItemSpellStaff extends ArsMagicaItem {
    ...
    public EnumAction getItemUseAction(ItemStack par1ItemStack) {
        if (isMagiTechStaff()) return EnumAction.none;
    ...
    public static ItemStack setSpellScroll(ItemStack stack, ItemStack spell) {
        ...
        if (!compound.hasKey(NBT_CHARGE)) compound.setFloat(NBT_CHARGE, ((ItemSpellStaff) stack.getItem()).maxCharge);
        return stack;
    }

    public static ItemStack copyChargeFrom(ItemStack my_stack, ItemStack stack) {
        if (stack.getItem() instanceof ItemSpellStaff && stack.stackTagCompound != null
            && stack.stackTagCompound.hasKey("current_charge")) {
    ...
    public void addInformation(ItemStack stack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        par3List.add(
            StatCollector.translateToLocal("am2.tooltip.charge") + ": " + (int) chargeRemaining + " / " + maxCharge);
    ...
    public String getItemStackDisplayName(ItemStack par1ItemStack) {
        ...
        if (par1ItemStack.hasTagCompound() && par1ItemStack.getTagCompound()
            .hasKey(NBT_SPELL_NAME))
            name += " (\2479" + par1ItemStack.getTagCompound()
                .getString(NBT_SPELL_NAME) + "\2477)";
        return name;
    }
                ...
                if (te instanceof IPowerNode) {
                    if (player.isSneaking()) {
                        ...
                        for (PowerTypes type : types) {
                            float power = PowerNodeRegistry.For(world)
                                .getPower((IPowerNode) te, type);
    ...
    public void onPlayerStoppedUsing(ItemStack itemstack, World world, EntityPlayer entityplayer, int i) {
        ...
        if (spell != null) {
            ...
            if (shape != null) {
                if (!shape.isChanneled()) if (SpellHelper.instance.applyStackStage(
                    spell,
                    entityplayer,
                    null,
                    entityplayer.posX,
                    ...
                    false,
                    false,
                    i) == SpellCastResult.SUCCESS) consumeStaffCharge(itemstack, entityplayer);
                if (world.isRemote && shape.isChanneled()) {
                    // SoundHelper.instance.stopSound(shape.getSoundForAffinity(SpellUtils.instance.mainAffinityFor(spell),
                    // spell, null));
                }
            }
        }
        ...
        if (entity instanceof EntityPlayerSP) {
            ...
            if (usingItem != null && usingItem.getItem() == this) {
                if (SkillData.For(player)
                    .isEntryKnown(
                        SkillTreeManager.instance.getSkillTreeEntry(SkillManager.instance.getSkill("SpellMotion")))) {
    ...
    private ItemStack getSpellStack(ItemStack staffStack) {
        if (!staffStack.hasTagCompound() || !staffStack.stackTagCompound.hasKey(NBT_SPELL)) return null;
        ItemStack stack = new ItemStack(ItemsCommonProxy.spell);
        stack.readFromNBT(
            staffStack.getTagCompound()
                .getCompoundTag(NBT_SPELL));
        return stack;
    }

    @Override
    public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer) {
        if (isMagiTechStaff()) return itemstack;

        if (getSpellStack(itemstack) != null) entityplayer.setItemInUse(itemstack, getMaxItemUseDuration(itemstack));
    ...
    public void onUsingTick(ItemStack itemstack, EntityPlayer player, int count) {
        ...
        if (spell != null) {
            ...
            if (SpellHelper.instance.applyStackStageOnUsing(
                spell,
                player,
                player,
                player.posX,
                ...
                false,
                true,
                count - 1) == SpellCastResult.SUCCESS) consumeStaffCharge(itemstack, player);
        }
    }

    @Override
    public int getDamage(ItemStack stack) {
        if (!stack.hasTagCompound()) return super.getDamage(stack);
        float chargeRemaining = stack.stackTagCompound.getFloat(NBT_CHARGE);
