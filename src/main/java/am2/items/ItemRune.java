...
public class ItemRune extends ArsMagicaItem {
    ...
    public void registerIcons(IIconRegister IIconRegister) {
        textures = new String[] { "rune_black", "rune_blank", "rune_blue", "rune_brown", "rune_cyan", "rune_gray",
            "rune_green", "rune_light_blue", "rune_light_gray", "rune_lime", "rune_magenta", "rune_orange", "rune_pink",
            "rune_purple", "rune_red", "rune_white", "rune_yellow", "infinityorb_blue", "infinityorb_green",
            "infinityorb_red", "debug_placeholder" };
    ...
    public IIcon getIconFromDamage(int damage) {
        if (damage < 0 || damage >= this.icons.length) return this.icons[0];
    ...
    private ItemStack doGiveSkillPoints(EntityPlayer player, ItemStack stack, SkillPointTypes type) {
        if (ExtendedProperties.For(player)
            ...
            SkillData.For(player)
                .incrementSpellPoints(type);
            if (player.worldObj.isRemote) {
                switch (type) {
                    case BLUE:
                        player.addChatMessage(
                            new ChatComponentText(StatCollector.translateToLocal("am2.tooltip.infOrbBlue")));
                        break;
                    case GREEN:
                        player.addChatMessage(
                            new ChatComponentText(StatCollector.translateToLocal("am2.tooltip.infOrbGreen")));
                        break;
                    case RED:
                        player.addChatMessage(
                            new ChatComponentText(StatCollector.translateToLocal("am2.tooltip.infOrbRed")));
                        break;
                    default:
                        return stack;
                }
            ...
            if (player.worldObj.isRemote) {
                ...
                player.addChatMessage(
                    new ChatComponentText(StatCollector.translateToLocal("am2.tooltip.infOrbFail" + message)));
            }
        }
        return stack;
    }
    ...
    public void displayBlockMeta(ItemStack stack, World world, EntityPlayer player) {
        if (world.isRemote) {
            ...
            if (mop != null && mop.typeOfHit == MovingObjectType.BLOCK) {
                ...
                player.addChatMessage(
                    new ChatComponentText(String.format("%s, Meta: %d", block.getLocalizedName(), meta)));
            }
        }
    }
    ...
    public void displaymana(EntityPlayer player) {
        ...
        int mana = ExtendedProperties.For(player)
            .getMagicLevel();
    ...
    public void spawnRibbonParticle(ItemStack stack, World world, EntityPlayer player) {
        ...
        AMCore.proxy.particleManager.RibbonFromPointToPoint(
            ...
            player.posY,
            player.posZ,
            player.posX + (target.xCoord * 10),
            player.posY + (target.yCoord * 10),
            player.posZ + (target.zCoord * 10));
    }
    ...
    public void spawnLineArcParticle(ItemStack stack, World world, EntityPlayer player) {
        if (world.isRemote) {
            ...
            AMLineArc arc = (AMLineArc) AMCore.proxy.particleManager.spawn(
                ...
                player.posY,
                player.posZ,
                player.posX + (look.xCoord * dist),
                player.posY + (look.yCoord * dist),
                player.posZ + (look.zCoord * dist));
            if (arc != null) {
    ...
    public void spawnWall(ItemStack stack, World world, EntityPlayer player) {
        if (!world.isRemote) {
            ...
            SpellUtils.instance
                .addSpellStageToScroll(spellStack, "touch", new String[] { "FireDamage" }, new String[0]);
    ...
    public static void buildAltar(World world, int x, int y, int z, int w, ItemStack struct, ItemStack stairs,
        ItemStack aug, Builder builder) {
        boolean ns = w % 2 == 0;
        ...
        int lecternSide = 4 - w;
        builder
            .apply(world, x + sgn * rot, y + 1, z + rot, new ItemStack(BlocksCommonProxy.blockLectern, 1, lecternSide));
    }
