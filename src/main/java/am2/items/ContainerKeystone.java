...
public class ContainerKeystone extends Container {
    ...
    public ContainerKeystone(InventoryPlayer inventoryplayer, ItemStack bookStack, ItemStack runeBagStack,
        InventoryKeyStone inventoryKeystone, InventoryRuneBag runeBag, int runeBagSlot) {
    ...
    public ItemStack[] getRuneBagInventory() {
        ...
        for (int i = InventoryKeyStone.inventorySize; i
            < InventoryKeyStone.inventorySize + InventoryRuneBag.inventorySize; ++i) {
    ...
    public boolean setInventoryToCombination(int comboIndex) {
        ...
        if (player.worldObj.isRemote) {
            AMNetHandler.INSTANCE.sendPacketToServer(
                AMPacketIDs.SET_KEYSTONE_COMBO,
                new AMDataWriter().add(comboIndex)
                    .generate());
            return true;
        }
        ...
        while (matchIndex < combo.metas.length
            && searchIndex < InventoryKeyStone.inventorySize + (hasrunebag ? InventoryRuneBag.inventorySize : 0)) {
            IInventory searchInventory = searchIndex >= InventoryKeyStone.inventorySize ? runeBag : keyStoneInventory;
            int inventoryIndex = searchIndex >= InventoryKeyStone.inventorySize
                ? searchIndex - InventoryKeyStone.inventorySize
                : searchIndex;
    ...
    private void swapInventorySlots(IInventory firstInventory, IInventory secondInventory, int slot1, int slot2) {
        ...
        if (firstInventory == secondInventory && slot1 == slot2) return;
