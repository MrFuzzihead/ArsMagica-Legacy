...
public class RecipeArsMagica {
    ...
    public boolean recipeIsMatch(ItemStack[] recipeItems) {
        ...
        for (int i = 0; i < recipeItems.length; ++i) {
            if (recipeItems[i] == null && craftingComponents[i] == null) continue;
            if (recipeItems[i] == null && craftingComponents[i] != null) return false;
            if (recipeItems[i] != null && craftingComponents[i] == null) return false;
            if (recipeItems[i].getItem() != craftingComponents[i].getItem()
                || (craftingComponents[i].getItemDamage() != AMCore.ANY_META
    ...
    public ItemStack getFuelID() {
        if (craftingComponents.length == 5) return craftingComponents[2];
        return null;
    }
}
