...
public class SpellBase extends ItemSpellBase {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer caster) {
        if (!stack.hasDisplayName()) {
            if (!world.isRemote) FMLNetworkHandler.openGui(
                caster,
                AMCore.instance,
                ArsMagicaGuiIdList.GUI_SPELL_CUSTOMIZATION,
                world,
                (int) caster.posX,
                (int) caster.posY,
                (int) caster.posZ);
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World world, EntityPlayer player, int ticksUsed) {
        ...
        if (shape != null) {
            if (!shape.isChanneled()) SpellHelper.instance
                .applyStackStage(stack, player, null, player.posX, player.posY, player.posZ, 0, world, true, true, 0);
        }
    }

    @Override
    public void onUsingTick(ItemStack stack, EntityPlayer caster, int count) {
        SpellHelper.instance.applyStackStageOnUsing(
            stack,
            caster,
            caster,
            caster.posX,
    ...
    @Override
    public MovingObjectPosition getMovingObjectPosition(EntityLivingBase caster, World world, double range,
        boolean includeEntities, boolean targetWater) {
        ...
        if (entityPos != null && mop != null) {
            if (new AMVector3(mop.hitVec).distanceSqTo(new AMVector3(caster))
                < new AMVector3(entityPos.hitVec).distanceSqTo(new AMVector3(caster))) {
                return mop;
            } else {
                return entityPos;
            }
