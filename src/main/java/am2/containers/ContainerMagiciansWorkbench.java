...
public class ContainerMagiciansWorkbench extends AM2Container {
    ...
    public ContainerMagiciansWorkbench(InventoryPlayer playerInventory, TileEntityMagiciansWorkbench tileEntity,
        ...
        if (tileEntity.getUpgradeStatus(TileEntityMagiciansWorkbench.UPG_CRAFT)) INVENTORY_STORAGE_START += 5;
        ...
        secondCraftMatrix = tileEntity.getUpgradeStatus(TileEntityMagiciansWorkbench.UPG_CRAFT)
            ? new InventoryCrafting(this, 3, 3)
            : new InventoryCrafting(this, 2, 2);
        ...
        // first output
        addSlotToContainer(
            new SlotMagicianWorkbenchOutput(playerInventory.player, firstCraftMatrix, firstoutput, this, 0, 37, 89));
        ...
        // second output
        addSlotToContainer(
            new SlotMagicianWorkbenchOutput(playerInventory.player, secondCraftMatrix, secondoutput, this, 0, 111, 89));
    ...
    public void onCraftMatrixChanged(IInventory inventory) {
        if (inventory == firstCraftMatrix) {
            this.firstoutput.setInventorySlotContents(
                0,
                CraftingManager.getInstance()
                    .findMatchingRecipe(firstCraftMatrix, world));
        } else if (inventory == secondCraftMatrix) {
            this.secondoutput.setInventorySlotContents(
                0,
                CraftingManager.getInstance()
                    .findMatchingRecipe(secondCraftMatrix, world));
        }
    }

    @Override
    public boolean canInteractWith(EntityPlayer entityplayer) {
        if ((world.getTileEntity(workbenchInventory.xCoord, workbenchInventory.yCoord, workbenchInventory.zCoord))
            != workbenchInventory) {
            return false;
        }
        return entityplayer.getDistanceSq(
            workbenchInventory.xCoord + 0.5,
            workbenchInventory.yCoord + 0.5,
            workbenchInventory.zCoord + 0.5) <= 64D;
    }
    ...
    public HashMap<ImmutablePair<Item, Integer>, Integer> getComponentCount(int recipeIndex) {
        ...
        RememberedRecipe recipe = this.workbenchInventory.getRememberedRecipeItems()
            .get(recipeIndex);
        for (ItemStack stack : recipe.components) {
            if (stack == null) continue;
            ImmutablePair<Item, Integer> pair = new ImmutablePair<Item, Integer>(
                stack.getItem(),
                stack.getItemDamage());
    ...
    private boolean hasComponent(ItemStack component, int qty) {
        int matchedQty = 0;
        for (int i = getWorkbench().getStorageStart() - 3; i
            < getWorkbench().getStorageStart() - 3 + getWorkbench().getStorageSize(); ++i) {
            ItemStack stack = getWorkbench().getStackInSlot(i);
            if (stack != null && stack.isItemEqual(component)) matchedQty += stack.stackSize;
            if (matchedQty >= qty) return true;
        }
        return false;
    }
    ...
    public boolean gridIsFreeFor(int recipeIndex) {
        ...
        if (recipe.components.length > 4
            && !this.workbenchInventory.getUpgradeStatus(TileEntityMagiciansWorkbench.UPG_CRAFT)) {
    ...
    private boolean craftingGridIsEmpty(boolean second) {
        if (!second) {
            for (int i = 0; i < 9; ++i) {
                if (getWorkbench().getStackInSlot(i) != null) return false;
            }
        } else {
            for (int i = 0; i < 9; ++i) {
                if (getWorkbench().getStackInSlot(i + 9) != null) return false;
            }
        }
        return true;
    }
    ...
    public void moveRecipeToCraftingGrid(int recipeIndex) {
        if (!gridIsFreeFor(recipeIndex) || isRecipeAlreadyInGrid(recipeIndex)) return;
        if (world.isRemote) {
    ...
    public boolean isRecipeAlreadyInGrid(int recipeIndex) {
        RememberedRecipe recipe = this.workbenchInventory.getRememberedRecipeItems()
            .get(recipeIndex);
        if (firstoutput.getStackInSlot(0) != null && firstoutput.getStackInSlot(0)
            .isItemEqual(recipe.output)) return true;
        else if (secondoutput.getStackInSlot(0) != null && secondoutput.getStackInSlot(0)
            .isItemEqual(recipe.output)) return true;
        return false;
    }
}
