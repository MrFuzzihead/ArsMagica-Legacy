...
public class SlotMagiciansWorkbenchCrafting extends Slot {
    ...
    public SlotMagiciansWorkbenchCrafting(EntityPlayer player, IInventory craftMatrix, IInventory craftResult,
        ContainerMagiciansWorkbench workbench, int index, int x, int y) {
    ...
    protected void onCrafting(ItemStack itemCrafted) {
        ...
        for (int i = 0; i < components.length; ++i) {
            if (this.craftMatrix.getStackInSlot(i) != null) components[i] = this.craftMatrix.getStackInSlot(i)
                .copy();
            else components[i] = null;
        }

        this.workbench.getWorkbench()
            .rememberRecipe(itemCrafted, components, this.craftMatrix.getSizeInventory() == 4);
    }
    ...
    // for (int n = this.workbench.getWorkbench().getStorageStart(); n < this.workbench.getWorkbench().getStorageStart()
    // + this.workbench.getWorkbench().getStorageSize(); ++n){
