...
public class SlotInscriptionTable extends Slot {
    ...
    public boolean isItemValid(ItemStack par1ItemStack) {
        ...
        if (par1ItemStack.getItem() == Items.written_book
            && (par1ItemStack.getTagCompound() == null || !par1ItemStack.getTagCompound()
                .getBoolean("spellFinalized")))
            return true;
        else if (par1ItemStack.getItem() == Items.writable_book) return true;
        else if (par1ItemStack.getItem() == ItemsCommonProxy.spell) return true;
        return false;
    }

    @Override
    public void onPickupFromSlot(EntityPlayer par1EntityPlayer, ItemStack par2ItemStack) {
        if (par2ItemStack.getItem() == Items.written_book) par2ItemStack = ((TileEntityInscriptionTable) this.inventory)
            .writeRecipeAndDataToBook(par2ItemStack, par1EntityPlayer, "Spell Recipe");
        else((TileEntityInscriptionTable) this.inventory).clearCurrentRecipe();
