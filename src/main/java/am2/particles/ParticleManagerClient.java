...
public class ParticleManagerClient extends ParticleManagerServer {
    ...
    @Override
    public AMLineArc spawn(World world, String name, double x, double y, double z, double targetX, double targetY,
        double targetZ) {
    ...
    @Override
    public void BoltFromEntityToEntity(World world, Entity caster, Entity source, Entity target, int damage, int type,
        int color) {
        ...
        LightningBolt bolt = new LightningBolt(
            ...
            pz,
            xx,
            target.boundingBox.minY + target.height / 2.0F,
            zz,
            world.rand.nextLong(),
            6,
            0.3F,
    ...
    @Override
    public void BoltFromPointToPoint(World world, double startX, double startY, double startZ, double endX, double endY,
        double endZ) {
    ...
    @Override
    public void BoltFromEntityToPoint(World world, Entity source, double endX, double endY, double endZ, int type,
        int color) {
        BoltFromPointToPoint(
            world,
            source.posX,
    ...
    @Override
    public void BoltFromPointToPoint(World world, double startX, double startY, double startZ, double endX, double endY,
        double endZ, int type, int color) {
    ...
    @Override
    public Object BeamFromPointToPoint(World world, double startX, double startY, double startZ, double endX,
        double endY, double endZ, int color) {
        if (AMCore.config.NoGFX()) {
            return null;
        }
    ...
    public Object BeamFromEntityToPoint(World world, Entity source, double endX, double endY, double endZ, int color) {
        if (!(source instanceof EntityPlayer) || source == AMCore.proxy.getLocalPlayer()) {
            ...
            return BeamFromPointToPoint(
                world,
                source.posX,
                source.posY + source.getEyeHeight() - 0.2f,
                source.posZ,
                endX,
                endY,
                endZ,
    ...
    @Override
    public void RibbonFromPointToPoint(World world, double startX, double startY, double startZ, double endX,
        double endY, double endZ) {
    ...
    @Override
    public void RibbonFromPointToPoint(World world, double startX, double startY, double startZ, double endX,
        double endY, double endZ, int type) {
            ...
            if (Minecraft.getMinecraft().thePlayer != ent
                ...
                if (AMParticle.particleTypes[particleIndex].startsWith("lightning_bolts")) {
                    int type = Integer.parseInt(
                        String.valueOf(
                            AMParticle.particleTypes[particleIndex]
                                .charAt(AMParticle.particleTypes[particleIndex].length() - 1)));
                            ...
                            switch (particleBehaviour) {
                                case 0: // fade
                                    effect.AddParticleController(
                                        new ParticleFadeOut(effect, 1, false).setFadeSpeed(particleSpeed));
                                    break;
                                case 1: // float
                                    effect.AddParticleController(
                                        new ParticleFloatUpward(effect, 0.2f, particleSpeed, 1, false));
                                    break;
                                case 2: // sink
                                    effect.AddParticleController(
                                        new ParticleFloatUpward(effect, 0.2f, -particleSpeed, 1, false));
                                ...
                                case 3: // orbit
                                    effect.AddParticleController(
                                        new ParticleOrbitEntity(effect, ent, particleSpeed, 1, false));
                                    break;
                                case 4: // arc
                                    effect.AddParticleController(
                                        new ParticleArcToEntity(effect, 1, ent, false).generateControlPoints()
                                            .SetSpeed(particleSpeed));
                                    break;
                                case 5: // flee
                                    effect.AddParticleController(
                                        new ParticleFleeEntity(effect, ent, particleSpeed, 2D, 1, false));
                                    break;
                                case 6: // forward
                                    effect.AddParticleController(
                                ...
                                case 7: // pendulum
                                    effect.AddParticleController(
                                        new ParticlePendulum(effect, 0.2f, particleSpeed, 1, false));
                                    break;
                                case 8: // grow
                ...
                for (int i = -radius; i <= radius; ++i) {
                    for (int j = -radius; j <= radius; ++j) {
                        for (int k = -radius; k <= radius; ++k) {
                            if (entityliving.worldObj.getBlock(
                                (int) entityliving.posX + i,
                                (int) entityliving.posY + j,
                                (int) entityliving.posZ + k) == Blocks.air
                                && entityliving.worldObj.getBlockLightValue(
                                    (int) entityliving.posX + i,
                                    (int) entityliving.posY + j,
                                    (int) entityliving.posZ + k) <= 7) {
                                AMParticle effect = spawn(
                                    world,
                                    "hr_sparkles_1",
                                ...
                                if (effect != null) {
                                    effect.setRGBColorF(
                                        world.rand.nextFloat() * 0.4f + 0.3f,
                                        0.6f,
                                        world.rand.nextFloat() * 0.4f + 0.6f);
                                    effect.setIgnoreMaxAge(false);
                                    effect.setMaxAge(40);
                                    effect
                                        .AddParticleController(new ParticleFloatUpward(effect, 0.01f, 0.01f, 1, false));
                                    effect.AddParticleController(
                                        new ParticleFadeOut(effect, 2, false).setFadeSpeed(0.01f));
                                }
                            }
                        }
                    }
    ...
    @Override
    public ParticleController createDefaultParticleController(int type, Object eff, AMVector3 location, float modifier,
        int meta) {
        ...
        switch (type) {
            ...
            case 2: // sink
                return new ParticleFloatUpward(effect, 0.01f, -0.03f * modifier, 1, false);
            case 3: // orbit
                return new ParticleOrbitPoint(effect, location.x, location.y, location.z, 1, false)
                    .SetOrbitSpeed(0.05f * modifier);
            case 4: // arc
                return new ParticleArcToPoint(effect, 1, location.x, location.y, location.z, false)
                    .generateControlPoints()
                    .SetSpeed(0.02f * modifier);
            case 5: // flee
                // return new ParticleFleePoint(effect, location.add(new AMVector3(0.5, 0.5, 0.5)), 0.02f * modifier,
                // 1.5f, 1, false);
                return new ParticleMoveOnHeading(
                    effect,
                    effect.worldObj.rand.nextInt(360),
                    0,
                    0.02f * modifier,
                    1,
                    false);
            case 6: // forward
                if ((meta & ~0x8) == 3) meta &= ~0x2;
                else if ((meta & ~0x8) == 1) meta |= 0x2;
