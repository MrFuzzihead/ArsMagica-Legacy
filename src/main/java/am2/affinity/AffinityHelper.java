...
public class AffinityHelper {
    ...
    public void onEntityLivingBase(LivingUpdateEvent event) {
        ...
        AffinityModifiers.instance
            .applySpeedModifiersBasedOnDepth((EntityPlayer) ent, natureDepth, iceDepth, lightningDepth);
        AffinityModifiers.instance
            .applyHealthModifiers((EntityPlayer) ent, enderDepth, waterDepth, fireDepth, lightningDepth);
        ...
        if (natureDepth == 1.0f) {
            if (ent.worldObj.canBlockSeeTheSky((int) ent.posX, (int) ent.posY, (int) ent.posZ)
                && ent.worldObj.isDaytime()) {
                affinityData.accumulatedHungerRegen += 0.02f;
                if (affinityData.accumulatedHungerRegen > 1.0f) {
                    ((EntityPlayer) ent).getFoodStats()
                        .addStats(1, 0.025f);
            ...
            if (ent.isCollidedHorizontally) {
                if (!ent.isSneaking()) {
                    float movement = ExtendedProperties.For(ent)
                        .getIsFlipped() ? -0.25f : 0.25f;
        ...
        if (enderDepth >= 0.75f && affinityData.hasActivatedNightVision()) {
            if (!ent.worldObj.isRemote
                && (!ent.isPotionActive(Potion.nightVision.id) || ent.getActivePotionEffect(Potion.nightVision)
                    .getDuration() <= 220)) {
                ent.addPotionEffect(new PotionEffect(Potion.nightVision.id, 300, 1));
            }
        }

        if (ent.onGround) affinityData.setLastGroundPosition(new AMVector3(ent));
        ...
        if (ent.isInWater()) {
            ...
            if (waterDepth > 0.4 && ent.worldObj.rand.nextInt(20) < 4) ent.setAir(ent.getAir() + 1);

            if (!ent.worldObj.isRemote && ent.worldObj.rand.nextInt(100) < 5) {
                ent.setAir(ent.getAir() + 1);
                byte[] data = new AMDataWriter().add(ent.getEntityId())
                    .add(ent.getAir())
                    .generate();
                AMNetHandler.INSTANCE.sendPacketToClientPlayer((EntityPlayerMP) ent, AMPacketIDs.SYNC_AIR_CHANGE, data);
            }

            if ((ent instanceof EntityPlayer && ((EntityPlayer) ent).inventory.armorInventory[1] != null
                && ((EntityPlayer) ent).inventory.armorInventory[1].getItem() == ItemsCommonProxy.waterGuardianOrbs)) {
                waterMovementFlag = true;
                if (!ent.worldObj.isRemote && (!ent.isPotionActive(BuffList.waterBreathing)
                    || ent.getActivePotionEffect(BuffList.waterBreathing)
                        .getDuration() <= 200))
                    ent.addPotionEffect(new BuffEffectWaterBreathing(400, 2));
            } else if (ent instanceof EntityPlayer && ((EntityPlayer) ent).inventory.armorInventory[1] != null
                && ((EntityPlayer) ent).inventory.armorInventory[1].getItem() == ItemsCommonProxy.archmageLeggings) {
        ...
        if (ent.worldObj.isRaining() && !ent.worldObj.isRemote
            && ent.worldObj.getBiomeGenForCoords((int) Math.floor(ent.posX), (int) Math.floor(ent.posZ))
            ...
            if (airDepth > 0.5f && airDepth < 0.85f && !ent.worldObj.isRemote && ent.worldObj.rand.nextInt(100) < 10) {
                if (!ent.isSneaking() && !ent.isPotionActive(BuffList.gravityWell)
                    && !ent.isInsideOfMaterial(Material.water)
                    && ent.isWet()) {
    ...
    private void applyFulmintion(EntityPlayer ent, float lightningDepth) {
        ...
        if (!ent.worldObj.isRemote) {
            if (lightningDepth > 0.5f && lightningDepth <= 0.8f) {
                int offsetX = (int) ent.posX - 5
                    + ent.getRNG()
                        .nextInt(11);
                int offsetY = (int) ent.posY - 5
                    + ent.getRNG()
                        .nextInt(11);
                int offsetZ = (int) ent.posZ - 5
                    + ent.getRNG()
                        .nextInt(11);
            ...
            if (lightningDepth >= 0.7f && lightningDepth <= 0.95f
                && ent.getRNG()
                    .nextDouble() < 0.05f) {
                List<EntityCreeper> creepers = ent.worldObj
                    .getEntitiesWithinAABB(EntityCreeper.class, ent.boundingBox.expand(5, 5, 5));
        ...
        if (lightningDepth > 0.25f && ent.isWet()) {
            ExtendedProperties.For(ent)
                .deductMana(100);
            if (ent.worldObj.isRemote) {
                AMCore.proxy.particleManager.BoltFromEntityToPoint(
                    ent.worldObj,
                    ent,
                    ent.posX - 2
                        + ent.getRNG()
                ...
                if (ent.getRNG()
                    ...
                    ent.worldObj.playSoundAtEntity(
                        ...
                        1.0f,
                        ent.worldObj.rand.nextFloat() + 0.5f);
            }
        }
    }

    private void makeIceBridge(EntityPlayer ent, float iceDepth) {
        AMVector3[] blocks = MathUtilities.GetHorizontalBlocksInFrontOfCharacter(
            ent,
            1,
            (int) Math.round(ent.posX),
            (int) Math.floor(ent.posY) - 1,
            (int) Math.round(ent.posZ));
        for (int i = 0; i < blocks.length; ++i) {
            ...
            for (int n = -1; n <= 1; ++n) {
                for (int p = -1; p <= 1; ++p) {
                    ...
                    if (iceDepth == 1.0f && block == Blocks.lava) ent.worldObj
                        .setBlock((int) current.x + n, (int) current.y, (int) current.z + p, Blocks.obsidian);
                    else if (iceDepth == 1.0f && block == Blocks.flowing_lava) ent.worldObj
                        .setBlock((int) current.x + n, (int) current.y, (int) current.z + p, Blocks.cobblestone);
                    ...
                    if (block == Blocks.fire) {
                        ent.worldObj
                            .setBlock((int) current.x + n, (int) current.y + 1, (int) current.z + p, Blocks.air);
                    }
                }
            }
        }
    ...
    private void applyReverseWaterMovement(EntityLivingBase entity) {
        ...
        if (!entity.worldObj.checkChunksExist(i, k, i1, j, l, j1)) {
                ...
                for (int l1 = k; l1 < l; ++l1) {
                    for (int i2 = i1; i2 < j1; ++i2) {
                        ...
                        if (block != null && block.getMaterial() == Material.water) {
                            double d0 = l1 + 1
                                - BlockLiquid.getLiquidHeightPercent(entity.worldObj.getBlockMetadata(k1, l1, i2));
            ...
            if (vec3.lengthVector() > 0.0D && entity.isInWater()) {
                ...
                // AMCore.proxy.packetSender.SendVelocityAddPacket(entity.worldObj, entity, vec3.xCoord * d1,
                // vec3.yCoord * d1, vec3.zCoord * d1);
            }
        }
    }
    ...
    public void onEntityJump(LivingJumpEvent event) {
        ...
        if (airDepth >= 0.5f) {
            ...
            if (ExtendedProperties.For(ent)
                .getIsFlipped()) velocity *= -1;
    ...
    public void onEntityHurt(LivingHurtEvent event) {
        ...
        if (event.source.getSourceOfDamage() instanceof EntityPlayer) {
            ...
            if (attackerLightningDepth > 0.75f && !ent.worldObj.isRemote
                && ((EntityPlayer) event.source.getSourceOfDamage()).getCurrentEquippedItem() == null) {
    ...
    public void onEntityDeath(LivingDeathEvent event) {
        if (event.entityLiving.getCreatureAttribute() != EnumCreatureAttribute.UNDEAD
            ...
            float lifeDepth = AffinityData.For(source)
    ...
    public void onSpellCast(SpellCastingEvent.Post event) {
        if (event.caster instanceof EntityPlayer && event.castResult == SpellCastResult.SUCCESS) {
            float affinityDepth = AffinityData.For(event.caster)
                .getAffinityDepth(Affinity.ARCANE);
